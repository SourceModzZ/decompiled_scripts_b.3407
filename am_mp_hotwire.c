#region Local Var
	var uLocal_0 = 0;
	var uLocal_1 = 0;
	int iLocal_2 = 0;
	int iLocal_3 = 0;
	int iLocal_4 = 0;
	int iLocal_5 = 0;
	int iLocal_6 = 0;
	int iLocal_7 = 0;
	int iLocal_8 = 0;
	int iLocal_9 = 0;
	int iLocal_10 = 0;
	int iLocal_11 = 0;
	float fLocal_12 = 0f;
	var uLocal_13 = 0;
	var uLocal_14 = 0;
	int iLocal_15 = 0;
	var uLocal_16 = 0;
	var uLocal_17 = 0;
	char* sLocal_18 = 0;
	var uLocal_19 = 0;
	var uLocal_20 = 0;
	float fLocal_21 = 0f;
	float fLocal_22 = 0f;
	float fLocal_23 = 0f;
	var uLocal_24 = 0;
	var uLocal_25 = 0;
	float fLocal_26 = 0f;
	var uLocal_27 = 0;
	var uLocal_28 = 0;
	var uLocal_29 = 0;
	float fLocal_30 = 0f;
	float fLocal_31 = 0f;
	var uLocal_32 = 0;
	var uLocal_33 = 0;
	int iLocal_34 = 0;
	var uLocal_35 = 0;
	var uLocal_36 = 0;
	var uLocal_37 = 0;
	int iLocal_38 = 0;
	int iLocal_39 = 0;
	int iLocal_40 = 0;
	int iLocal_41 = 0;
	var uLocal_42 = 0;
	var uLocal_43 = 0;
	var uLocal_44 = 0;
	var uLocal_45 = 0;
	var uLocal_46 = 0;
	var uLocal_47 = 0;
	var uLocal_48 = 0;
	var uLocal_49 = 0;
	var uLocal_50 = 0;
	var uLocal_51 = 0;
	var uLocal_52 = 0;
	var uLocal_53 = 0;
	var uLocal_54 = 0;
	var uLocal_55 = 0;
	var uLocal_56 = 0;
	var uLocal_57 = 0;
	var uLocal_58 = 0;
	var uLocal_59 = 0;
	var uLocal_60 = 0;
	float fLocal_61 = 0f;
	var uLocal_62 = 0;
	float fLocal_63 = 0f;
	var uLocal_64 = 0;
	var uLocal_65 = 0;
	var uLocal_66 = 0;
	var uLocal_67 = -1;
	var uLocal_68 = 0;
	var uLocal_69 = 0;
	var uLocal_70 = 0;
	var uLocal_71 = 0;
	var uLocal_72 = 0;
	var uLocal_73 = 0;
	var uLocal_74 = 0;
	var uLocal_75 = 0;
	var uLocal_76 = -2;
	var uLocal_77 = -2;
	var uLocal_78 = -2;
	var uLocal_79 = 5;
	var uLocal_80 = 0;
	var uLocal_81 = 0;
	var uLocal_82 = 0;
	var uLocal_83 = 0;
	var uLocal_84 = 0;
	var uLocal_85 = 2;
	var uLocal_86 = 0;
	var uLocal_87 = 0;
	var uLocal_88 = 2;
	var uLocal_89 = 0;
	var uLocal_90 = 0;
	float fLocal_91 = 0f;
	float fLocal_92 = 0f;
	float fLocal_93 = 0f;
	float fLocal_94 = 0f;
	var uLocal_95 = 0;
	var uLocal_96 = 0;
	var uLocal_97 = 0;
	var uLocal_98 = 0;
	var uLocal_99 = 0;
	var uLocal_100 = 0;
	var uLocal_101 = 0;
	var uLocal_102 = 0;
	var uLocal_103 = 0;
	var uLocal_104 = 0;
	var uLocal_105 = 0;
	var uLocal_106 = 0;
	var uLocal_107 = 0;
	var uLocal_108 = 0;
	var uLocal_109 = 0;
	var uLocal_110 = 0;
	var uLocal_111 = 0;
	var uLocal_112 = 0;
	var uLocal_113 = 0;
	var uLocal_114 = 0;
	var uLocal_115 = 0;
	var uLocal_116 = 0;
	var uLocal_117 = 0;
	var uLocal_118 = 0;
	var uLocal_119 = 0;
	var uLocal_120 = 0;
	var uLocal_121 = 0;
	var uLocal_122 = 0;
	var uLocal_123 = 0;
	var uLocal_124 = 0;
	var uLocal_125 = 0;
	var uLocal_126 = 0;
	var uLocal_127 = 0;
	var uLocal_128 = 0;
	var uLocal_129 = 0;
	var uLocal_130 = 0;
	var uLocal_131 = 0;
	var uLocal_132 = 0;
	var uLocal_133 = 0;
	var uLocal_134 = 0;
	int iLocal_135 = 0;
	var uLocal_136 = 0;
	float fLocal_137 = 0f;
	var uLocal_138 = 0;
	var uLocal_139 = 0;
	var uLocal_140 = 0;
	var uLocal_141 = 0;
	var uLocal_142 = 0;
	var uLocal_143 = 0;
	var uLocal_144 = 0;
	var uLocal_145 = 0;
	var uLocal_146 = 0;
	var uLocal_147 = 0;
	var uLocal_148 = 0;
	var uLocal_149 = 0;
	var uLocal_150 = 0;
	var uLocal_151 = 0;
	var uLocal_152 = 0;
	var uLocal_153 = 0;
	var uLocal_154 = 0;
	var uLocal_155 = 0;
	var uLocal_156 = 0;
	var uLocal_157 = 0;
	var uLocal_158 = 0;
	float fLocal_159 = 0f;
	float fLocal_160 = 0f;
	var uLocal_161 = 0;
	char* sLocal_162 = 0;
	int iLocal_163 = 0;
	float fLocal_164 = 0f;
	var uLocal_165 = 4;
	var uLocal_166 = 0;
	var uLocal_167 = 0;
	var uLocal_168 = 0;
	var uLocal_169 = 0;
	var uLocal_170 = 0;
	var uLocal_171 = 0;
	var uLocal_172 = 0;
	var uLocal_173 = 0;
	var uLocal_174 = 2;
	var uLocal_175 = 4;
	var uLocal_176 = 0;
	var uLocal_177 = 0;
	var uLocal_178 = 0;
	var uLocal_179 = 0;
	var uLocal_180 = 0;
	var uLocal_181 = 0;
	var uLocal_182 = 0;
	var uLocal_183 = 0;
	var uLocal_184 = 4;
	var uLocal_185 = 0;
	var uLocal_186 = 0;
	var uLocal_187 = 0;
	var uLocal_188 = 0;
	var uLocal_189 = 0;
	var uLocal_190 = 0;
	var uLocal_191 = 0;
	var uLocal_192 = 0;
	var uLocal_193 = 4;
	var uLocal_194 = 4;
	var uLocal_195 = 0;
	var uLocal_196 = 0;
	var uLocal_197 = 0;
	var uLocal_198 = 0;
	var uLocal_199 = 0;
	var uLocal_200 = 0;
	var uLocal_201 = 0;
	var uLocal_202 = 0;
	var uLocal_203 = 4;
	var uLocal_204 = 0;
	var uLocal_205 = 0;
	var uLocal_206 = 0;
	var uLocal_207 = 0;
	var uLocal_208 = 0;
	var uLocal_209 = 0;
	var uLocal_210 = 0;
	var uLocal_211 = 0;
	var uLocal_212 = 4;
	var uLocal_213 = 0;
	var uLocal_214 = 0;
	var uLocal_215 = 0;
	var uLocal_216 = 0;
	var uLocal_217 = 0;
	var uLocal_218 = 0;
	var uLocal_219 = 0;
	var uLocal_220 = 0;
	var uLocal_221 = 4;
	var uLocal_222 = 0;
	var uLocal_223 = 0;
	var uLocal_224 = 0;
	var uLocal_225 = 0;
	var uLocal_226 = 0;
	var uLocal_227 = 0;
	var uLocal_228 = 0;
	var uLocal_229 = 0;
	var uLocal_230 = 4;
	var uLocal_231 = 4;
	var uLocal_232 = 0;
	var uLocal_233 = 0;
	var uLocal_234 = 0;
	var uLocal_235 = 0;
	var uLocal_236 = 0;
	var uLocal_237 = 0;
	var uLocal_238 = 0;
	var uLocal_239 = 0;
	var uLocal_240 = 4;
	var uLocal_241 = 0;
	var uLocal_242 = 0;
	var uLocal_243 = 0;
	var uLocal_244 = 0;
	var uLocal_245 = 0;
	var uLocal_246 = 0;
	var uLocal_247 = 0;
	var uLocal_248 = 0;
	var uLocal_249 = 4;
	var uLocal_250 = 0;
	var uLocal_251 = 0;
	var uLocal_252 = 0;
	var uLocal_253 = 0;
	var uLocal_254 = 0;
	var uLocal_255 = 0;
	var uLocal_256 = 0;
	var uLocal_257 = 0;
	var uLocal_258 = 4;
	var uLocal_259 = 0;
	var uLocal_260 = 0;
	var uLocal_261 = 0;
	var uLocal_262 = 0;
	var uLocal_263 = 0;
	var uLocal_264 = 0;
	var uLocal_265 = 0;
	var uLocal_266 = 0;
	int iLocal_267 = 0;
	int iLocal_268 = 0;
	var uLocal_269 = 0;
	var uLocal_270 = 0;
	var uLocal_271 = 0;
	var uLocal_272 = 0;
	var uLocal_273 = 4;
	var uLocal_274 = 0;
	var uLocal_275 = 0;
	var uLocal_276 = 0;
	var uLocal_277 = 0;
	var uLocal_278 = 0;
	var uLocal_279 = 4;
	var uLocal_280 = 0;
	var uLocal_281 = 0;
	var uLocal_282 = 0;
	var uLocal_283 = 0;
	var uLocal_284 = 4;
	var uLocal_285 = 0;
	var uLocal_286 = 0;
	var uLocal_287 = 0;
	var uLocal_288 = 0;
	var uLocal_289 = 0;
	var uLocal_290 = 0;
	var uLocal_291 = 350;
	var uLocal_292 = 0;
	var uLocal_293 = 0;
	var uLocal_294 = 0;
	var uLocal_295 = 0;
	var uLocal_296 = 0;
	var uLocal_297 = 0;
	int iLocal_298 = 0;
	var uLocal_299 = 0;
	var uLocal_300 = 0;
	float fLocal_301 = 0f;
	var uLocal_302 = 0;
	var uLocal_303 = 0;
	var uLocal_304 = 0;
	var uLocal_305 = 0;
	var uLocal_306 = 255;
	var uLocal_307 = 0;
	var uLocal_308 = 0;
	var uLocal_309 = 0;
	var uLocal_310 = 255;
	var uLocal_311 = 0;
	var uLocal_312 = 0;
	var uLocal_313 = 0;
	var uLocal_314 = 255;
	var uLocal_315 = 0;
	var uLocal_316 = 0;
	var uLocal_317 = 0;
	var uLocal_318 = 255;
	var uLocal_319 = 4;
	var uLocal_320 = 0;
	var uLocal_321 = 0;
	var uLocal_322 = 0;
	var uLocal_323 = 255;
	var uLocal_324 = 0;
	var uLocal_325 = 0;
	var uLocal_326 = 0;
	var uLocal_327 = 255;
	var uLocal_328 = 0;
	var uLocal_329 = 0;
	var uLocal_330 = 0;
	var uLocal_331 = 255;
	var uLocal_332 = 0;
	var uLocal_333 = 0;
	var uLocal_334 = 0;
	var uLocal_335 = 255;
	var uLocal_336 = 32;
	var uLocal_337 = 0;
	var uLocal_338 = 0;
	var uLocal_339 = 0;
	var uLocal_340 = 0;
	var uLocal_341 = 0;
	var uLocal_342 = 0;
	var uLocal_343 = 0;
	var uLocal_344 = 0;
	var uLocal_345 = 0;
	var uLocal_346 = 0;
	var uLocal_347 = 0;
	var uLocal_348 = 0;
	var uLocal_349 = 0;
	var uLocal_350 = 0;
	var uLocal_351 = 0;
	var uLocal_352 = 0;
	var uLocal_353 = 0;
	var uLocal_354 = 0;
	var uLocal_355 = 0;
	var uLocal_356 = 0;
	var uLocal_357 = 0;
	var uLocal_358 = 0;
	var uLocal_359 = 0;
	var uLocal_360 = 0;
	var uLocal_361 = 0;
	var uLocal_362 = 0;
	var uLocal_363 = 0;
	var uLocal_364 = 0;
	var uLocal_365 = 0;
	var uLocal_366 = 0;
	var uLocal_367 = 0;
	var uLocal_368 = 0;
	var uLocal_369 = 0;
	var uLocal_370 = -1;
	var uLocal_371 = 0;
	var uLocal_372 = 0;
	var uLocal_373 = 0;
	var uLocal_374 = 0;
	var uLocal_375 = 0;
	var uLocal_376 = 0;
	var uLocal_377 = 0;
	var uLocal_378 = 0;
	var uLocal_379 = 0;
	var uLocal_380 = 0;
	var uLocal_381 = 0;
	var uLocal_382 = 0;
	var uLocal_383 = 0;
	var uLocal_384 = 0;
	var uLocal_385 = 4;
	var uLocal_386 = 0;
	var uLocal_387 = 0;
	var uLocal_388 = 0;
	var uLocal_389 = 0;
	var uLocal_390 = 0;
	var uLocal_391 = 0;
	var uLocal_392 = 0;
	var uLocal_393 = 0;
	var uScriptParam_0 = 0;
	var uScriptParam_1 = -1;
	var uScriptParam_2 = -1;
	var uScriptParam_3 = 0;
	var uScriptParam_4 = 0;
	var uScriptParam_5 = 0;
	var uScriptParam_6 = 0;
	var uScriptParam_7 = 0;
	var uScriptParam_8 = 0;
	var uScriptParam_9 = -1;
	var uScriptParam_10 = 0;
	var uScriptParam_11 = 0;
	var uScriptParam_12 = 0;
	var uScriptParam_13 = 0;
	var uScriptParam_14 = 0;
	var uScriptParam_15 = 0;
	var uScriptParam_16 = -1;
	var uScriptParam_17 = 0;
	var uScriptParam_18 = 0;
	var uScriptParam_19 = -1;
	var uScriptParam_20 = -1;
#endregion

void main() // Position - 0x0
{
	iLocal_2 = 1;
	iLocal_3 = 134;
	iLocal_4 = 134;
	iLocal_5 = 1;
	iLocal_6 = 1;
	iLocal_7 = 1;
	iLocal_8 = 134;
	iLocal_9 = 1;
	iLocal_10 = 12;
	iLocal_11 = 12;
	fLocal_12 = 0.001f;
	iLocal_15 = -1;
	sLocal_18 = "NULL";
	fLocal_21 = 80f;
	fLocal_22 = 140f;
	fLocal_23 = 180f;
	fLocal_26 = 0f;
	fLocal_30 = -0.0375f;
	fLocal_31 = 0.17f;
	iLocal_34 = 3;
	iLocal_38 = 1;
	iLocal_39 = 65;
	iLocal_40 = 49;
	iLocal_41 = 64;
	fLocal_61 = (0.05f + 0.275f) - 0.01f;
	fLocal_63 = 0f;
	fLocal_91 = 3f;
	fLocal_92 = 0f;
	fLocal_93 = 2f;
	fLocal_94 = 100f;
	iLocal_135 = -1;
	fLocal_137 = 0.5f;
	sLocal_162 = "DLC_XM17_IAA_SF_Hack";
	iLocal_163 = -1;
	fLocal_164 = 0f;

	if (unk_0x76CD105BCAC6EB9F())
		if (!func_152(uScriptParam_0))
			func_148();
	else
		func_148();

	while (true)
	{
		func_147();
	
		if (_SHOULD_NETWORK_SCRIPT_TERMINATE())
			func_148();
	
		if (func_137())
			func_148();
	
		func_2();
	
		if (unk_0x93E08E0F531E2C35())
			func_1();
	}

	return;
}

void func_1() // Position - 0xFB
{
	return;
}

void func_2() // Position - 0x103
{
	func_3();
	return;
}

int func_3() // Position - 0x110
{
	int num;

	if (func_136() == 0)
	{
		func_134();
	
		if (unk_0xAE231F549813BBDF(2))
			unk_0x66EFB3D6110055C4(2, 199, 1);
	
		func_11(&uLocal_385[num /*2*/]);
		unk_0x0B0C9A0F9AAEB7F0(&Global_1957673, 1);
	
		if (!func_10(&uLocal_370))
			func_7(&uLocal_370, 3, 2, -1);
	
		if (IS_BIT_SET(uLocal_385[num /*2*/], 0))
		{
			func_6(1);
			func_7(&uLocal_370, 0, 3, -1);
		}
		else if (IS_BIT_SET(uLocal_385[num /*2*/], 1))
		{
			func_6(2);
			func_7(&uLocal_370, 0, 0, -1);
		}
		else if (IS_BIT_SET(uLocal_385[num /*2*/], 2))
		{
			func_6(3);
			func_5(&uLocal_370, false);
		}
	}

	if (func_4())
		if (IS_BIT_SET(uLocal_385[num /*2*/], 0) || func_136() == 1)
			return 1;

	return 0;
}

BOOL func_4() // Position - 0x1D6
{
	return IS_BIT_SET(Global_1957673, 1);
}

void func_5(var uParam0, BOOL bParam1) // Position - 0x1E4
{
	uParam0->f_5 = { uParam0->f_11 };
	uParam0->f_8 = { uParam0->f_11 };
	uParam0->f_3 = 0f;
	uParam0->f_1 = 0f;
	uParam0->f_2 = 0;
	uParam0->f_4 = 0;
	uParam0->f_14 = 0;
	*uParam0 = -1;

	if (bParam1)
		unk_0x9882AC0599785A65(0);

	return;
}

void func_6(int iParam0) // Position - 0x22B
{
	if (Global_1957673.f_1 != iParam0)
		Global_1957673.f_1 = iParam0;

	return;
}

void func_7(var uParam0, int iParam1, int iParam2, int iParam3) // Position - 0x246
{
	if (iParam1 == -1)
		return;

	uParam0->f_5 = { func_9(iParam2) };
	uParam0->f_8 = { func_9(iParam2) };
	uParam0->f_1 = 0f;
	uParam0->f_3 = 0f;
	uParam0->f_4 = iParam3;
	*uParam0 = iParam1;
	func_8(&(uParam0->f_2));
	return;
}

void func_8(var uParam0) // Position - 0x28C
{
	if (*uParam0 == 0)
		*uParam0 = unk_0x0728E77B2BF91D54();

	return;
}

Vector3 func_9(int iParam0) // Position - 0x2A2
{
	switch (iParam0)
	{
		case 0:
			return 255f, 0f, 0f;
	
		case 1:
			return 0f, 0f, 255f;
	
		case 2:
			return 100f, 255f, 255f;
	
		case 3:
			return 0f, 255f, 0f;
	
		case 4:
			return 255f, 255f, 255f;
	
		case 5:
			return 255f, 50f, 0f;
	
		case 6:
			return 255f, 165f, 0f;
	
		case 7:
			return 255f, 45f, 60f;
	
		case 8:
			return 255f, 0f, 255f;
	
		case 9:
			return 190f, 50f, 90f;
	
		case 10:
			return 0f, 255f, 200f;
	
		case 11:
			return 0f, 0f, 0f;
	
		default:
		
	}

	return 0f, 0f, 0f;
}

BOOL func_10(var uParam0) // Position - 0x3A0
{
	return *uParam0 != -1;
}

void func_11(var uParam0) // Position - 0x3AD
{
	int value;
	int value2;

	unk_0x21564F65F997D833(&value, &value2);

	if (iLocal_298 != 4)
		func_113();

	if (iLocal_298 != 0)
		func_111();

	if (unk_0xA0C7B98BCF1EEF9E(uLocal_161))
		unk_0xA91A4C18A2DB01BD(uLocal_161, 100, 100, 100, 255, 0);

	unk_0x3F002AA9562BF0BE(iLocal_163, "Damage", fLocal_164);

	switch (iLocal_298)
	{
		case 0:
			func_110();
			func_109();
			func_108();
			iLocal_267 = 0;
		
			if (!func_107(215, -1))
			{
				iLocal_268 = 1;
				uLocal_269.f_15[0] = 2;
				uLocal_269.f_15[1] = 0;
				uLocal_269.f_15[2] = 3;
				uLocal_269.f_15[3] = 1;
			}
			else
			{
				func_106();
			}
		
			unk_0x8744D2E3FC95740E(&(uLocal_269.f_20), 0);
		
			if (unk_0x76CD105BCAC6EB9F())
			{
				_SET_PLAYER_CONTROL_EX(unk_0x259BE71D8A81D4FA(), false, 0, 0);
				func_94();
			}
			else
			{
				unk_0x4686BC3BFDBB5348(unk_0x259BE71D8A81D4FA(), 0, 0);
			}
		
			if (SYSTEM::TO_FLOAT(value) / SYSTEM::TO_FLOAT(value2) > 16f / 9f)
				func_93(1, 1, true, 0);
		
			Global_4718592.f_180957 = 1;
			unk_0x0B0C9A0F9AAEB7F0(&(Global_1845221[unk_0x259BE71D8A81D4FA() /*889*/].f_885), 16);
			uLocal_161 = unk_0x8DE4F68A9728925E("HACKING_MESSAGE");
		
			if (unk_0xA0C7B98BCF1EEF9E(uLocal_161))
				func_92(1);
		
			if (iLocal_163 == -1)
				iLocal_163 = unk_0xCA369FBC0DE29517();
		
			fLocal_164 = 0f;
			unk_0x8744D2E3FC95740E(uParam0, 2);
			break;
	
		case 1:
			func_82();
			func_77();
			func_63();
		
			if (func_62())
				func_61();
		
			if (func_60())
			{
				func_59();
				func_56(uParam0, &uLocal_269, 240000);
				func_92(3);
			}
			else if (func_50())
			{
				func_56(uParam0, &uLocal_269, 240000);
				func_92(2);
			}
		
			if (unk_0x9F0C06CFBACDD6A1(iLocal_163))
				unk_0xBF3D28CA44F3BE2D(iLocal_163, "Background_Loop", sLocal_162, 1);
		
			unk_0x66EFB3D6110055C4(2, 199, 1);
			unk_0x66EFB3D6110055C4(2, 200, 1);
			unk_0x66EFB3D6110055C4(0, 199, 1);
			unk_0x66EFB3D6110055C4(0, 200, 1);
			unk_0x4CC1CF98851922CE(0, 202);
			unk_0x4CC1CF98851922CE(2, 202);
		
			if (unk_0x2645430E708CBFAC(2, 202))
			{
				func_56(uParam0, &uLocal_269, 240000);
				unk_0x0B0C9A0F9AAEB7F0(uParam0, 2);
			}
		
			if (!unk_0xBC2EE32DE886BD08("DLC_XM17_IAA_Hack_Minigame_Scene"))
				unk_0xCAC4020CCF361AC8("DLC_XM17_IAA_Hack_Minigame_Scene");
		
			if (func_48(0, -1, false))
				func_37(value, -1, 0, true, false, false, true, 1, false);
			break;
	
		case 2:
			if (iLocal_267 == 3)
			{
				if (!_STOPWATCH_IS_INITIALIZED(&uLocal_299))
				{
					_STOPWATCH_RESET(&uLocal_299, false, false);
				}
				else if (_STOPWATCH_HAS_TIME_PASSED(&uLocal_299, 3000, false))
				{
					unk_0x0B0C9A0F9AAEB7F0(uParam0, 0);
					func_29(215, 1, -1, 1);
					func_92(4);
				}
			}
		
			func_27();
			break;
	
		case 3:
			if (!_STOPWATCH_IS_INITIALIZED(&uLocal_299))
			{
				_STOPWATCH_RESET(&uLocal_299, false, false);
			}
			else if (_STOPWATCH_HAS_TIME_PASSED(&uLocal_299, 3000, false))
			{
				unk_0x0B0C9A0F9AAEB7F0(uParam0, 1);
				func_92(4);
			}
			else
			{
				unk_0xFFA2B456A81EA1EB("MPHotwire", "failed", 0.5f, 0.5f, 0.344f / fLocal_159, 0.127f, 0f, 255, 255, 255, SYSTEM::ROUND(uLocal_269.f_26), 0, 0);
			
				if (uLocal_269.f_26 < 255f)
					uLocal_269.f_26 = uLocal_269.f_26 + (255f * SYSTEM::TIMESTEP());
				else
					func_25("BEAM_F" /*FAILURE*/, 223, 55, 66, 255, 0.612f, 0.612f, 0.5f, 0.455f);
			}
			break;
	
		case 4:
			func_12(uParam0, false);
			break;
	}

	return;
}

void func_12(var uParam0, BOOL bParam1) // Position - 0x6E5
{
	func_23();
	Global_4718592.f_180957 = 0;
	func_22();
	unk_0x8744D2E3FC95740E(&(Global_1845221[unk_0x259BE71D8A81D4FA() /*889*/].f_885), 16);
	func_93(0, 1, true, 0);
	func_21();

	if (!bParam1)
		if (func_19() && !unk_0x787F8EE1F6FBDC6D())
			if (unk_0x76CD105BCAC6EB9F())
				_SET_PLAYER_CONTROL_EX(unk_0x259BE71D8A81D4FA(), true, 0, 0);
			else
				unk_0x4686BC3BFDBB5348(unk_0x259BE71D8A81D4FA(), 1, 0);

	_STOPWATCH_DESTROY(&uLocal_299);
	unk_0xD314260005F064BF(&uLocal_161);

	if (unk_0xBC2EE32DE886BD08("DLC_XM17_IAA_Hack_Minigame_Scene"))
		unk_0xB43467C43086A6A1("DLC_XM17_IAA_Hack_Minigame_Scene");

	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0x4A8C381C258A124D()))
	{
		unk_0x0428AFDCAA63B06E(unk_0x4A8C381C258A124D(), 242, 1);
		unk_0x0428AFDCAA63B06E(unk_0x4A8C381C258A124D(), 243, 1);
	}

	fLocal_164 = 0f;
	func_16(&iLocal_163);
	func_13(true, -1);
	func_92(0);
	*uParam0 = 0;
	return;
}

void func_13(BOOL bParam0, int iParam1) // Position - 0x7AE
{
	int num;

	if (!func_15(&num, false, iParam1))
		return;

	if (Global_23970.f_9116)
	{
		unk_0xDCFF86AAD108A201(15);
		Global_23970.f_9116 = 0;
	}

	unk_0x043244A32AD6E17D(0f);

	if (Global_23970.f_6231[num])
	{
		unk_0xA790E8E6FD7393AC(9, 0);
		Global_23970.f_6231[num] = false;
	}

	if (Global_23970.f_6217[num])
	{
		unk_0x633E3833FB96BCCB("CommonMenu");
		Global_23970.f_6217[num] = false;
	}

	if (Global_23970.f_6224[num])
	{
		unk_0x633E3833FB96BCCB("MPShopSale");
		Global_23970.f_6224[num] = false;
	}

	if (bParam0)
	{
		func_14(&Global_23970.f_6263[num /*10*/]);
		Global_23970.f_6324[num] = 0;
	}
	else
	{
		Global_23970.f_6324[num] = 0;
	}

	unk_0x633E3833FB96BCCB("Shared");
	return;
}

void func_14(var uParam0) // Position - 0x871
{
	if (uParam0->f_9 != 0)
	{
		if (unk_0xA0C7B98BCF1EEF9E(*uParam0))
			unk_0xD314260005F064BF(uParam0);
	
		*uParam0 = 0;
		uParam0->f_9 = 0;
	}

	return;
}

BOOL func_15(var uParam0, BOOL bParam1, int iParam2) // Position - 0x89D
{
	var unk;
	int num;
	int i;
	int num2;

	if (iParam2 == -1)
		if (unk_0x76CD105BCAC6EB9F() && unk_0xE8DB952A4BA8F328())
			iParam2 = unk_0xF25E5B7C1279A85B();

	TEXT_LABEL_ASSIGN_STRING(&unk, unk_0x1AF90EB93E0012D6(), 64);
	TEXT_LABEL_APPEND_INT(&unk, iParam2, 64);
	num = unk_0x70E57E9927B6BA58(&unk);
	num2 = -1;

	for (i = 0; i < 6; i = i + 1)
	{
		if (Global_23970.f_6324[i] == num)
		{
			*uParam0 = i;
			return true;
		}
		else if (Global_23970.f_6324[i] == 0)
		{
			num2 = i;
		}
	}

	if (bParam1)
	{
		if (num2 != -1)
		{
			Global_23970.f_6324[num2] = num;
			*uParam0 = num2;
			return true;
		}
	}

	return false;
}

void func_16(var uParam0) // Position - 0x93A
{
	if (*uParam0 != -1)
	{
		unk_0x8E4825CCACA34B58(*uParam0);
		unk_0x394AFAC073E1F277(*uParam0);
		*uParam0 = -1;
	}

	return;
}

BOOL _DOES_ENTITY_EXIST_AND_IS_ALIVE(var uParam0) // Position - 0x95B
{
	if (unk_0xFC8BFE4B41177C22(uParam0))
		if (!unk_0x1C2F771CDC87A3A5(uParam0, 0))
			return true;

	return false;
}

void _STOPWATCH_DESTROY(var uParam0) // Position - 0x97C
{
	uParam0->f_1 = 0;
	return;
}

BOOL func_19() // Position - 0x989
{
	if (func_20() == 0)
		return true;

	return false;
}

int func_20() // Position - 0x99E
{
	return Global_1574633.f_18;
}

void func_21() // Position - 0x9AC
{
	if (Global_2672939.f_1023.f_10)
		Global_2672939.f_1023.f_10 = 0;

	return;
}

void func_22() // Position - 0x9CA
{
	Global_1929312;
	Global_1929312 = false;
	return;
}

void func_23() // Position - 0x9DE
{
	if (!Global_1574755)
		return;

	func_24();
	return;
}

void func_24() // Position - 0x9F5
{
	Global_1574755 = false;
	TEXT_LABEL_ASSIGN_STRING(&(Global_1574755.f_1), "", 32);
	Global_1574755.f_9 = 0;
	return;
}

void func_25(char* sParam0, int iParam1, int iParam2, int iParam3, int iParam4, float fParam5, float fParam6, float fParam7, float fParam8) // Position - 0xA14
{
	float value;

	unk_0xBFE94E91C83D8794(fParam5, fParam6);
	value = SYSTEM::TO_FLOAT(unk_0x1DD05E817C89C737());
	value = unk_0x1D5CD3EAAA7422B0(SYSTEM::SIN(value * 0.35f));
	iParam4 = 255 - (SYSTEM::ROUND(value) * 100);
	unk_0x5A18938160AE52D0(iParam1, iParam2, iParam3, iParam4);
	unk_0xEAF65721ACB2FDFB(1);
	unk_0xEAEB6E7D3FAEBD5B("STRING");
	unk_0xACF853FB3F6EA7D4(sParam0);
	unk_0x25DD447A6EB3A86F(func_26(fParam7), fParam8, 0);
	return;
}

float func_26(float fParam0) // Position - 0xA78
{
	fParam0 = 0.5f - ((0.5f - fParam0) / fLocal_159);
	return fParam0;
}

void func_27() // Position - 0xA95
{
	float num;
	float num2;
	float num3;
	float num4;
	float num5;
	float num6;

	num = 0.25f;
	num2 = 0.511f;
	num3 = 0.454f * 50f;
	unk_0xFFA2B456A81EA1EB("MPHotwire", "passed", 0.5f, 0.5f, 0.344f / fLocal_159, 0.127f, 0f, 255, 255, 255, SYSTEM::ROUND(uLocal_269.f_26), 0, 0);
	num = 0.03f;

	if (iLocal_267 != 0)
	{
		num = 0.011f;
		num4 = 0.375f;
		num5 = func_28(num4, 0.5f, uLocal_269.f_24);
		unk_0xFFA2B456A81EA1EB("MPHotwire", "load_bar", func_26(num5), num2, (uLocal_269.f_24 * num * num3) / fLocal_159, num, 90f, 255, 255, 255, 255, 0, 0);
	}

	switch (iLocal_267)
	{
		case 0:
			uLocal_269.f_26 = uLocal_269.f_26 + (200f * SYSTEM::TIMESTEP());
		
			if (uLocal_269.f_26 >= 255f)
			{
				uLocal_269.f_26 = 255f;
				iLocal_267 = 1;
			}
			break;
	
		case 1:
			if (uLocal_269.f_24 < 1f)
			{
				uLocal_269.f_24 = uLocal_269.f_24 + (0.5f * SYSTEM::TIMESTEP());
			
				if (uLocal_269.f_24 >= 1f)
					uLocal_269.f_24 = 1f;
			}
			else
			{
				uLocal_269.f_24 = 1f;
				iLocal_267 = 3;
				unk_0xBF3D28CA44F3BE2D(-1, "Hack_Success", sLocal_162, 1);
				fLocal_164 = 0f;
			
				if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0x4A8C381C258A124D()))
					unk_0xBD618A73193F9982(-1, "Hack_Success_Remote", unk_0x4A8C381C258A124D(), sLocal_162, 1, 30);
			}
			break;
	
		case 3:
			num6 = 0.612f;
			func_25("BEAM_P" /*SUCCESS*/, 76, 190, 148, 255, num6, num6, 0.5f, 0.456f);
			break;
	}

	return;
}

float func_28(float fParam0, float fParam1, float fParam2) // Position - 0xC22
{
	return ((1f - fParam2) * fParam0) + (fParam2 * fParam1);
}

void func_29(int iParam0, int iParam1, int iParam2, int iParam3) // Position - 0xC37
{
	int num;

	num = func_30(iParam0, iParam2);

	if (num != 0)
		unk_0xF1D0B0CE940F620D(num, iParam1, iParam3);

	return;
}

int func_30(int iParam0, int iParam1) // Position - 0xC5A
{
	return unk_0xD69CE161FE614531(2, iParam0, func_31(iParam1));
}

int func_31(int iParam0) // Position - 0xC6F
{
	int num;
	int num2;

	num = iParam0;

	if (num == -1)
	{
		num2 = func_32();
	
		if (num2 > -1)
		{
			Global_2751934 = 0;
			num = num2;
		}
		else
		{
			num = 0;
			Global_2751934 = 1;
		}
	}

	return num;
}

int func_32() // Position - 0xCA3
{
	return Global_1574926;
}

BOOL _STOPWATCH_HAS_TIME_PASSED(var pStopwatch, int millis, BOOL useLocalTimer) // Position - 0xCAF
{
	if (millis == -1)
		return true;

	_STOPWATCH_INITIALIZE(pStopwatch, useLocalTimer, false);

	if (unk_0x76CD105BCAC6EB9F() && !useLocalTimer)
		if (unk_0x510D0699BE9C6D06(unk_0x775142054EC39277(unk_0x7E3F74F641EE6B27(), *pStopwatch)) >= millis)
			return true;
	else if (unk_0x510D0699BE9C6D06(unk_0x775142054EC39277(unk_0x1DD05E817C89C737(), *pStopwatch)) >= millis)
		return true;

	return false;
}

void _STOPWATCH_INITIALIZE(var pStopwatch, BOOL useLocalTimer, BOOL useAccurateTime) // Position - 0xD0D
{
	if (pStopwatch->f_1 == 0)
	{
		if (unk_0x76CD105BCAC6EB9F() && !useLocalTimer)
			if (!useAccurateTime)
				*pStopwatch = unk_0x7E3F74F641EE6B27();
			else
				*pStopwatch = unk_0x0728E77B2BF91D54();
		else
			*pStopwatch = unk_0x1DD05E817C89C737();
	
		pStopwatch->f_1 = 1;
	}

	return;
}

void _STOPWATCH_RESET(var pStopwatch, BOOL useLocalTimer, BOOL useAccurateTime) // Position - 0xD52
{
	if (unk_0x76CD105BCAC6EB9F() && !useLocalTimer)
		if (!useAccurateTime)
			*pStopwatch = unk_0x7E3F74F641EE6B27();
		else
			*pStopwatch = unk_0x0728E77B2BF91D54();
	else
		*pStopwatch = unk_0x1DD05E817C89C737();

	pStopwatch->f_1 = 1;
	return;
}

BOOL _STOPWATCH_IS_INITIALIZED(var uParam0) // Position - 0xD8F
{
	return uParam0->f_1;
}

void func_37(int iParam0, int iParam1, int iParam2, BOOL bParam3, BOOL bParam4, BOOL bParam5, BOOL bParam6, int iParam7, BOOL bParam8) // Position - 0xD9B
{
	int num;
	int i;
	int j;
	int num2;
	int num3;

	if (!func_15(&num, false, iParam1))
		return;

	iParam0 = iParam0;

	if (bParam3 && !func_45(bParam4, bParam8))
		return;

	if (func_43())
		return;

	if (unk_0xA43CD45F18522E3F())
		return;

	if (iParam7 == 0)
		if (func_40(unk_0x259BE71D8A81D4FA(), 0))
			return;

	if (unk_0x761778199FE1211C())
		if (unk_0x66DA7155B68E7638() == 0 || unk_0xA43CD45F18522E3F())
			return;

	if (Global_23970.f_5326 != 0)
	{
		if (unk_0x7811C74D5B749F76(2))
		{
			for (i = 0; i < Global_23970.f_5326; i = i + 1)
			{
				if (Global_23970.f_5625[i] != 365)
					TEXT_LABEL_ASSIGN_STRING(&Global_23970.f_5328[i /*16*/], unk_0xE934399D6F2C3AC5(2, Global_23970.f_5625[i], 1), 64);
				else if (Global_23970.f_5640[i] != 32)
					TEXT_LABEL_ASSIGN_STRING(&Global_23970.f_5328[i /*16*/], unk_0xF761D79754BC3043(2, Global_23970.f_5640[i], 1), 64);
			}
		
			Global_23970.f_5327 = 0;
		}
	
		if (!Global_23970.f_5327)
		{
			unk_0x88F483FBD433696A(Global_23970.f_6263[num /*10*/], "CLEAR_ALL");
			unk_0xE6B753D52F4CA222();
			unk_0x88F483FBD433696A(Global_23970.f_6263[num /*10*/], "SET_MAX_WIDTH");
			unk_0x74BF156C860580D4(1f - (Global_23970.f_5684 / 100f));
			unk_0xE6B753D52F4CA222();
		
			if (unk_0x761778199FE1211C())
			{
				unk_0x88F483FBD433696A(Global_23970.f_6263[num /*10*/], "TOGGLE_MOUSE_BUTTONS");
				unk_0x557F1E2300EF1A3E(1);
				unk_0xE6B753D52F4CA222();
			}
		
			for (i = 0; i < Global_23970.f_5326; i = i + 1)
			{
				if (unk_0x70E57E9927B6BA58(&Global_23970.f_5553[i /*4*/]) != unk_0x70E57E9927B6BA58("PREV"))
				{
					unk_0x88F483FBD433696A(Global_23970.f_6263[num /*10*/], "SET_DATA_SLOT");
					unk_0x330108B080A2132F(i);
					func_39(&Global_23970.f_5328[i /*16*/]);
				
					for (j = i + 1; j < 14 && unk_0x70E57E9927B6BA58(&Global_23970.f_5553[j /*4*/]) == unk_0x70E57E9927B6BA58("PREV"); j = j + 1)
					{
						func_39(&Global_23970.f_5328[j /*16*/]);
					}
				
					if (Global_23970.f_5610[i] == -1)
					{
						func_38(&Global_23970.f_5553[i /*4*/]);
					}
					else
					{
						num2 = Global_23970.f_5610[i];
					
						if (iParam2 >= 0)
							num2 = iParam2;
					
						unk_0x882AEFD55B8D51FB(&Global_23970.f_5553[i /*4*/]);
					
						if (bParam5)
							unk_0xBD34A69071611974(num2, 70);
						else
							unk_0x511D14ED2DA887E1(num2);
					
						unk_0xCFAD3D478C87321A();
					}
				
					if (unk_0x761778199FE1211C())
					{
						if (Global_23970.f_5625[i] != 365 && IS_BIT_SET(Global_23970.f_5655, i))
						{
							unk_0x557F1E2300EF1A3E(1);
							unk_0x330108B080A2132F(Global_23970.f_5625[i]);
						}
						else
						{
							unk_0x557F1E2300EF1A3E(0);
							unk_0x330108B080A2132F(365);
						}
					}
				
					unk_0xE6B753D52F4CA222();
				}
			}
		
			if (unk_0x70E57E9927B6BA58(&(Global_4542374.f_16)) != unk_0x70E57E9927B6BA58(""))
			{
				unk_0x88F483FBD433696A(Global_23970.f_6263[num /*10*/], "SET_DATA_SLOT");
				unk_0x330108B080A2132F(Global_23970.f_5326);
				func_39(&Global_4542374);
			
				if (Global_4542374.f_20 == -1)
				{
					func_38(&(Global_4542374.f_16));
				}
				else
				{
					num3 = Global_23970.f_5610[i];
				
					if (iParam2 >= 0)
						num3 = iParam2;
				
					unk_0x882AEFD55B8D51FB(&(Global_4542374.f_16));
				
					if (bParam5)
						unk_0xBD34A69071611974(num3, 70);
					else
						unk_0x511D14ED2DA887E1(num3);
				
					unk_0xCFAD3D478C87321A();
				}
			
				unk_0xE6B753D52F4CA222();
			}
		
			unk_0x88F483FBD433696A(Global_23970.f_6263[num /*10*/], "SET_BACKGROUND_COLOUR");
			unk_0x330108B080A2132F(0);
			unk_0x330108B080A2132F(0);
			unk_0x330108B080A2132F(0);
			unk_0x330108B080A2132F(80);
			unk_0xE6B753D52F4CA222();
			unk_0x88F483FBD433696A(Global_23970.f_6263[num /*10*/], "DRAW_INSTRUCTIONAL_BUTTONS");
		
			if (Global_23970.f_5685)
				unk_0x330108B080A2132F(1);
			else
				unk_0x330108B080A2132F(0);
		
			unk_0xE6B753D52F4CA222();
			Global_23970.f_5327 = 1;
		}
	
		for (i = 0; i < Global_23970.f_5326; i = i + 1)
		{
			if (Global_23970.f_5610[i] != -1)
			{
				if (iParam2 > 0)
				{
					unk_0x88F483FBD433696A(Global_23970.f_6263[num /*10*/], "OVERRIDE_RESPAWN_TEXT");
					unk_0x330108B080A2132F(i);
					unk_0x882AEFD55B8D51FB(&Global_23970.f_5553[i /*4*/]);
				
					if (bParam5)
						unk_0xBD34A69071611974(iParam2, 70);
					else
						unk_0x511D14ED2DA887E1(iParam2);
				
					unk_0xCFAD3D478C87321A();
					unk_0xE6B753D52F4CA222();
				}
			}
		}
	
		if (Global_4542374.f_20 != -1)
		{
			if (iParam2 > 0)
			{
				unk_0x88F483FBD433696A(Global_23970.f_6263[num /*10*/], "OVERRIDE_RESPAWN_TEXT");
				unk_0x330108B080A2132F(i);
				unk_0x882AEFD55B8D51FB(&(Global_4542374.f_16));
			
				if (bParam5)
					unk_0xBD34A69071611974(iParam2, 70);
				else
					unk_0x511D14ED2DA887E1(iParam2);
			
				unk_0xCFAD3D478C87321A();
				unk_0xE6B753D52F4CA222();
			}
		}
	
		unk_0x9A122D542F2BB60E(76, 66);
		unk_0x9C066F8D86A1A438(0f, 0f, 0f, 0f);
	
		if (bParam6)
		{
			if (!Global_23970.f_9116)
			{
				unk_0x36A472841BBC9D4A(15, 0f, -0.0375f);
				Global_23970.f_9116 = 1;
			}
		}
		else if (Global_23970.f_9116)
		{
			unk_0xDCFF86AAD108A201(15);
			Global_23970.f_9116 = 0;
		}
	
		unk_0x90B531766063C5CD();
	
		if (Global_23970.f_5658)
		{
			unk_0x9A122D542F2BB60E(82, 66);
			unk_0x9C066F8D86A1A438(0f, 0f, 0f, 0f);
			unk_0x6D16B99FEB0AFFF1(Global_23970.f_6263[num /*10*/], Global_23970.f_5656, Global_23970.f_5657, 1f, 1f, 255, 255, 255, 255, 0);
			unk_0x90B531766063C5CD();
		}
		else
		{
			unk_0xA91A4C18A2DB01BD(Global_23970.f_6263[num /*10*/], 255, 255, 255, 255, 0);
		}
	}

	return;
}

void func_38(char* sParam0) // Position - 0x129C
{
	unk_0x882AEFD55B8D51FB(sParam0);
	unk_0xCFAD3D478C87321A();
	return;
}

void func_39(char* sParam0) // Position - 0x12AE
{
	unk_0xCE3E870AC37B4253(sParam0);
	return;
}

BOOL func_40(int iParam0, int iParam1) // Position - 0x12BC
{
	BOOL flag;

	if (!func_42(iParam0))
		return false;

	if (iParam0 == unk_0x259BE71D8A81D4FA())
		flag = func_41(-1, false) == 8;
	else
		flag = Global_1845221[iParam0 /*889*/].f_206 == 8;

	if (iParam1 == 1)
		if (unk_0x762604C40829DB72(iParam0))
			flag = unk_0x1864096A95E36EBA(iParam0) == 8;

	return flag;
}

int func_41(int iParam0, BOOL bParam1) // Position - 0x1315
{
	int num;
	int num2;

	num2 = iParam0;

	if (num2 == -1)
		num2 = func_32();

	if (Global_1575064[num2] == 1)
	{
		bParam1;
		num = 8;
	}
	else
	{
		num = Global_1574920[num2];
		bParam1;
	}

	return num;
}

BOOL func_42(int iParam0) // Position - 0x1356
{
	int num;

	num = iParam0;

	if (num < 0)
		return false;

	if (num >= 32)
		return false;

	return true;
}

BOOL func_43() // Position - 0x1378
{
	var unk;

	if (Global_21066.f_1 > 3)
		return true;

	if (func_44())
	{
		unk = { 0f, -500f, 0f };
		unk_0x78C4EBB0251847E2(&unk);
	
		if (Global_21011 == 0)
			if (unk.f_1 > -119f)
				return true;
			else
				return false;
		else if (unk.f_1 > -101f)
			return true;
		else
			return false;
	}

	return false;
}

BOOL func_44() // Position - 0x13E6
{
	if (unk_0x486FF5D06E9659F1(joaat("cellphone_flashhand")) > 0)
		return true;

	return false;
}

BOOL func_45(BOOL bParam0, BOOL bParam1) // Position - 0x1400
{
	if (Global_2672939.f_1761.f_701 != 0)
		return true;

	if (!unk_0x9390801B06EE998F() || func_47(8, -1) && func_46() != 65 || unk_0x05AA183DA1344935() != 0 && !bParam1 || unk_0x3555462DB47B7AB1() && !bParam0 || unk_0x2B8BAF9BA2A3D36B() || Global_79800 || Global_23970.f_9115 || unk_0xB11671B812399BA2() || Global_101740.f_1490)
		return false;

	return true;
}

int func_46() // Position - 0x149D
{
	return Global_1575012;
}

BOOL func_47(int iParam0, int iParam1) // Position - 0x14A9
{
	switch (iParam0)
	{
		case 5:
			if (iParam1 > -1)
				return Global_1673654.f_203[iParam1];
			break;
	}

	return IS_BIT_SET(Global_1673654.f_1048, iParam0);
}

BOOL func_48(char* sParam0, int iParam1, BOOL bParam2) // Position - 0x14E1
{
	int num;
	BOOL flag;
	BOOL flag2;

	if (!func_15(&num, true, iParam1))
		return false;

	flag = true;
	TEXT_LABEL_ASSIGN_STRING(&Global_23970.f_6238[num /*4*/], sParam0, 16);

	if (!unk_0xD6F9DEE4765092A9(&Global_23970.f_6238[num /*4*/]))
	{
		unk_0xF2CB0224D3BE0B42(&Global_23970.f_6238[num /*4*/], 9);
		Global_23970.f_6231[num] = true;
	
		if (!unk_0xCC2EFE4B1D0EE422(&Global_23970.f_6238[num /*4*/], 9))
			flag = false;
	}

	unk_0xD0D00ED689D6CA81("CommonMenu", 0);
	unk_0xD0D00ED689D6CA81("Shared", 1);

	if (!unk_0x38D063D8CF6D1967("Shared"))
		flag = false;

	Global_23970.f_6217[num] = true;

	if (!unk_0x38D063D8CF6D1967("CommonMenu"))
		flag = false;

	if (bParam2)
	{
		unk_0xD0D00ED689D6CA81("MPShopSale", 0);
		Global_23970.f_6224[num] = true;
	
		if (!unk_0x38D063D8CF6D1967("MPShopSale"))
			flag = false;
	}

	flag2 = false;
	TEXT_LABEL_ASSIGN_STRING(&(Global_23970.f_6263[num /*10*/].f_1), "instructional_buttons", 24);
	flag2 = func_49(&Global_23970.f_6263[num /*10*/]);
	!flag || !flag2;
	return flag && flag2;
}

BOOL func_49(var uParam0) // Position - 0x15E9
{
	switch (uParam0->f_9)
	{
		case 0:
			if (!unk_0xA0C7B98BCF1EEF9E(*uParam0))
			{
				*uParam0 = unk_0x8DE4F68A9728925E(&(uParam0->f_1));
				uParam0->f_9 = 1;
			
				if (uParam0->f_7)
				{
					if (unk_0xA0C7B98BCF1EEF9E(*uParam0))
					{
						uParam0->f_8 = unk_0x1DD05E817C89C737();
						uParam0->f_9 = 2;
					}
				}
			}
			else
			{
				uParam0->f_8 = unk_0x1DD05E817C89C737();
				uParam0->f_9 = 2;
			}
			break;
	
		case 1:
			if (unk_0xA0C7B98BCF1EEF9E(*uParam0))
			{
				uParam0->f_8 = unk_0x1DD05E817C89C737();
				uParam0->f_9 = 2;
			}
			break;
	
		case 2:
			if (!unk_0xA0C7B98BCF1EEF9E(*uParam0))
				uParam0->f_9 = 0;
			break;
	}

	return uParam0->f_9 == 2;
}

BOOL func_50() // Position - 0x168B
{
	BOOL num;

	num = 0;

	if (_STOPWATCH_IS_INITIALIZED(&(uLocal_269.f_2)))
	{
		if (_STOPWATCH_HAS_TIME_PASSED(&(uLocal_269.f_2), uLocal_269.f_22, false))
		{
			if (iLocal_267 == 4)
			{
				num = 1;
				iLocal_267 = 0;
				func_92(2);
				_STOPWATCH_DESTROY(&(uLocal_269.f_2));
			}
		}
	}
	else if (func_55())
	{
		if (unk_0x875A214D5EBCA509(2, 204))
			if (func_52())
				if (!_STOPWATCH_IS_INITIALIZED(&(uLocal_269.f_2)))
					func_51();
	}

	return num;
}

void func_51() // Position - 0x16FE
{
	_STOPWATCH_RESET(&(uLocal_269.f_2), false, false);
	iLocal_267 = 0;
	func_92(2);
	return;
}

BOOL func_52() // Position - 0x171A
{
	int i;
	int num;
	var unk;
	var unk6;
	var unk7;
	var unk8;

	i = 0;
	num = 0;
	unk = 4;

	if (iLocal_268 == 5 && func_54())
	{
		unk6 = uLocal_269.f_15[0];
		unk7 = uLocal_269.f_15[2];
		uLocal_269.f_15[0] = uLocal_269.f_15[3];
		uLocal_269.f_15[2] = unk6;
		uLocal_269.f_15[3] = unk7;
	}

	for (i = 0; i < 4; i = i + 1)
	{
		if (uLocal_269.f_10[i] == uLocal_269.f_15[i])
		{
			unk_0x0B0C9A0F9AAEB7F0(&(uLocal_269.f_21), uLocal_269.f_10[i]);
			unk[i] = 1f;
			num = num + 1;
		}
		else
		{
			uLocal_269.f_4[uLocal_269.f_10[i]] = uLocal_269.f_4[uLocal_269.f_10[i]] - 1;
			unk_0x8744D2E3FC95740E(&(uLocal_269.f_21), uLocal_269.f_10[i]);
			unk[i] = 0f;
			uLocal_269.f_10[i] = -1;
		
			if (!func_62() && !func_107(215, -1))
				func_53(7);
		
			unk_0x97A041099E92C69F(2, 350, 10);
		
			if (uLocal_269.f_4[i] < 1)
				unk_0x0B0C9A0F9AAEB7F0(&(uLocal_269.f_20), 0);
		
			fLocal_164 = fLocal_164 + 0.075f;
		}
	}

	if (!IS_BIT_SET(uLocal_269.f_20, 0))
	{
		unk8 = unk_0xCA369FBC0DE29517();
		unk_0xBF3D28CA44F3BE2D(unk8, "Test_Circuit", sLocal_162, 1);
		unk_0x3F002AA9562BF0BE(unk8, "Wire_01", unk[0]);
		unk_0x3F002AA9562BF0BE(unk8, "Wire_02", unk[1]);
		unk_0x3F002AA9562BF0BE(unk8, "Wire_03", unk[2]);
		unk_0x3F002AA9562BF0BE(unk8, "Wire_04", unk[3]);
		unk_0x394AFAC073E1F277(unk8);
	}

	return num == 4;
}

void func_53(int iParam0) // Position - 0x18AD
{
	iLocal_268 = iParam0;
	return;
}

BOOL func_54() // Position - 0x18BA
{
	int i;
	int num;

	i = 0;
	num = 0;

	for (i = 0; i < 4; i = i + 1)
	{
		if (uLocal_269.f_10[i] == uLocal_269.f_15[i])
			num = num + 1;
	}

	return num == 4;
}

BOOL func_55() // Position - 0x18F9
{
	int i;

	i = 0;

	for (i = 0; i < 4; i = i + 1)
	{
		if (uLocal_269.f_10[i] == -1)
			return false;
	}

	return true;
}

void func_56(var uParam0, var uParam1, int iParam2) // Position - 0x1928
{
	uParam0->f_1 = func_57(_STOPWATCH_GET_TIME_PASSED(uParam1, false, false), 0, iParam2);
	return;
}

int func_57(int iParam0, int iParam1, int iParam2) // Position - 0x1943
{
	if (iParam0 > iParam2)
		return iParam2;
	else if (iParam0 < iParam1)
		return iParam1;

	return iParam0;
}

int _STOPWATCH_GET_TIME_PASSED(var pStopwatch, BOOL useLocalTimer, BOOL useAccurateTime) // Position - 0x1968
{
	if (unk_0x76CD105BCAC6EB9F() && !useLocalTimer)
		if (!useAccurateTime)
			return unk_0x775142054EC39277(unk_0x7E3F74F641EE6B27(), *pStopwatch);
		else
			return unk_0x775142054EC39277(unk_0x0728E77B2BF91D54(), *pStopwatch);

	return unk_0x775142054EC39277(unk_0x1DD05E817C89C737(), *pStopwatch);
}

void func_59() // Position - 0x19AF
{
	unk_0xBF3D28CA44F3BE2D(-1, "Hack_Fail", sLocal_162, 1);

	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(unk_0x4A8C381C258A124D()))
		unk_0xBD618A73193F9982(-1, "Hack_Fail_Remote", unk_0x4A8C381C258A124D(), sLocal_162, 1, 30);

	fLocal_164 = 0f;
	return;
}

BOOL func_60() // Position - 0x19E3
{
	if (_STOPWATCH_HAS_TIME_PASSED(&uLocal_269, 240000, false))
		return true;

	if (IS_BIT_SET(uLocal_269.f_20, 0))
		return true;

	return false;
}

void func_61() // Position - 0x1A0D
{
	char* str;

	str = "";

	switch (iLocal_268)
	{
		case 1:
			if (unk_0xAE231F549813BBDF(0))
				str = "HWTUT_SEL_PC";
			else
				str = "HWTUT_SEL";
		
			if (uLocal_269.f_9 > -1)
				func_53(2);
			break;
	
		case 2:
			str = "HWTUT_WIRE";
		
			if (fLocal_301 > 0.35f)
				func_53(3);
			break;
	
		case 3:
			if (unk_0xAE231F549813BBDF(0))
				str = "HWTUT_SELR_PC";
			else
				str = "HWTUT_SELR";
		
			if (uLocal_269.f_9 > -1)
				if (uLocal_269.f_10[uLocal_269.f_9] > -1)
					func_53(4);
			else
				func_53(4);
			break;
	
		case 4:
			if (func_55())
				func_53(5);
			break;
	
		case 5:
			str = "HWTUT_TEST";
		
			if (!func_55())
			{
				_STOPWATCH_INITIALIZE(&(uLocal_269.f_27), false, false);
				func_53(6);
			}
			break;
	
		case 6:
			str = "HWTUT_FINAL";
		
			if (_STOPWATCH_HAS_TIME_PASSED(&(uLocal_269.f_27), 8500, false))
			{
				_STOPWATCH_DESTROY(&(uLocal_269.f_27));
				func_53(0);
			}
			break;
	
		case 7:
			str = "HWTUT_LIVES";
		
			if (_STOPWATCH_IS_INITIALIZED(&(uLocal_269.f_27)))
				if (_STOPWATCH_HAS_TIME_PASSED(&(uLocal_269.f_27), 8500, false))
					func_53(0);
			else
				_STOPWATCH_RESET(&(uLocal_269.f_27), false, false);
			break;
	}

	if (!unk_0x1B79E937E91F40C3(str, ""))
		unk_0x2B742757DC68DB01(str, 1);

	return;
}

BOOL func_62() // Position - 0x1B71
{
	return iLocal_268 != 0;
}

void func_63() // Position - 0x1B7E
{
	float num;
	BOOL flag;
	float num2;
	float num3;
	var unk;
	int num4;
	float num5;
	float num6;
	int num7;
	float num8;
	float num9;
	float num10;

	num = 0.01f;
	flag = true;

	if (unk_0xAE231F549813BBDF(0))
	{
		fLocal_301 = unk_0xB504E1B50AA21FC5(2, 239);
		fLocal_301.f_1 = unk_0xB504E1B50AA21FC5(2, 240);
	}
	else
	{
		num2 = unk_0xB504E1B50AA21FC5(2, 218);
		num3 = unk_0xB504E1B50AA21FC5(2, 219);
	
		if (num2 > 0.1f || num2 < -0.1f)
			fLocal_301 = fLocal_301 + ((num2 / 1.5f) * SYSTEM::TIMESTEP());
	
		if (num3 > 0.1f || num3 < -0.1f)
			fLocal_301.f_1 = fLocal_301.f_1 + ((num3 / 1.5f) * SYSTEM::TIMESTEP());
	
		func_76(&fLocal_301, &(fLocal_301.f_1));
	}

	unk.f_3 = 255;
	func_75(&unk, 255, 0, 0, 255);
	num4 = func_74();

	if (num4 > -1)
	{
		unk = { uLocal_319[num4 /*4*/] };
	
		if (uLocal_269.f_9 == -1)
		{
			num5 = SYSTEM::TO_FLOAT(unk_0x510D0699BE9C6D06(unk_0x775142054EC39277(unk_0x7E3F74F641EE6B27(), uLocal_269)));
			num6 = 0.5f;
			num = (SYSTEM::SIN(num5 * num6) * 0.019f) + 0.001f;
		}
	
		if (unk_0x875A214D5EBCA509(2, 201) || unk_0xAE231F549813BBDF(0) && unk_0x875A214D5EBCA509(2, 237))
		{
			if (uLocal_269.f_9 == -1)
			{
				unk_0xBF3D28CA44F3BE2D(-1, "Grab_Wire", sLocal_162, 1);
				flag = false;
			}
		
			uLocal_269.f_9 = num4;
		}
	}

	if (uLocal_269.f_9 > -1)
	{
		num7 = func_71();
		func_65(func_69(uLocal_269.f_9), fLocal_301);
	
		if (num7 > -1)
		{
			if (!func_64(num7))
			{
				func_75(&unk, 255, 0, 0, 255);
				num8 = SYSTEM::TO_FLOAT(unk_0x510D0699BE9C6D06(unk_0x775142054EC39277(unk_0x7E3F74F641EE6B27(), uLocal_269)));
				num9 = 0.5f;
				num = (SYSTEM::SIN(num8 * num9) * 0.019f) + 0.001f;
			
				if (unk_0x875A214D5EBCA509(2, 201) || unk_0xAE231F549813BBDF(0) && unk_0x875A214D5EBCA509(2, 237))
				{
					uLocal_269.f_10[uLocal_269.f_9] = num7;
					uLocal_269.f_9 = -1;
					unk_0xBF3D28CA44F3BE2D(-1, "Attach_Wire", sLocal_162, 1);
					flag = false;
				}
			}
		}
	}

	if (unk_0x875A214D5EBCA509(2, 201) || unk_0xAE231F549813BBDF(0) && unk_0x875A214D5EBCA509(2, 237))
		if (flag)
			unk_0xBF3D28CA44F3BE2D(-1, "Error", sLocal_162, 1);

	num10 = 0.1f + num;
	unk_0xFFA2B456A81EA1EB("MPHotwire", "Cursor_0", func_26(fLocal_301), fLocal_301.f_1, (1f / fLocal_159) * num10, 1f * num10, 0f, 255, 255, 255, 255, 0, 0);
	return;
}

BOOL func_64(int iParam0) // Position - 0x1DF3
{
	int i;

	for (i = 0; i <= 3; i = i + 1)
	{
		if (uLocal_269.f_10[i] == iParam0)
			return true;
	}

	return false;
}

void func_65(float fParam0, var uParam1, var uParam2, var uParam3) // Position - 0x1E20
{
	float num;
	float num2;
	var unk2;

	num = { func_67(func_68(fParam0, uParam2), 2f) };
	num2 = func_66(fParam0, uParam2);
	unk2 = unk_0x4964D7A2BFD2F9A3(fParam0.f_1 - uParam2.f_1, fParam0 - uParam2);
	unk_0xC54B91005A54695F("MPHotwire", "line", func_26(num), num.f_1, num2 / fLocal_159, 0.004f, unk2, 255, 255, 255, 255, 0, 0);
	return;
}

float func_66(var uParam0, var uParam1, float fParam2, var uParam3) // Position - 0x1E88
{
	return SYSTEM::SQRT(SYSTEM::POW(fParam2 - uParam0, 2f) + SYSTEM::POW(fParam2.f_1 - uParam0.f_1, 2f));
}

struct<2> func_67(float fParam0, var uParam1, float fParam2) // Position - 0x1EAD
{
	float num;

	num = fParam0 / fParam2;
	num.f_1 = fParam0.f_1 / fParam2;
	return num;
}

struct<2> func_68(float fParam0, var uParam1, var uParam2, var uParam3) // Position - 0x1ECB
{
	float num;

	num = fParam0 + uParam2;
	num.f_1 = fParam0.f_1 + uParam2.f_1;
	return num;
}

struct<2> func_69(int iParam0) // Position - 0x1EEB
{
	float num;

	num = { func_70(iParam0) };
	num = num + 0.081f;
	return num;
}

struct<2> func_70(int iParam0) // Position - 0x1F0B
{
	return func_67(func_68(uLocal_165.f_28[iParam0 /*9*/][0 /*2*/], uLocal_165.f_28[iParam0 /*9*/][2 /*2*/]), 2f);
}

int func_71() // Position - 0x1F36
{
	int i;

	for (i = 0; i <= 3; i = i + 1)
	{
		if (func_72(func_73(i), fLocal_301) < SYSTEM::POW(uLocal_165.f_65[i /*9*/][3 /*2*/] - uLocal_165.f_65[i /*9*/][0 /*2*/], 2f))
			return i;
	}

	return -1;
}

float func_72(var uParam0, var uParam1, float fParam2, var uParam3) // Position - 0x1F85
{
	return SYSTEM::POW(fParam2 - uParam0, 2f) + SYSTEM::POW(fParam2.f_1 - uParam0.f_1, 2f);
}

struct<2> func_73(int iParam0) // Position - 0x1FA6
{
	return func_67(func_68(uLocal_165.f_65[iParam0 /*9*/][0 /*2*/], uLocal_165.f_65[iParam0 /*9*/][2 /*2*/]), 2f);
}

int func_74() // Position - 0x1FD1
{
	int i;

	if (uLocal_269.f_9 != -1)
		return uLocal_269.f_9;

	for (i = 0; i <= 3; i = i + 1)
	{
		if (uLocal_269.f_10[i] == -1)
			if (func_72(func_70(i), fLocal_301) < SYSTEM::POW(uLocal_165.f_28[i /*9*/][3 /*2*/] - uLocal_165.f_28[i /*9*/][0 /*2*/], 2f))
				return i;
	}

	return -1;
}

void func_75(var uParam0, int iParam1, int iParam2, int iParam3, int iParam4) // Position - 0x203E
{
	*uParam0 = iParam1;
	uParam0->f_1 = iParam2;
	uParam0->f_2 = iParam3;
	uParam0->f_3 = iParam4;
	return;
}

void func_76(var uParam0, var uParam1) // Position - 0x205D
{
	float num;
	float num2;
	float num3;
	float num4;

	num = -0.1f;
	num2 = 1.1f;
	num3 = 0.175f;
	num4 = 0.845f;

	if (*uParam0 > num2)
		*uParam0 = num2;

	if (*uParam0 < num)
		*uParam0 = num;

	if (*uParam1 > num4)
		*uParam1 = num4;

	if (*uParam1 < num3)
		*uParam1 = num3;

	return;
}

void func_77() // Position - 0x20B9
{
	int num;
	int num2;
	int num3;
	var unk;

	if (_STOPWATCH_IS_INITIALIZED(&uLocal_269))
	{
		num = 240000 - _STOPWATCH_GET_TIME_PASSED(&uLocal_269, false, false);
		num2 = num / 60000;
		num3 = (num / 1000) % 60;
		unk_0xBFE94E91C83D8794(0.35f, 0.35f);
		unk_0x5A18938160AE52D0(76, 190, 148, 255);
		unk_0xEAF65721ACB2FDFB(1);
		TEXT_LABEL_ASSIGN_STRING(&unk, "BEAM_TR" /*~1~:~1~*/, 16);
	
		if (num3 < 10)
			TEXT_LABEL_APPEND_STRING(&unk, "S", 16);
	
		unk_0xEAEB6E7D3FAEBD5B(&unk);
		unk_0x511D14ED2DA887E1(num2);
		unk_0x511D14ED2DA887E1(num3);
		unk_0x25DD447A6EB3A86F(func_26(0.5f), 0.17f, 0);
	}

	if (!func_62() || iLocal_268 == 4)
		func_79();

	if (func_55())
	{
		func_78(202, "HWSF_QT", -1);
		func_78(204, "HWSF_TST", -1);
	}
	else if (uLocal_269.f_9 > -1)
	{
		func_78(202, "HWSF_QT", -1);
	
		if (unk_0xAE231F549813BBDF(2))
		{
			func_78(237, "HWSF_CONN", -1);
			func_78(239, "HWSF_MWRE", -1);
		}
		else
		{
			func_78(201, "HWSF_CONN", -1);
			func_78(195, "HWSF_MWRE", -1);
		}
	}
	else
	{
		func_78(202, "HWSF_QT", -1);
	
		if (unk_0xAE231F549813BBDF(2))
		{
			func_78(237, "HWSF_SEL", -1);
			func_78(239, "HWSF_MCUR", -1);
		}
		else
		{
			func_78(201, "HWSF_SEL", -1);
			func_78(195, "HWSF_MCUR", -1);
		}
	}

	return;
}

void func_78(int iParam0, char* sParam1, int iParam2) // Position - 0x220F
{
	char* str;

	str = unk_0xE934399D6F2C3AC5(2, iParam0, 1);

	if (Global_23970.f_5326 >= 14)
	{
		TEXT_LABEL_ASSIGN_STRING(&Global_4542374, str, 64);
		TEXT_LABEL_ASSIGN_STRING(&(Global_4542374.f_16), sParam1, 16);
		Global_4542374.f_20 = iParam2;
		return;
		return;
	}

	unk_0x8744D2E3FC95740E(&(Global_23970.f_5655), Global_23970.f_5326);
	TEXT_LABEL_ASSIGN_STRING(&Global_23970.f_5328[Global_23970.f_5326 /*16*/], str, 64);
	TEXT_LABEL_ASSIGN_STRING(&Global_23970.f_5553[Global_23970.f_5326 /*4*/], sParam1, 16);
	Global_23970.f_5610[Global_23970.f_5326] = iParam2;
	Global_23970.f_5625[Global_23970.f_5326] = iParam0;
	Global_23970.f_5640[Global_23970.f_5326] = 32;
	Global_23970.f_5326 = Global_23970.f_5326 + 1;
	return;
}

void func_79() // Position - 0x22BE
{
	char* str;

	str = "";

	if (func_55())
		str = "HTWR_PSY";
	else if (uLocal_269.f_9 != -1)
		if (unk_0xAE231F549813BBDF(0))
			str = "HTWR_HLP2_PC";
		else
			str = "HTWR_HLP2";
	else if (unk_0xAE231F549813BBDF(0))
		str = "HTWR_HLP_PC";
	else
		str = "HTWR_HLP";

	if (!_IS_THIS_HELP_MESSAGE_BEING_DISPLAYED(str))
		unk_0x428C32CC68809A35(1);

	_DISPLAY_HELP_TEXT_NO_SOUND(str, -1);
	return;
}

void _DISPLAY_HELP_TEXT_NO_SOUND(char* text, int iParam1) // Position - 0x232B
{
	unk_0xAC98CA65AD9A3215(text);
	unk_0x5E01B6B1F460FE3F(0, 0, 0, iParam1);
	return;
}

BOOL _IS_THIS_HELP_MESSAGE_BEING_DISPLAYED(char* message) // Position - 0x2342
{
	unk_0x39DCBE5519BD783A(message);
	return unk_0x7EBCD400E7DE179C(0);
}

void func_82() // Position - 0x2355
{
	int i;
	int j;
	BOOL flag;
	BOOL flag2;
	BOOL flag3;
	BOOL flag4;
	int num;
	float num2;
	var unk2;
	int num3;

	for (i = 0; i <= 3; i = i + 1)
	{
		if (uLocal_269.f_10[i] != -1)
			func_90(i, uLocal_269.f_10[i]);
	}

	for (i = 0; i <= 3; i = i + 1)
	{
		flag = func_74() == i;
		flag2 = func_71() == i && uLocal_269.f_9 > -1 && !func_64(i);
		flag3 = func_62() && iLocal_268 == 1 && i == 1;
		flag4 = func_62() && iLocal_268 == 3 && i == uLocal_269.f_15[1];
	
		if (iLocal_268 == 4)
			if (uLocal_269.f_9 > -1)
				if (!func_64(i))
					flag4 = true;
			else if (uLocal_269.f_10[i] == -1)
				flag3 = true;
	
		num = SYSTEM::ROUND(func_89(0, 0.4f));
		num2 = { func_67(func_68(uLocal_165.f_28[i /*9*/][0 /*2*/], uLocal_165.f_28[i /*9*/][2 /*2*/]), 2f) };
	
		if (flag3)
			unk_0xFFA2B456A81EA1EB("MPHotwire", "leftnode_roll", func_26(num2), num2.f_1, (0.17f * 1.01f) / fLocal_159, 0.062f * 1.01f, 0f, 255, 255, 0, num, 0, 0);
	
		if (flag)
		{
			func_88(i, -1);
			unk_0xFFA2B456A81EA1EB("MPHotwire", "leftnode_roll", func_26(num2), num2.f_1, 0.17f / fLocal_159, 0.062f, 0f, 255, 255, 255, 255, 0, 0);
		}
		else
		{
			unk_0x8744D2E3FC95740E(&(uLocal_269.f_20), 1 + i);
			unk_0xFFA2B456A81EA1EB("MPHotwire", "leftnode", func_26(num2), num2.f_1, 0.164f / fLocal_159, 0.056f, 0f, 255, 255, 255, 255, 0, 0);
		}
	
		num2 = { func_67(func_68(uLocal_165.f_65[i /*9*/][0 /*2*/], uLocal_165.f_65[i /*9*/][2 /*2*/]), 2f) };
	
		if (flag4)
			unk_0xFFA2B456A81EA1EB("MPHotwire", "rightnode_roll", func_26(num2), num2.f_1, (0.17f * 1.01f) / fLocal_159, 0.062f * 1.01f, 0f, 255, 255, 0, num, 0, 0);
	
		if (flag2)
		{
			func_88(-1, i);
			unk_0xFFA2B456A81EA1EB("MPHotwire", "rightnode_roll", func_26(num2), num2.f_1, 0.17f / fLocal_159, 0.062f, 0f, 255, 255, 255, 255, 0, 0);
		}
		else
		{
			unk_0x8744D2E3FC95740E(&(uLocal_269.f_20), 5 + i);
			unk_0xFFA2B456A81EA1EB("MPHotwire", "rightnode", func_26(num2), num2.f_1, 0.164f / fLocal_159, 0.056f, 0f, 255, 255, 255, 255, 0, 0);
		}
	
		unk2 = { func_86(0.851f, func_87(i)) };
		func_85(unk2, IS_BIT_SET(uLocal_269.f_21, i));
	
		if (uLocal_269.f_4[i] < 1)
			func_83(func_84(i, 0));
	
		if (uLocal_269.f_4[i] < 2)
			func_83(func_84(i, 1));
	
		if (uLocal_269.f_4[i] < 3)
			func_83(func_84(i, 2));
	
		if (uLocal_269.f_4[i] < 4)
			func_83(func_84(i, 3));
	}

	for (i = 0; i <= 1; i = i + 1)
	{
		for (j = 0; j <= 3; j = j + 1)
		{
			num3 = j + 1;
		
			if (num3 >= 4)
				num3 = 0;
		}
	}

	for (i = 0; i <= 3; i = i + 1)
	{
		j = i + 1;
	
		if (j >= 4)
			j = 0;
	}

	return;
}

void func_83(float fParam0, var uParam1) // Position - 0x26C1
{
	float num;
	float num2;
	float num3;
	float num4;
	int num5;

	num = 0.02f;
	num2 = SYSTEM::TO_FLOAT(unk_0x510D0699BE9C6D06(unk_0x775142054EC39277(unk_0x7E3F74F641EE6B27(), uLocal_269))) + (fParam0.f_1 * 9999f);
	num3 = 0.5f;
	num4 = SYSTEM::SIN(num2 * num3 * 1f) + 0.1f;
	num5 = SYSTEM::ROUND(255f * num4);
	unk_0xFFA2B456A81EA1EB("MPHotwire", "lifelight", func_26(fParam0), fParam0.f_1, num / fLocal_159, num, 0f, 255, 255, 255, num5, 0, 0);
	return;
}

struct<2> func_84(int iParam0, int iParam1) // Position - 0x273D
{
	switch (iParam0)
	{
		case 0:
			switch (iParam1)
			{
				case 3:
					return func_86(0.984f, 0.289f);
			
				case 2:
					return func_86(0.95f, 0.289f);
			
				case 1:
					return func_86(0.915f, 0.289f);
			
				case 0:
					return func_86(0.88f, 0.289f);
			
				default:
					break;
			}
			break;
	
		case 1:
			switch (iParam1)
			{
				case 3:
					return func_86(0.984f, 0.439f);
			
				case 2:
					return func_86(0.95f, 0.439f);
			
				case 1:
					return func_86(0.915f, 0.439f);
			
				case 0:
					return func_86(0.88f, 0.439f);
			
				default:
					break;
			}
			break;
	
		case 2:
			switch (iParam1)
			{
				case 3:
					return func_86(0.984f, 0.589f);
			
				case 2:
					return func_86(0.95f, 0.589f);
			
				case 1:
					return func_86(0.915f, 0.589f);
			
				case 0:
					return func_86(0.88f, 0.589f);
			
				default:
					break;
			}
			break;
	
		case 3:
			switch (iParam1)
			{
				case 3:
					return func_86(0.984f, 0.739f);
			
				case 2:
					return func_86(0.95f, 0.739f);
			
				case 1:
					return func_86(0.915f, 0.739f);
			
				case 0:
					return func_86(0.88f, 0.739f);
			
				default:
					break;
			}
			break;
	}

	return func_86(0f, 0f);
}

void func_85(float fParam0, var uParam1, BOOL bParam2) // Position - 0x2902
{
	float num;
	int num2;

	num = 0.018f;
	num2 = SYSTEM::ROUND(func_89(fParam0.f_1 * 9999f, 1050253722));

	if (bParam2)
		unk_0xFFA2B456A81EA1EB("MPHotwire", "correctlight", func_26(fParam0), fParam0.f_1, num / fLocal_159, num, 0f, 255, 255, 255, 255, 0, 0);
	else
		unk_0xFFA2B456A81EA1EB("MPHotwire", "wronglight", func_26(fParam0), fParam0.f_1, num / fLocal_159, num, 0f, 255, 255, 255, num2, 0, 0);

	return;
}

struct<2> func_86(float fParam0, float fParam1) // Position - 0x297C
{
	float num;

	num = fParam0;
	num.f_1 = fParam1;
	return num;
}

float func_87(int iParam0) // Position - 0x2992
{
	switch (iParam0)
	{
		case 0:
			return 0.29f;
	
		case 1:
			return 0.44f;
	
		case 2:
			return 0.59f;
	
		case 3:
			return 0.74f;
	
		default:
		
	}

	return 0f;
}

void func_88(int iParam0, int iParam1) // Position - 0x29DA
{
	if (iParam0 > -1)
	{
		if (!IS_BIT_SET(uLocal_269.f_20, 1 + iParam0))
		{
			unk_0xBF3D28CA44F3BE2D(-1, "Output_Highlight", sLocal_162, 1);
			unk_0x0B0C9A0F9AAEB7F0(&(uLocal_269.f_20), 1 + iParam0);
		}
	}

	if (iParam1 > -1)
	{
		if (!IS_BIT_SET(uLocal_269.f_20, 5 + iParam1))
		{
			unk_0xBF3D28CA44F3BE2D(-1, "Input_Highlight", sLocal_162, 1);
			unk_0x0B0C9A0F9AAEB7F0(&(uLocal_269.f_20), 5 + iParam1);
		}
	}

	return;
}

float func_89(int iParam0, int iParam1) // Position - 0x2A3C
{
	float num;
	float num2;

	num = SYSTEM::TO_FLOAT(unk_0x510D0699BE9C6D06(unk_0x775142054EC39277(unk_0x7E3F74F641EE6B27(), uLocal_269))) + iParam0;
	num2 = SYSTEM::SIN(num * iParam1 * 1f) + 0.1f;
	return 255f * num2;
}

void func_90(int iParam0, var uParam1) // Position - 0x2A77
{
	float num;
	var unk;
	float num2;
	var unk2;
	var unk3;
	float num3;
	var unk4;
	var unk5;
	var unk6;
	BOOL flag;

	func_91(iParam0, uParam1, &num, &unk, &num2, &unk2, &unk3, &unk6, &num3, &unk4, &unk5, &flag);
	unk_0xFFA2B456A81EA1EB("MPHotwire", &unk6, func_26(num), unk, num2 / fLocal_159, unk2, unk3, 255, 255, 255, 255, 0, 0);

	if (flag)
		unk_0xFFA2B456A81EA1EB("MPHotwire", &unk6, func_26(num3), unk4, num2 / fLocal_159, unk2, unk5, 255, 255, 255, 255, 0, 0);

	return;
}

void func_91(int iParam0, int iParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6, char* sParam7, var uParam8, var uParam9, var uParam10, var uParam11) // Position - 0x2AE4
{
	switch (iParam0)
	{
		case 0:
			switch (iParam1)
			{
				case 0:
					*uParam2 = 0.5f;
					*uParam3 = 0.29f;
					*uParam4 = 0.7f;
					*uParam5 = 0.004f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line", 32);
					*uParam11 = 0;
					break;
			
				case 1:
					*uParam2 = 0.402f - 0.076f;
					*uParam3 = 0.372f;
					*uParam4 = 0.35f;
					*uParam5 = 0.17f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line1", 32);
					*uParam8 = 0.599f + 0.076f;
					*uParam9 = 0.37f;
					*uParam10 = 180f;
					*uParam11 = 1;
					break;
			
				case 2:
					*uParam2 = 0.5f;
					*uParam3 = 0.36f;
					*uParam4 = 0.7f;
					*uParam5 = 0.17f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line2", 32);
					*uParam8 = 0.5f;
					*uParam9 = 0.528f;
					*uParam10 = 180f;
					*uParam11 = 1;
					break;
			
				case 3:
					*uParam2 = 0.5f;
					*uParam3 = 0.392f;
					*uParam4 = 0.7f;
					*uParam5 = 0.25f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line3", 32);
					*uParam8 = 0.5f;
					*uParam9 = 0.64f;
					*uParam10 = 180f;
					*uParam11 = 1;
					break;
			}
			break;
	
		case 1:
			switch (iParam1)
			{
				case 0:
					*uParam2 = 0.402f - 0.076f;
					*uParam3 = 0.369f;
					*uParam4 = -0.35f;
					*uParam5 = 0.17f;
					*uParam6 = 180f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line1", 32);
					*uParam8 = 0.599f + 0.076f;
					*uParam9 = 0.371f;
					*uParam10 = 0f;
					*uParam11 = 1;
					break;
			
				case 1:
					*uParam2 = 0.5f;
					*uParam3 = 0.44f;
					*uParam4 = 0.7f;
					*uParam5 = 0.004f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line", 32);
					*uParam11 = 0;
					break;
			
				case 2:
					*uParam2 = 0.402f - 0.076f;
					*uParam3 = 0.522f;
					*uParam4 = 0.35f;
					*uParam5 = 0.17f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line1", 32);
					*uParam8 = 0.599f + 0.076f;
					*uParam9 = 0.52f;
					*uParam10 = 180f;
					*uParam11 = 1;
					break;
			
				case 3:
					*uParam2 = 0.5f;
					*uParam3 = 0.51f;
					*uParam4 = 0.7f;
					*uParam5 = 0.17f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line2", 32);
					*uParam8 = 0.5f;
					*uParam9 = 0.678f;
					*uParam10 = 180f;
					*uParam11 = 1;
					break;
			}
			break;
	
		case 2:
			switch (iParam1)
			{
				case 0:
					*uParam2 = 0.5f;
					*uParam3 = 0.36f;
					*uParam4 = -0.7f;
					*uParam5 = 0.17f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line2", 32);
					*uParam8 = 0.5f;
					*uParam9 = 0.528f;
					*uParam10 = 180f;
					*uParam11 = 1;
					break;
			
				case 1:
					*uParam2 = 0.402f - 0.076f;
					*uParam3 = 0.519f;
					*uParam4 = -0.35f;
					*uParam5 = 0.17f;
					*uParam6 = 180f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line1", 32);
					*uParam8 = 0.599f + 0.076f;
					*uParam9 = 0.521f;
					*uParam10 = 0f;
					*uParam11 = 1;
					break;
			
				case 2:
					*uParam2 = 0.5f;
					*uParam3 = 0.59f;
					*uParam4 = 0.7f;
					*uParam5 = 0.004f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line", 32);
					*uParam11 = 0;
					break;
			
				case 3:
					*uParam2 = 0.402f - 0.076f;
					*uParam3 = 0.672f;
					*uParam4 = 0.35f;
					*uParam5 = 0.17f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line1", 32);
					*uParam8 = 0.599f + 0.076f;
					*uParam9 = 0.67f;
					*uParam10 = 180f;
					*uParam11 = 1;
					break;
			}
			break;
	
		case 3:
			switch (iParam1)
			{
				case 0:
					*uParam2 = 0.5f;
					*uParam3 = 0.392f;
					*uParam4 = -0.7f;
					*uParam5 = 0.25f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line3", 32);
					*uParam8 = 0.5f;
					*uParam9 = 0.64f;
					*uParam10 = 180f;
					*uParam11 = 1;
					break;
			
				case 1:
					*uParam2 = 0.5f;
					*uParam3 = 0.51f;
					*uParam4 = -0.7f;
					*uParam5 = 0.17f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line2", 32);
					*uParam8 = 0.5f;
					*uParam9 = 0.678f;
					*uParam10 = 180f;
					*uParam11 = 1;
					break;
			
				case 2:
					*uParam2 = 0.402f - 0.076f;
					*uParam3 = 0.669f;
					*uParam4 = -0.35f;
					*uParam5 = 0.17f;
					*uParam6 = 180f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line1", 32);
					*uParam8 = 0.599f + 0.076f;
					*uParam9 = 0.671f;
					*uParam10 = 0f;
					*uParam11 = 1;
					break;
			
				case 3:
					*uParam2 = 0.5f;
					*uParam3 = 0.74f;
					*uParam4 = 0.7f;
					*uParam5 = 0.004f;
					*uParam6 = 0f;
					TEXT_LABEL_ASSIGN_STRING(sParam7, "line", 32);
					*uParam11 = 0;
					break;
			}
			break;
	}

	return;
}

void func_92(int iParam0) // Position - 0x302B
{
	iLocal_298 = iParam0;
	return;
}

int func_93(int iParam0, int iParam1, BOOL bParam2, int iParam3) // Position - 0x3038
{
	int num;

	num = 0;

	if (unk_0x761778199FE1211C())
	{
		if (unk_0x4A0B7E53EBC937D5() != iParam0 && bParam2)
		{
			unk_0x7882946B06ED216B(iParam0, iParam1, 1, iParam3);
			num = 1;
		}
	}

	return num;
}

void func_94() // Position - 0x306B
{
	if (!Global_2672939.f_1023.f_10)
		Global_2672939.f_1023.f_10 = 1;

	return;
}

void _SET_PLAYER_CONTROL_EX(Player plParam0, BOOL bParam1, eSetPlayerControlFlags espcfParam2, int iParam3) // Position - 0x308A
{
	eSetPlayerControlFlags controlBs;
	BOOL flag;
	BOOL flag2;
	BOOL flag3;
	BOOL flag4;
	BOOL flag5;
	BOOL flag6;
	BOOL flag7;
	BOOL flag8;
	BOOL flag9;
	BOOL flag10;
	BOOL flag11;
	BOOL flag12;
	BOOL flag13;
	BOOL flag14;
	BOOL flag15;
	BOOL flag16;
	BOOL flag17;
	BOOL flag18;
	BOOL flag19;
	BOOL flag20;
	BOOL flag21;
	BOOL flag22;
	BOOL flag23;
	BOOL flag24;
	BOOL flag25;
	BOOL flag26;
	var unk;
	int num;

	if (bParam1)
		if (unk_0x828F3FAE99DA791A())
			unk_0x6FF322107B12B749(0);

	if (func_105())
		if (bParam1)
			return;
		else
			iParam3 = 1;

	if (!unk_0x76CD105BCAC6EB9F())
	{
		controlBs = espcfParam2;
		unk_0x4686BC3BFDBB5348(plParam0, bParam1, controlBs);
	}
	else
	{
		flag = true;
		flag2 = espcfParam2 & SPC_AMBIENT_SCRIPT != 0;
		flag3 = espcfParam2 & SPC_CLEAR_TASKS != 0;
		flag4 = espcfParam2 & SPC_REMOVE_FIRES != 0;
		flag5 = espcfParam2 & SPC_REMOVE_EXPLOSIONS != 0;
		flag6 = espcfParam2 & SPC_REMOVE_PROJECTILES != 0;
		flag7 = espcfParam2 & SPC_DEACTIVATE_GADGETS != 0;
		flag8 = espcfParam2 & SPC_REENABLE_CONTROL_ON_DEATH != 0;
		flag9 = espcfParam2 & SPC_LEAVE_CAMERA_CONTROL_ON != 0;
		flag10 = espcfParam2 & SPC_ALLOW_PLAYER_DAMAGE != 0;
		flag11 = espcfParam2 & SPC_DONT_STOP_OTHER_CARS_AROUND_PLAYER != 0;
		flag12 = espcfParam2 & SPC_PREVENT_EVERYBODY_BACKOFF != 0;
		flag13 = espcfParam2 & SPC_ALLOW_PAD_SHAKE != 0;
		flag14 = espcfParam2 & 8192 != 0;
		flag15 = espcfParam2 & 16384 != 0;
		flag16 = espcfParam2 & 32768 != 0;
		flag17 = espcfParam2 & 65536 != 0;
		flag18 = espcfParam2 & 131072 != 0;
		flag19 = espcfParam2 & 262144 != 0;
		flag20 = espcfParam2 & 524288 != 0;
		flag21 = espcfParam2 & 1048576 != 0;
		flag22 = espcfParam2 & 2097152 != 0;
		flag23 = espcfParam2 & 4194304 != 0;
		flag24 = espcfParam2 & 8388608 != 0;
		flag25 = espcfParam2 & 16777216 != 0;
	
		if (espcfParam2 & 33554432 != 0 || unk_0x834C960822A4683F())
			flag = false;
	
		if (!func_19())
		{
			flag26 = false;
		
			if (bParam1 == true)
				flag26 = true;
		
			if (flag16 == false && !flag21)
				flag26 = true;
		
			if (flag10 == true)
				flag26 = true;
		
			if (flag26)
				return;
		}
	
		flag18;
	
		if (unk_0x762604C40829DB72(plParam0) && unk_0x75EAB09F5E974116(plParam0) || iParam3 == 1)
		{
			if (iParam3 && unk_0x1C2F771CDC87A3A5(unk_0x56E414973C2A8C0E(plParam0), 0))
				return;
		
			unk = unk_0x56E414973C2A8C0E(plParam0);
		
			if (!flag20)
			{
				if (flag19 && bParam1 == false && unk_0x76CD105BCAC6EB9F())
					unk_0x6E2FA5434737D22F(1);
				else if (flag14 || !func_40(unk_0x259BE71D8A81D4FA(), 0) && !func_104() && !func_103(unk_0x259BE71D8A81D4FA()))
					unk_0x4285E11B28063EE0(unk, !flag14, 0);
			
				if (!flag14)
				{
					if (unk_0x76CD105BCAC6EB9F() && !flag19)
						unk_0x6E2FA5434737D22F(0);
				
					Global_2657991[plParam0 /*467*/].f_257 = 0;
				}
			}
		
			if (bParam1)
			{
				if (flag)
				{
					func_100(0, 0, 0);
				
					if (flag25)
						unk_0xE3F88173F42C071B();
				}
			
				if (!func_99(unk) && !unk_0xB431D60610E7F85F(unk))
					if (!flag22)
						unk_0x44C48AC14D3C09ED(unk, 1, 0);
			
				if (!unk_0xA3736D76B0E93E93(unk))
				{
					if (!flag21)
						unk_0x5D7CD709B34C90F0(unk, 0);
				
					unk_0x11C125313CB8ADA2(unk, 1);
				}
				else if (!flag21)
				{
					unk_0x5D7CD709B34C90F0(unk, 0);
				}
			
				unk_0x3F58BFCF656F0DF1(unk, 1);
				unk_0x2B52F77101390E6F(plParam0, 0);
				unk_0x6C4F5AA91D39455C(plParam0, 0);
			
				if (unk_0x4CEC77F224BCD884(unk) && unk_0x69CD279BFCFE278E(unk))
					unk_0xAF8337BF5A296283(unk);
			
				unk_0x9FF00EA9A61211D2(unk, 1);
			
				if (unk_0xE916D57851F785AB(unk_0x259BE71D8A81D4FA()) == 0)
				{
					func_98();
					func_97();
				}
			
				if (unk_0x705A7AB2D4BC0A9B())
					!flag23;
			
				unk_0x787F8EE1F6FBDC6D();
				Global_2657991[plParam0 /*467*/].f_258 = 0;
			
				if (!flag24)
					flag3 = true;
			
				if (Global_2697787)
				{
					unk_0xDD98B34A4A3AFA89(1);
					unk_0xDD98B34A4A3AFA89(1);
					Global_2697787 = false;
				}
			
				if (Global_2635516.f_2982)
					Global_2635516.f_2982 = 0;
			}
			else
			{
				if (!func_99(unk) && !unk_0xB431D60610E7F85F(unk))
				{
					if (!flag22)
						unk_0x44C48AC14D3C09ED(unk, !flag15, 0);
				
					if (!unk_0xA3736D76B0E93E93(unk))
					{
						if (!flag21)
							unk_0x5D7CD709B34C90F0(unk, flag16);
					
						if (!flag16)
							unk_0x11C125313CB8ADA2(unk, 1);
					}
				
					if (func_96(*Global_4718592.f_187633))
						unk_0x5D7CD709B34C90F0(unk, 1);
				}
			
				if (Global_1575056)
					flag10 = false;
			
				if (flag10)
					unk_0x2B52F77101390E6F(plParam0, 0);
				else
					unk_0x2B52F77101390E6F(plParam0, 1);
			
				unk_0x3F58BFCF656F0DF1(unk, flag17);
			
				if (flag3)
					if (!unk_0xBFD01C2045360289(unk) && !unk_0x7F420695E3F776FB(unk, 0))
						unk_0x19626F992DC71FB9(unk);
			}
		
			num = 0;
		
			if (flag2)
				num = num | 2;
		
			if (flag3)
				num = num | 4;
		
			if (flag4)
				num = num | 8;
		
			if (flag5)
				num = num | 16;
		
			if (flag6)
				num = num | 32;
		
			if (flag7)
				num = num | 64;
		
			if (flag8)
				num = num | 128;
		
			if (flag9)
				num = num | 256;
		
			if (flag10)
				num = num | 512;
		
			if (flag11)
				num = num | 1024;
		
			if (flag12)
				num = num | 2048;
		
			if (flag13)
				num = num | 4096;
		
			unk_0x4686BC3BFDBB5348(plParam0, bParam1, num);
		}
	}

	return;
}

BOOL func_96(int iParam0) // Position - 0x353E
{
	return iParam0 == 17;
}

void func_97() // Position - 0x354B
{
	var unk;

	Global_2672939.f_1100 = 0;
	Global_2672939.f_1101 = 0;
	Global_2672939.f_1102 = { 9999.9f, 9999.9f, 9999.9f };
	Global_2672939.f_1107 = -1;
	Global_2672939.f_1108 = 0;
	Global_2635516.f_2993 = { unk };
	return;
}

void func_98() // Position - 0x3598
{
	Global_2635516.f_703 = 0;
	Global_2635516.f_3036 = 0;
	Global_2635516.f_516 = 0;
	Global_2635516.f_607 = 0;
	Global_2657991[unk_0x259BE71D8A81D4FA() /*467*/].f_220 = 0;
	Global_2635516.f_2991 = 0;
	return;
}

BOOL func_99(var uParam0) // Position - 0x35D6
{
	int num;

	if (unk_0x7F420695E3F776FB(uParam0, 1))
	{
		return true;
	}
	else
	{
		num = unk_0x9B5C1660CCDF7189(uParam0, joaat("SCRIPT_TASK_ENTER_VEHICLE"));
	
		if (num == 0)
			return true;
	}

	return false;
}

void func_100(int iParam0, int iParam1, int iParam2) // Position - 0x3607
{
	int num;
	int i;

	unk_0x1C2F771CDC87A3A5(iParam1, 0);

	if (iParam0 == 1)
		if (unk_0xFC8BFE4B41177C22(iParam1))
			if (unk_0x501EBB0523078750(iParam1))
				if (!unk_0xDFAE61B1AEF262C2(unk_0x259BE71D8A81D4FA(), unk_0x1C1C92A1CBAE364B(iParam1)))
					num = 1;

	if (num == 0)
	{
		if (iParam2 == 1)
		{
			if (iParam0 == 1)
			{
				func_102();
			}
			else if (!IS_BIT_SET(Global_2621446.f_67, 1))
			{
				if (IS_BIT_SET(Global_2621446.f_67, 2))
				{
					for (i = 0; i < 8; i = i + 1)
					{
						unk_0x07938654FF332D78(i, Global_2621446.f_58[i]);
					}
				}
			
				unk_0x0B0C9A0F9AAEB7F0(&(Global_2621446.f_67), 1);
			}
		}
	
		if (func_40(unk_0x259BE71D8A81D4FA(), 0))
			unk_0x522EF6D7B4059F2C(iParam0, iParam1, 1);
		else
			unk_0xF11FEC6A04FD7226(iParam0, iParam1);
	
		unk_0x4BDB60141FD6E156(iParam0, iParam1);
		func_101(joaat("MPPLY_IS_CHAR_SPECTATING"), iParam0);
	}

	return;
}

void func_101(int iParam0, int iParam1) // Position - 0x36D7
{
	int num;

	num = iParam0;

	if (num != 0)
		unk_0xF1D0B0CE940F620D(num, iParam1, 1);

	return;
}

void func_102() // Position - 0x36F3
{
	int i;

	if (!unk_0xAD15761928FCF79C())
	{
		if (!IS_BIT_SET(Global_2621446.f_67, 2))
		{
			for (i = 0; i < 8; i = i + 1)
			{
				Global_2621446.f_58[i] = unk_0xBCF87EE3DC296C2A(i);
			}
		
			unk_0x0B0C9A0F9AAEB7F0(&(Global_2621446.f_67), 2);
			unk_0x0B0C9A0F9AAEB7F0(&(Global_2621446.f_67), 0);
		}
	}

	return;
}

BOOL func_103(int iParam0) // Position - 0x374B
{
	if (func_40(iParam0, 0))
		return true;

	if (func_104())
		if (iParam0 == unk_0x259BE71D8A81D4FA())
			return true;

	if (IS_BIT_SET(Global_2657991[iParam0 /*467*/].f_202, 2))
		return true;

	return false;
}

BOOL func_104() // Position - 0x378A
{
	return IS_BIT_SET(Global_2621446, 3);
}

BOOL func_105() // Position - 0x3798
{
	if (IS_BIT_SET(Global_1845221[unk_0x259BE71D8A81D4FA() /*889*/].f_885, 2) && !(Global_2685658.f_2847.f_216 == -1))
		return true;

	return false;
}

void func_106() // Position - 0x37C9
{
	int i;
	int num;
	var unk;

	unk_0x3B9F35917EDE3ACB(unk_0x7E3F74F641EE6B27());

	for (i = 0; i <= 3; i = i + 1)
	{
		uLocal_269.f_15[i] = i;
	}

	for (i = 0; i <= 3; i = i + 1)
	{
		num = i + unk_0xC5935DFB3F39785A(0, 4 - i);
		unk = uLocal_269.f_15[num];
		uLocal_269.f_15[num] = uLocal_269.f_15[i];
		uLocal_269.f_15[i] = unk;
	}

	return;
}

BOOL func_107(int iParam0, int iParam1) // Position - 0x383E
{
	var unk;
	BOOL num;

	unk = func_30(iParam0, iParam1);

	if (unk_0xF249567F2E83E093(unk, &num, -1))
		return num;

	return false;
}

void func_108() // Position - 0x3862
{
	func_75(&uLocal_303, 255, 255, 255, 255);
	func_75(&uLocal_307, 255, 255, 255, 255);
	func_75(&uLocal_311, 255, 255, 255, 255);
	func_75(&uLocal_315, 175, 175, 175, 255);
	func_75(&uLocal_319[0 /*4*/], 255, 0, 255, 255);
	func_75(&uLocal_319[1 /*4*/], 0, 255, 0, 255);
	func_75(&uLocal_319[2 /*4*/], 0, 0, 255, 255);
	func_75(&uLocal_319[3 /*4*/], 255, 255, 0, 255);
	return;
}

void func_109() // Position - 0x38DD
{
	float num;

	fLocal_301 = 0.5f;
	fLocal_301.f_1 = 0.5f;
	num = 0.071f;
	uLocal_165[0 /*2*/] = { func_86(-0.1f, 0.845f) };
	uLocal_165[1 /*2*/] = { func_86(-0.1f, 0.17f) };
	uLocal_165[2 /*2*/] = { func_86(1.1f, 0.17f) };
	uLocal_165[3 /*2*/] = { func_86(1.1f, 0.845f) };
	uLocal_165.f_9[0 /*9*/][0 /*2*/] = { func_86(0.1f, 0.845f) };
	uLocal_165.f_9[0 /*9*/][1 /*2*/] = { func_86(0.1f, 0.17f) };
	uLocal_165.f_9[0 /*9*/][2 /*2*/] = { func_86(0.2f, 0.17f) };
	uLocal_165.f_9[0 /*9*/][3 /*2*/] = { func_86(0.2f, 0.845f) };
	uLocal_165.f_9[1 /*9*/][0 /*2*/] = { func_86(0.8f, 0.845f) };
	uLocal_165.f_9[1 /*9*/][1 /*2*/] = { func_86(0.8f, 0.17f) };
	uLocal_165.f_9[1 /*9*/][2 /*2*/] = { func_86(0.9f, 0.17f) };
	uLocal_165.f_9[1 /*9*/][3 /*2*/] = { func_86(0.9f, 0.845f) };
	uLocal_165.f_28[0 /*9*/][0 /*2*/] = { func_86(0.1f - num, 0.32f) };
	uLocal_165.f_28[0 /*9*/][1 /*2*/] = { func_86(0.1f - num, 0.245f) };
	uLocal_165.f_28[0 /*9*/][2 /*2*/] = { func_86(0.2f - num, 0.245f) };
	uLocal_165.f_28[0 /*9*/][3 /*2*/] = { func_86(0.2f - num, 0.32f) };
	uLocal_165.f_28[1 /*9*/][0 /*2*/] = { func_86(0.1f - num, 0.47f) };
	uLocal_165.f_28[1 /*9*/][1 /*2*/] = { func_86(0.1f - num, 0.395f) };
	uLocal_165.f_28[1 /*9*/][2 /*2*/] = { func_86(0.2f - num, 0.395f) };
	uLocal_165.f_28[1 /*9*/][3 /*2*/] = { func_86(0.2f - num, 0.47f) };
	uLocal_165.f_28[2 /*9*/][0 /*2*/] = { func_86(0.1f - num, 0.62f) };
	uLocal_165.f_28[2 /*9*/][1 /*2*/] = { func_86(0.1f - num, 0.545f) };
	uLocal_165.f_28[2 /*9*/][2 /*2*/] = { func_86(0.2f - num, 0.545f) };
	uLocal_165.f_28[2 /*9*/][3 /*2*/] = { func_86(0.2f - num, 0.62f) };
	uLocal_165.f_28[3 /*9*/][0 /*2*/] = { func_86(0.1f - num, 0.77f) };
	uLocal_165.f_28[3 /*9*/][1 /*2*/] = { func_86(0.1f - num, 0.695f) };
	uLocal_165.f_28[3 /*9*/][2 /*2*/] = { func_86(0.2f - num, 0.695f) };
	uLocal_165.f_28[3 /*9*/][3 /*2*/] = { func_86(0.2f - num, 0.77f) };
	uLocal_165.f_65[0 /*9*/][0 /*2*/] = { func_86(0.8f + num, 0.32f) };
	uLocal_165.f_65[0 /*9*/][1 /*2*/] = { func_86(0.8f + num, 0.245f) };
	uLocal_165.f_65[0 /*9*/][2 /*2*/] = { func_86(0.9f + num, 0.245f) };
	uLocal_165.f_65[0 /*9*/][3 /*2*/] = { func_86(0.9f + num, 0.32f) };
	uLocal_165.f_65[1 /*9*/][0 /*2*/] = { func_86(0.8f + num, 0.47f) };
	uLocal_165.f_65[1 /*9*/][1 /*2*/] = { func_86(0.8f + num, 0.395f) };
	uLocal_165.f_65[1 /*9*/][2 /*2*/] = { func_86(0.9f + num, 0.395f) };
	uLocal_165.f_65[1 /*9*/][3 /*2*/] = { func_86(0.9f + num, 0.47f) };
	uLocal_165.f_65[2 /*9*/][0 /*2*/] = { func_86(0.8f + num, 0.62f) };
	uLocal_165.f_65[2 /*9*/][1 /*2*/] = { func_86(0.8f + num, 0.545f) };
	uLocal_165.f_65[2 /*9*/][2 /*2*/] = { func_86(0.9f + num, 0.545f) };
	uLocal_165.f_65[2 /*9*/][3 /*2*/] = { func_86(0.9f + num, 0.62f) };
	uLocal_165.f_65[3 /*9*/][0 /*2*/] = { func_86(0.8f + num, 0.77f) };
	uLocal_165.f_65[3 /*9*/][1 /*2*/] = { func_86(0.8f + num, 0.695f) };
	uLocal_165.f_65[3 /*9*/][2 /*2*/] = { func_86(0.9f + num, 0.695f) };
	uLocal_165.f_65[3 /*9*/][3 /*2*/] = { func_86(0.9f + num, 0.77f) };
	return;
}

void func_110() // Position - 0x3DC2
{
	var unk;
	int i;

	unk.f_4 = 4;
	unk.f_10 = 4;
	unk.f_15 = 4;
	unk.f_22 = 350;
	uLocal_269 = { unk };

	for (i = 0; i <= 3; i = i + 1)
	{
		uLocal_269.f_4[i] = 4;
		uLocal_269.f_10[i] = -1;
		unk_0x8744D2E3FC95740E(&(uLocal_269.f_21), i);
	}

	uLocal_269.f_9 = -1;
	return;
}

void func_111() // Position - 0x3E29
{
	func_112(0f, 0f, 1f, 1f, 0, 0, 0, 255);
	unk_0xFFA2B456A81EA1EB("MPHotwire", "bg", 0.5f, 0.5f, 1f * fLocal_160, 1f, 0f, uLocal_303, uLocal_303.f_1, uLocal_303.f_2, uLocal_303.f_3, 0, 0);
	return;
}

void func_112(float fParam0, float fParam1, float fParam2, float fParam3, int iParam4, int iParam5, int iParam6, int iParam7) // Position - 0x3E6D
{
	unk_0x81645EE95A114FAE(fParam0 + (fParam2 * 0.5f), fParam1 + (fParam3 * 0.5f), fParam2, fParam3, iParam4, iParam5, iParam6, iParam7, 0);
	return;
}

void func_113() // Position - 0x3E9C
{
	func_127(0);
	unk_0xE0EEB603997F273F(0);
	unk_0xE0EEB603997F273F(2);
	unk_0xA66AC5949E305457(0);
	func_126(1);
	func_125(4, -1);
	unk_0xD10838CEA97E4725(19);
	func_124();
	func_123();
	func_122();
	unk_0x0C9B2F8C2BD128C2();

	if (!func_19())
		func_92(4);

	func_121();
	unk_0xC0964AABD3C0CC7E();

	if (!unk_0x4D9174D8796EA622())
	{
		unk_0x43AE50D2A33F6E2A();
		unk_0xA4F67CEB594AE064(1);
	}

	func_120();
	func_119();
	func_114();
	return;
}

void func_114() // Position - 0x3F0B
{
	int num;

	func_118(false, false);
	func_117(1, 0, 0, 0, 0);
	func_116(0, 0, 0, 0, 0);
	func_48(0, -1, false);
	num = -1;

	if (func_15(&num, false, -1) && unk_0xA0C7B98BCF1EEF9E(Global_23970.f_6263[num /*10*/]))
		func_115(-1);

	return;
}

void func_115(int iParam0) // Position - 0x3F59
{
	int i;
	int num;

	Global_23970.f_5326 = 0;
	Global_23970.f_5327 = 0;

	for (i = 0; i < 14; i = i + 1)
	{
		TEXT_LABEL_ASSIGN_STRING(&Global_23970.f_5553[i /*4*/], "", 16);
		Global_23970.f_5610[i] = -1;
		Global_23970.f_5625[i] = 365;
		Global_23970.f_5640[i] = 32;
	}

	Global_23970.f_5655 = 0;
	TEXT_LABEL_ASSIGN_STRING(&(Global_4542374.f_16), "", 16);
	Global_4542374.f_20 = -1;

	if (unk_0x761778199FE1211C())
	{
		if (!func_15(&num, false, iParam0))
			return;
	
		if (unk_0xA0C7B98BCF1EEF9E(Global_23970.f_6263[num /*10*/]))
		{
			unk_0x88F483FBD433696A(Global_23970.f_6263[num /*10*/], "TOGGLE_MOUSE_BUTTONS");
			unk_0x557F1E2300EF1A3E(0);
			unk_0xE6B753D52F4CA222();
		}
	}

	return;
}

void func_116(int iParam0, int iParam1, int iParam2, int iParam3, int iParam4) // Position - 0x4012
{
	Global_23970.f_5678[0] = iParam0;
	Global_23970.f_5678[1] = iParam1;
	Global_23970.f_5678[2] = iParam2;
	Global_23970.f_5678[3] = iParam3;
	Global_23970.f_5678[4] = iParam4;
	return;
}

void func_117(int iParam0, int iParam1, int iParam2, int iParam3, int iParam4) // Position - 0x4051
{
	Global_23970.f_5659[0] = iParam0;
	Global_23970.f_5659[1] = iParam1;
	Global_23970.f_5659[2] = iParam2;
	Global_23970.f_5659[3] = iParam3;
	Global_23970.f_5659[4] = iParam4;
	Global_23970.f_5829 = 0;

	if (iParam0 != 0)
		Global_23970.f_5829 = Global_23970.f_5829 + 1;

	if (iParam1 != 0)
		Global_23970.f_5829 = Global_23970.f_5829 + 1;

	if (iParam2 != 0)
		Global_23970.f_5829 = Global_23970.f_5829 + 1;

	if (iParam3 != 0)
		Global_23970.f_5829 = Global_23970.f_5829 + 1;

	if (iParam4 != 0)
		Global_23970.f_5829 = Global_23970.f_5829 + 1;

	return;
}

void func_118(BOOL bParam0, BOOL bParam1) // Position - 0x40FB
{
	int i;
	int j;
	float num;

	for (i = 0; i < 256; i = i + 1)
	{
		TEXT_LABEL_ASSIGN_STRING(&Global_23970.f_79[i /*6*/], "", 24);
	
		for (j = 0; j < 4; j = j + 1)
		{
			Global_23970.f_2387[i /*5*/][j] = 0;
		}
	}

	for (i = 0; i < 50; i = i + 1)
	{
		TEXT_LABEL_ASSIGN_STRING(&Global_2696970[i /*16*/], "", 64);
	}

	for (i = 0; i < 256; i = i + 1)
	{
		Global_23970.f_4469[i] = 0;
	}

	for (i = 0; i < 128; i = i + 1)
	{
		Global_23970.f_4726[i] = 0f;
	}

	for (i = 0; i < 256; i = i + 1)
	{
		Global_23970.f_4984[i] = 0;
	}

	for (i = 0; i < 128; i = i + 1)
	{
		Global_23970.f_5692[i] = 0;
		Global_23970.f_5830[i] = 0;
		Global_23970.f_5959[i] = 0;
		Global_23970.f_6482[i] = 0f;
		Global_23970.f_6088[i] = 0;
		Global_23970.f_6348[i] = 0f;
	}

	for (i = 0; i < 5; i = i + 1)
	{
		Global_23970.f_5659[i] = 0;
		Global_23970.f_5671[i] = 0f;
		Global_23970.f_5665[i] = -1f;
		Global_23970.f_5678[i] = 0;
		Global_23970.f_5686[i] = 1;
	}

	for (i = 0; i < 14; i = i + 1)
	{
		TEXT_LABEL_ASSIGN_STRING(&Global_23970.f_5553[i /*4*/], "", 16);
		Global_23970.f_5610[i] = -1;
		Global_23970.f_5625[i] = 365;
		Global_23970.f_5640[i] = 32;
	}

	for (i = 0; i < 66; i = i + 1)
	{
		TEXT_LABEL_ASSIGN_STRING(&Global_23970.f_6623[i /*16*/], "", 64);
		TEXT_LABEL_ASSIGN_STRING(&Global_23970.f_7680[i /*16*/], "", 64);
	}

	if (bParam1)
	{
		for (i = 0; i < 256; i = i + 1)
		{
			Global_23970.f_1616[i] = 0;
		}
	}

	TEXT_LABEL_ASSIGN_STRING(&(Global_4542374.f_16), "", 16);
	Global_4542374.f_20 = -1;
	Global_23970 = 0;
	Global_23970.f_5821 = 0;
	Global_23970.f_5822 = 0;
	Global_23970.f_5823 = 0;
	Global_23970.f_5825 = 0;
	Global_23970.f_5826 = 0;
	Global_23970.f_5827 = 0;
	Global_23970.f_5824 = 0;
	Global_23970.f_6477 = 0;
	Global_23970.f_6617 = 0;
	Global_23970.f_6342 = 0;
	Global_23970.f_6341 = 0;
	Global_23970.f_6343 = 0;
	TEXT_LABEL_ASSIGN_STRING(&(Global_23970.f_5241), "", 24);
	Global_23970.f_5319 = 0;
	Global_23970.f_5320 = 0;
	Global_23970.f_5321 = 0;
	Global_23970.f_5322 = 0;
	Global_23970.f_5323 = 0;
	Global_23970.f_5324 = 0;

	for (i = 0; i < 4; i = i + 1)
	{
		Global_23970.f_5253[i] = 0;
	}

	Global_23970.f_5325 = 0;
	TEXT_LABEL_ASSIGN_STRING(&(Global_4542374.f_21), "", 16);
	Global_4542374.f_61 = 0;
	Global_4542374.f_62 = 0;
	Global_4542374.f_63 = 0;
	Global_4542374.f_64 = 0;
	Global_4542374.f_65 = 0;
	Global_4542374.f_66 = 0;

	for (i = 0; i < 4; i = i + 1)
	{
		Global_4542374.f_25[i] = 0;
	}

	Global_4542374.f_67 = 0;
	TEXT_LABEL_ASSIGN_STRING(&(Global_23970.f_1), "", 16);
	Global_23970.f_5677 = 0f;
	Global_23970.f_74 = 0;
	Global_23970.f_75 = 0;
	Global_23970.f_76 = 0;
	Global_23970.f_77 = 0;
	Global_23970.f_78 = 0;

	for (i = 0; i < 4; i = i + 1)
	{
		Global_23970.f_5[i] = 0;
	}

	Global_23970.f_6347 = 0;
	Global_23970.f_6346 = 0;
	Global_23970.f_6344 = 0;
	Global_23970.f_6345 = 0;
	Global_23970.f_5326 = 0;
	Global_23970.f_5327 = 0;
	Global_23970.f_5828 = 10;
	Global_23970.f_5829 = 0;
	Global_23970.f_6479 = 0f;
	Global_23970.f_6480 = 0f;
	Global_23970.f_6331 = 0;
	Global_23970.f_6332 = 0;
	Global_23970.f_6333 = 0f;
	Global_23970.f_6334 = 0;
	Global_23970.f_6336 = 0;
	Global_23970.f_6335 = 0;
	Global_23970.f_6337 = 0;
	Global_23970.f_6338 = 0;
	Global_23970.f_6339 = 0;
	Global_23970.f_6340 = 0;
	Global_23970.f_9112 = 0;

	for (i = 0; i < 2; i = i + 1)
	{
		Global_23970.f_6611[i] = -1;
		Global_23970.f_6614[i] = -1;
	}

	Global_23970.f_5684 = 0f;
	Global_23970.f_5655 = 0;
	Global_23970.f_5685 = 0;

	for (i = 0; i < Global_23970.f_6618; i = i + 1)
	{
		Global_23970.f_6618[i] = 0;
	}

	Global_23970.f_9091 = 0;
	Global_23970.f_9086 = 0;
	Global_23970.f_9096 = 0;
	Global_23970.f_9101 = 0;
	Global_23970.f_9106 = 0;
	Global_23970.f_9108 = 0;
	Global_23970.f_9114 = 0;
	Global_23967 = 0.05f;
	Global_23968 = 0.05f;
	Global_23969 = 0.225f;
	num = unk_0x4AE9635532D92447(0);

	if (bParam0)
		Global_23969 = 0.225f * (1.7777778f / num);
	else if (num < 1.77777f)
		Global_23969 = 0.225f * (1.7777778f / num);
	else
		Global_23969 = 0.225f;

	return;
}

void func_119() // Position - 0x45E5
{
	fLocal_160 = 1.778f / fLocal_159;
	fLocal_160 = fLocal_160;
	return;
}

void func_120() // Position - 0x45FB
{
	fLocal_159 = unk_0x4AE9635532D92447(0);

	if (unk_0x761778199FE1211C())
		if (fLocal_159 >= 4f)
			fLocal_159 = fLocal_159 / 3f;

	return;
}

void func_121() // Position - 0x461E
{
	Global_23831.f_134 = 1;
	return;
}

void func_122() // Position - 0x462C
{
	Global_2739811.f_4667 = 0;
	return;
}

void func_123() // Position - 0x463C
{
	Global_23831.f_6 = 1;
	return;
}

void func_124() // Position - 0x464A
{
	Global_1574755 = true;
	TEXT_LABEL_ASSIGN_STRING(&(Global_1574755.f_1), unk_0x1AF90EB93E0012D6(), 32);
	Global_1574755.f_9 = unk_0x70E57E9927B6BA58(&(Global_1574755.f_1));
	return;
}

void func_125(int iParam0, int iParam1) // Position - 0x4673
{
	unk_0x0B0C9A0F9AAEB7F0(&(Global_1673654.f_1047), iParam0);

	switch (iParam0)
	{
		case 5:
			if (iParam1 > -1)
				Global_1673654.f_170[iParam1] = 1;
			break;
	}

	return;
}

void func_126(int iParam0) // Position - 0x46A9
{
	Global_1675211.f_1163 = iParam0;
	return;
}

void func_127(int iParam0) // Position - 0x46BA
{
	if (func_133())
		return;

	if (!(Global_21066.f_1 == 1))
	{
		if (func_132(0))
			func_128(iParam0);
	
		unk_0x0B0C9A0F9AAEB7F0(&Global_8920, 2);
	}

	return;
}

void func_128(int iParam0) // Position - 0x46ED
{
	if (func_133())
		return;

	if (Global_21284)
		if (func_131())
			func_130(true, true);
		else
			func_130(false, false);

	if (Global_21066.f_1 == 10 || Global_21066.f_1 == 9)
		unk_0x0B0C9A0F9AAEB7F0(&Global_8920, 16);

	if (unk_0xE87F28FD4128D063())
		unk_0x0F15249D24BC5ADA(0);

	Global_22425 = 5;

	if (iParam0 == 1)
		unk_0x0B0C9A0F9AAEB7F0(&Global_8919, 30);
	else
		unk_0x8744D2E3FC95740E(&Global_8919, 30);

	if (!func_129())
		Global_21066.f_1 = 3;

	return;
}

BOOL func_129() // Position - 0x4777
{
	if (Global_21066.f_1 == 1 || Global_21066.f_1 == 0)
		return true;

	return false;
}

void func_130(BOOL bParam0, BOOL bParam1) // Position - 0x479E
{
	if (bParam0)
	{
		if (func_132(0))
		{
			Global_21284 = true;
		
			if (bParam1)
				unk_0x78C4EBB0251847E2(&Global_21003);
		
			Global_20994 = { Global_21012[Global_21011 /*3*/] };
			unk_0x6B7EA0158D00C600(Global_20994);
		}
	}
	else if (Global_21284 == true)
	{
		Global_21284 = false;
		Global_20994 = { Global_21019[Global_21011 /*3*/] };
	
		if (bParam1)
			unk_0x6B7EA0158D00C600(Global_21003);
		else
			unk_0x6B7EA0158D00C600(Global_20994);
	}

	return;
}

BOOL func_131() // Position - 0x4812
{
	return IS_BIT_SET(Global_1957675, 5);
}

BOOL func_132(int iParam0) // Position - 0x4820
{
	if (iParam0 == 1)
		if (Global_21066.f_1 > 3)
			if (IS_BIT_SET(Global_8919, 14))
				return true;
			else
				return false;
		else
			return false;

	if (unk_0x486FF5D06E9659F1(joaat("cellphone_flashhand")) > 0)
		return true;

	if (Global_21066.f_1 > 3)
		return true;

	return false;
}

BOOL func_133() // Position - 0x4877
{
	return IS_BIT_SET(Global_1957675, 19);
}

void func_134() // Position - 0x4886
{
	unk_0x66EFB3D6110055C4(0, 20, 1);
	unk_0x66EFB3D6110055C4(2, 20, 1);
	unk_0x66EFB3D6110055C4(0, 19, 1);
	unk_0x66EFB3D6110055C4(2, 19, 1);
	unk_0x66EFB3D6110055C4(2, 27, 1);
	unk_0x66EFB3D6110055C4(2, 172, 1);
	unk_0x66EFB3D6110055C4(2, 173, 1);
	unk_0x66EFB3D6110055C4(2, 177, 1);
	func_123();
	func_135();
	func_127(0);
	func_122();
	func_121();
	return;
}

void func_135() // Position - 0x48E3
{
	unk_0x0B0C9A0F9AAEB7F0(&Global_8920, 4);
	return;
}

int func_136() // Position - 0x48F3
{
	return Global_1957673.f_1;
}

BOOL func_137() // Position - 0x4901
{
	if (func_138())
		return true;

	return false;
}

BOOL func_138() // Position - 0x4915
{
	return IS_BIT_SET(Global_1957673, 0);
}

BOOL _SHOULD_NETWORK_SCRIPT_TERMINATE() // Position - 0x4923
{
	if (Global_1575056 == false)
		if (!unk_0x76CD105BCAC6EB9F())
			return true;

	if (func_146())
		return true;

	if (Global_2699004)
		return true;

	if (func_145())
		return true;

	if (_DOES_EVENT_OF_TYPE_EXIST(159))
		if (!func_143())
			return true;

	if (_DOES_EVENT_OF_TYPE_EXIST(157))
		return true;

	if (!unk_0x261E3728EE56B3AC())
		return true;

	if (_GET_CURRENT_SESSION_TYPE_SCRIPT_HASH() != 0)
		if (unk_0x486FF5D06E9659F1(_GET_CURRENT_SESSION_TYPE_SCRIPT_HASH()) == 0)
			return true;

	return false;
}

int _GET_CURRENT_SESSION_TYPE_SCRIPT_HASH() // Position - 0x49A7
{
	switch (func_142())
	{
		case 0:
			return func_141();
	
		case 2:
			return joaat("creator");
	}

	return 0;
}

int func_141() // Position - 0x49DA
{
	switch (Global_2699111)
	{
		case 0:
			return joaat("freemode");
	
		default:
		
	}

	return joaat("freemode");
}

int func_142() // Position - 0x49FE
{
	return Global_33087;
}

BOOL func_143() // Position - 0x4A09
{
	return Global_2684718.f_700;
}

BOOL _DOES_EVENT_OF_TYPE_EXIST(int iParam0) // Position - 0x4A18
{
	if (unk_0x5E3ED023C0E7CC7C(1, iParam0))
		return true;

	return false;
}

BOOL func_145() // Position - 0x4A2F
{
	return Global_2696416;
}

BOOL func_146() // Position - 0x4A3B
{
	return Global_2684718.f_695;
}

void func_147() // Position - 0x4A4A
{
	SYSTEM::WAIT(0);
	return;
}

void func_148() // Position - 0x4A57
{
	func_150(0);
	func_149();
	return;
}

void func_149() // Position - 0x4A68
{
	unk_0xBBC29EBE6E1A48FA();
	return;
}

void func_150(int iParam0) // Position - 0x4A74
{
	func_12(&uLocal_385[iParam0 /*2*/], false);
	func_151();
	unk_0x633E3833FB96BCCB("MPHotwire");
	return;
}

void func_151() // Position - 0x4A93
{
	var unk;

	unk_0xDD7F149F8BAF1260(&Global_1957673, &unk, 2);
	return;
}

BOOL func_152(var uParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6, var uParam7, var uParam8, var uParam9, var uParam10, var uParam11, var uParam12, var uParam13, var uParam14, var uParam15, var uParam16, var uParam17, var uParam18, var uParam19, var uParam20) // Position - 0x4AA6
{
	unk_0x8744D2E3FC95740E(&Global_1957673, 0);
	func_156(func_157(uParam0), uParam0);
	_NETWORK_ENSURE_SCRIPT_IS_NETWORKED(0, -1, false);
	unk_0x6F4A865F87C7A3AD(&uLocal_369, 1, 0);
	unk_0x7B13DC83218D9AF5(&uLocal_336, 33, 0);

	if (!_NETWORK_WAIT_FOR_HOST_BROADCAST_DATA())
		return false;

	unk_0xAECC5FA98C879D67(0);
	unk_0xD0D00ED689D6CA81("MPHotwire", 0);
	unk_0xF2CB0224D3BE0B42("MC_PLAY", 9);
	return true;
}

BOOL _NETWORK_WAIT_FOR_HOST_BROADCAST_DATA() // Position - 0x4B05
{
	int num;

	num = 0;

	while (true)
	{
		num = num + 1;
	
		if (!unk_0x76CD105BCAC6EB9F())
			return false;
	
		if (unk_0x37F4AB46DE999660())
			return true;
	
		if (func_146())
			return false;
	
		if (_DOES_EVENT_OF_TYPE_EXIST(157))
			return false;
	
		if (num >= 3600)
			return false;
	
		SYSTEM::WAIT(0);
	}

	return false;
}

int _NETWORK_ENSURE_SCRIPT_IS_NETWORKED(int iParam0, int iParam1, BOOL bNoTerminate) // Position - 0x4B5E
{
	int i;

	for (i = unk_0xDFF16B5B12604EFF(); i != 2; i = unk_0xDFF16B5B12604EFF())
	{
		if (i == 3 || i == 4 || i == 5 || i == 6)
			if (!bNoTerminate)
				func_149();
			else
				return 0;
	
		if (!func_155(false))
		{
			if (iParam0 == 0)
			{
				if (!unk_0x76CD105BCAC6EB9F())
					if (!bNoTerminate)
						func_149();
					else
						return 0;
			
				if (func_146())
					if (!bNoTerminate)
						func_149();
					else
						return 0;
			
				if (_DOES_EVENT_OF_TYPE_EXIST(157))
					if (!bNoTerminate)
						func_149();
					else
						return 0;
			}
			else if (!unk_0xBF52D447C795492B())
			{
				if (!bNoTerminate)
					func_149();
				else
					return 0;
			}
		}
	
		SYSTEM::WAIT(0);
	}

	if (iParam1 > -1)
		Global_1574667 = i;

	if (iParam0 == 0)
		if (!unk_0x76CD105BCAC6EB9F())
			if (!bNoTerminate)
				func_149();
			else
				return 0;
	else if (!unk_0xBF52D447C795492B())
		if (!bNoTerminate)
			func_149();
		else
			return 0;

	return 1;
}

BOOL func_155(BOOL bParam0) // Position - 0x4C74
{
	bParam0;
	return Global_1575056;
}

void func_156(int iParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6, var uParam7, var uParam8, var uParam9, var uParam10, var uParam11, var uParam12, var uParam13, var uParam14, var uParam15, var uParam16, var uParam17, var uParam18, var uParam19, var uParam20, var uParam21) // Position - 0x4C85
{
	if (!unk_0x76CD105BCAC6EB9F())
		func_149();

	unk_0xDB2434E51017FFCC(iParam0, 0, uParam1.f_16);
	return;
}

int func_157(int iParam0) // Position - 0x4CA4
{
	switch (iParam0)
	{
		case 3:
			return 2;
	
		case 1:
			return 32;
	
		case 32:
			return 32;
	
		case 33:
			return 32;
	
		case 34:
			return 32;
	
		case 35:
			return 32;
	
		case 36:
			return 32;
	
		case 37:
			return 32;
	
		case 38:
			return 32;
	
		case 39:
			return 32;
	
		case 40:
			return 32;
	
		case 41:
			return 32;
	
		case 42:
			return 32;
	
		case 43:
			return 32;
	
		case 44:
			return 32;
	
		case 45:
			return 32;
	
		case 46:
			return 32;
	
		case 47:
			return 32;
	
		case 48:
			return 32;
	
		case 49:
			return 32;
	
		case 50:
			return 4;
	
		case 51:
			return 32;
	
		case 52:
			return 32;
	
		case 53:
			return 32;
	
		case 54:
			return 32;
	
		case 55:
			return 32;
	
		case 56:
			return 32;
	
		case 57:
			return 32;
	
		case 58:
			return 32;
	
		case 59:
			return 32;
	
		case 60:
			return 32;
	
		case 61:
			return 32;
	
		case 62:
			return 32;
	
		case 63:
			return 32;
	
		case 64:
			return 4;
	
		case 65:
			return 32;
	
		case 66:
			return 4;
	
		case 67:
			return 4;
	
		case 68:
			return 32;
	
		case 69:
			return 32;
	
		case 70:
			return 4;
	
		case 71:
			return 32;
	
		case 72:
			return 32;
	
		case 73:
		case 74:
			return 4;
	
		case 75:
			return 32;
	
		case 76:
			return 32;
	
		case 77:
			return 32;
	
		case 78:
			return 32;
	
		case 79:
			return 32;
	
		case 80:
			return 32;
	
		case 81:
			return 32;
	
		case 82:
			return 32;
	
		case 84:
			return 32;
	
		case 83:
			return 32;
	
		case 85:
			return 32;
	
		case 86:
			return 8;
	
		case 87:
			return 32;
	
		case 88:
			return 32;
	
		case 89:
			return 32;
	
		case 90:
			return 32;
	
		case 91:
			return 8;
	
		case 92:
			return 32;
	
		case 93:
			return 8;
	
		case 94:
			return 8;
	
		case 102:
			return 8;
	
		case 95:
			return 8;
	
		case 96:
			return 32;
	
		case 97:
			return 32;
	
		case 98:
			return 32;
	
		case 99:
			return 8;
	
		case 100:
			return 32;
	
		case 101:
			return 32;
	
		case 103:
			return 32;
	
		case 104:
			return 32;
	
		case 105:
			return 32;
	
		case 106:
			return 8;
	
		case 107:
			return 8;
	
		case 108:
			return 8;
	
		case 109:
			return 8;
	
		case 110:
			return 8;
	
		case 111:
			return 8;
	
		case 112:
			return 8;
	
		case 113:
			return 8;
	
		case 114:
			return 32;
	
		case 115:
			return 8;
	
		case 116:
			return 8;
	
		case 117:
			return 8;
	
		case 118:
			return 8;
	
		case 119:
			return 32;
	
		case 120:
			return 32;
	
		case 121:
			return 32;
	
		case 122:
			return 32;
	
		case 123:
			return 8;
	
		case 124:
			return 8;
	
		case 125:
			return 8;
	
		case 126:
			return 8;
	
		case 12:
			return 32;
	
		case 4:
			return 16;
	
		case 13:
			return 32;
	
		case 5:
			return 16;
	
		case 6:
			return 2;
	
		case 8:
			return 2;
	
		case 9:
			return 2;
	
		case 7:
			return 16;
	
		case 10:
			return 2;
	
		case 11:
			return 4;
	
		case 15:
			return 32;
	
		case 16:
			return 32;
	
		case 27:
			return 2;
	
		case 25:
			return 2;
	
		case 26:
			return 2;
	
		case 18:
			return 32;
	
		case 28:
			return 32;
	
		case 29:
			return 2;
	
		case 30:
			return 32;
	
		case 31:
			return 32;
	
		case 17:
			return 2;
	
		case 208:
			return 32;
	
		case 209:
			return 32;
	
		case 19:
			return 32;
	
		case 22:
			return 32;
	
		case 23:
			return 32;
	
		case 24:
			return 32;
	
		case 20:
			return 2;
	
		case 0:
			return 0;
	
		case 21:
			return 32;
	
		case 220:
			return 32;
	
		case 221:
			return 32;
	
		case 210:
			return 32;
	
		case 211:
			return 32;
	
		case 215:
			return 32;
	
		case 213:
			return 32;
	
		case 214:
			return 32;
	
		case 218:
			return 32;
	
		case 219:
			return 32;
	
		case 216:
			return 32;
	
		case 217:
			return 32;
	
		case 222:
			return 32;
	
		case 223:
			return 32;
	
		case 224:
			return 32;
	
		case 225:
			return 32;
	
		case 226:
			return 2;
	
		case 231:
			return 1;
	
		case 227:
			return 2;
	
		case 228:
			return 4;
	
		case 229:
			return 2;
	
		case 230:
			return 2;
	
		case 212:
			return 1;
	
		case 232:
			return 2;
	
		case 233:
		case 234:
		case 235:
		case 236:
		case 237:
		case 238:
			return 0;
	
		case 254:
			return 1;
	
		case 239:
			return 4;
	
		case 242:
			return 4;
	
		case 243:
			return 1;
	
		case 244:
			return 1;
	
		case 250:
			return 1;
	
		case 246:
			return 2;
	
		case 251:
			return 1;
	
		case 247:
			return 1;
	
		case 245:
			return 2;
	
		case 248:
			return 8;
	
		case 249:
			return 8;
	
		case 252:
			return 1;
	
		case 253:
			return 2;
	
		case 142:
			return 8;
	
		case 148:
			return 1;
	
		case 170:
			return 1;
	
		case 179:
			return 1;
	
		case 196:
			return 1;
	
		case 206:
			return 32;
	
		case 240:
			return 16;
	
		case 241:
			return 32;
	
		default:
		
	}

	switch (func_158(func_159(iParam0, true)))
	{
		case 0:
			return 8;
	
		case 1:
			return 32;
	
		case 2:
			return 32;
	
		default:
		
	}

	return 0;
}

int func_158(int iParam0) // Position - 0x5444
{
	switch (iParam0)
	{
		case 150:
			return 1;
	
		case 24:
			return 2;
	
		case 26:
			return 2;
	
		case 256:
			return 0;
	
		case 258:
			return 0;
	
		case 259:
			return 2;
	
		case 271:
			return 0;
	
		case 273:
			return 2;
	
		case 276:
			return 0;
	
		case 277:
			return 0;
	
		case 262:
			return 0;
	
		case 263:
			return 0;
	
		case 264:
			return 0;
	
		case 269:
			return 2;
	
		case 270:
			return 2;
	
		case 275:
			return 2;
	
		case 268:
			return 2;
	
		case 286:
			return 2;
	
		case 267:
			return 0;
	
		case 266:
			return 2;
	
		case 287:
			return 2;
	
		case 289:
			return 2;
	
		case 147:
			return 2;
	
		case 291:
			return 0;
	
		case 292:
			return 0;
	
		case 293:
			return 0;
	
		case 294:
			return 0;
	
		case 295:
			return 0;
	
		case 296:
			return 0;
	
		case 297:
			return 0;
	
		case 298:
			return 0;
	
		case 299:
			return 0;
	
		case 300:
			return 0;
	
		case 301:
			return 0;
	
		case 288:
			return 2;
	
		case 290:
			return 2;
	
		case 304:
			return 0;
	
		case 305:
			return 0;
	
		case 306:
			return 0;
	
		case 307:
			return 0;
	
		case 308:
			return 0;
	
		case 309:
			return 0;
	
		case 310:
			return 2;
	
		case 311:
			return 2;
	
		case 312:
			return 2;
	
		case 320:
			return 2;
	
		case 313:
			return 2;
	
		case 314:
			return 0;
	
		case 315:
			return 0;
	
		case 316:
			return 0;
	
		case 317:
			return 0;
	
		case 318:
			return 0;
	
		case 322:
			return 0;
	
		case 323:
			return 2;
	
		case 324:
			return 0;
	
		case 325:
			return 0;
	
		case 326:
			return 0;
	
		case 327:
			return 0;
	
		case 328:
			return 0;
	
		case 329:
			return 0;
	
		case 330:
			return 0;
	
		case 331:
			return 0;
	
		case 332:
			return 0;
	
		case 333:
			return 0;
	
		case 337:
			return 0;
	
		case 338:
			return 0;
	
		case 339:
			return 0;
	
		case 340:
			return 0;
	
		case 343:
			return 0;
	
		case 344:
			return 0;
	
		case 345:
			return 0;
	
		case 346:
			return 0;
	
		case 347:
			return 0;
	
		case 348:
			return 0;
	
		case 349:
			return 0;
	
		case 350:
			return 0;
	
		case 351:
			return 0;
	
		case 353:
			return 2;
	
		default:
		
	}

	return -1;
}

int func_159(int iParam0, BOOL bParam1) // Position - 0x5760
{
	switch (iParam0)
	{
		case 226:
			return 15;
	
		case 233:
			return 8;
	
		case 227:
			return 14;
	
		case 231:
			return 122;
	
		case 234:
			return 1;
	
		case 232:
			return 5;
	
		case 235:
			return 6;
	
		case 228:
			return 11;
	
		case 236:
			return 0;
	
		case 237:
			return 2;
	
		case 229:
			return 13;
	
		case 238:
			return 3;
	
		case 230:
			return 12;
	
		case 49:
			return 148;
	
		case 52:
			return 151;
	
		case 53:
			return 152;
	
		case 54:
			return 157;
	
		case 55:
			return 153;
	
		case 56:
			return 154;
	
		case 57:
			return 155;
	
		case 58:
			return 159;
	
		case 51:
			return 162;
	
		case 60:
			return 142;
	
		case 62:
			return 160;
	
		case 63:
			return 164;
	
		case 64:
			return 163;
	
		case 65:
			return 166;
	
		case 66:
			return 167;
	
		case 67:
			return 168;
	
		case 68:
			return 169;
	
		case 69:
			return 170;
	
		case 70:
			return 171;
	
		case 71:
			return 172;
	
		case 72:
			return 173;
	
		case 73:
			return 178;
	
		case 74:
			return 188;
	
		case 75:
			return 214;
	
		case 76:
			return 215;
	
		case 77:
			return 216;
	
		case 78:
			return 217;
	
		case 79:
			return 218;
	
		case 80:
			return 219;
	
		case 81:
			return 220;
	
		case 82:
			return 221;
	
		case 84:
			return 179;
	
		case 83:
			return 189;
	
		case 85:
			return 180;
	
		case 87:
			return 182;
	
		case 88:
			return 183;
	
		case 89:
			return 185;
	
		case 90:
			return 186;
	
		case 91:
			return 190;
	
		case 92:
			return 191;
	
		case 93:
			return 192;
	
		case 94:
			return 193;
	
		case 102:
			return 205;
	
		case 95:
			return 194;
	
		case 96:
			return 197;
	
		case 97:
			return 198;
	
		case 99:
			return 199;
	
		case 100:
			return 200;
	
		case 101:
			return 201;
	
		case 103:
			return 207;
	
		case 104:
			return 208;
	
		case 105:
			return 209;
	
		case 106:
			return 210;
	
		case 98:
			return 195;
	
		case 107:
			return 225;
	
		case 108:
			return 226;
	
		case 109:
			return 227;
	
		case 110:
			return 229;
	
		case 111:
			return 230;
	
		case 113:
			return 233;
	
		case 115:
			return 237;
	
		case 116:
			return 238;
	
		case 117:
			return 239;
	
		case 118:
			return 240;
	
		case 119:
			return 241;
	
		case 120:
			return 242;
	
		case 121:
			return 244;
	
		case 122:
			return 248;
	
		case 123:
			return 249;
	
		case 124:
			return 250;
	
		case 125:
			return 243;
	
		case 126:
			return 158;
	
		case 86:
			return 181;
	
		case 127:
			return 150;
	
		case 128:
			return 24;
	
		case 129:
			return 26;
	
		case 130:
			return 256;
	
		case 131:
			return 258;
	
		case 133:
			return 259;
	
		case 134:
			return 271;
	
		case 135:
			return 273;
	
		case 136:
			return 276;
	
		case 137:
			return 277;
	
		case 138:
			return 262;
	
		case 139:
			return 263;
	
		case 140:
			return 264;
	
		case 141:
			return 268;
	
		case 143:
			return 269;
	
		case 144:
			return 270;
	
		case 145:
			return 275;
	
		case 146:
			return 286;
	
		case 148:
			return 267;
	
		case 147:
			return 266;
	
		case 151:
			return 147;
	
		case 149:
			return 287;
	
		case 152:
			return 291;
	
		case 153:
			return 292;
	
		case 154:
			return 293;
	
		case 155:
			return 294;
	
		case 156:
			return 295;
	
		case 157:
			return 296;
	
		case 158:
			return 297;
	
		case 159:
			return 298;
	
		case 160:
			return 299;
	
		case 161:
			return 300;
	
		case 162:
			return 301;
	
		case 163:
			return 288;
	
		case 164:
			return 290;
	
		case 165:
			return 304;
	
		case 166:
			return 305;
	
		case 167:
			return 306;
	
		case 168:
			return 307;
	
		case 169:
			return 308;
	
		case 170:
			return 309;
	
		case 171:
			return 310;
	
		case 172:
			return 311;
	
		case 173:
			return 312;
	
		case 174:
			return 313;
	
		case 175:
			return 314;
	
		case 176:
			return 315;
	
		case 177:
			return 316;
	
		case 178:
			return 317;
	
		case 179:
			return 318;
	
		case 180:
			return 320;
	
		case 181:
			return 322;
	
		case 182:
			return 323;
	
		case 183:
			return 324;
	
		case 184:
			return 325;
	
		case 185:
			return 326;
	
		case 186:
			return 327;
	
		case 187:
			return 328;
	
		case 188:
			return 329;
	
		case 189:
			return 330;
	
		case 190:
			return 331;
	
		case 191:
			return 332;
	
		case 192:
			return 333;
	
		case 193:
			return 337;
	
		case 194:
			return 338;
	
		case 195:
			return 339;
	
		case 196:
			return 340;
	
		case 197:
			return 343;
	
		case 198:
			return 344;
	
		case 199:
			return 345;
	
		case 200:
			return 346;
	
		case 201:
			return 347;
	
		case 202:
			return 348;
	
		case 203:
			return 349;
	
		case 204:
			return 350;
	
		case 205:
			return 351;
	
		case 206:
			return 352;
	
		case 207:
			return 353;
	
		default:
		
	}

	bParam1;
	return 352;
}

