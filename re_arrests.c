#region Local Var
	var uLocal_0 = 0;
	var uLocal_1 = 0;
	int iLocal_2 = 0;
	int iLocal_3 = 0;
	int iLocal_4 = 0;
	int iLocal_5 = 0;
	int iLocal_6 = 0;
	int iLocal_7 = 0;
	int iLocal_8 = 0;
	int iLocal_9 = 0;
	int iLocal_10 = 0;
	int iLocal_11 = 0;
	float fLocal_12 = 0f;
	var uLocal_13 = 0;
	var uLocal_14 = 0;
	int iLocal_15 = 0;
	var uLocal_16 = 0;
	var uLocal_17 = 0;
	char* sLocal_18 = 0;
	float fLocal_19 = 0f;
	var uLocal_20 = 0;
	var uLocal_21 = 0;
	var uLocal_22 = 0;
	float fLocal_23 = 0f;
	float fLocal_24 = 0f;
	var uLocal_25 = 0;
	int iLocal_26 = 0;
	var uLocal_27 = 0;
	var uLocal_28 = 0;
	float fLocal_29 = 0f;
	float fLocal_30 = 0f;
	float fLocal_31 = 0f;
	var uLocal_32 = 0;
	var uLocal_33 = 0;
	var uLocal_34 = 0;
	var uLocal_35 = 0;
	var uLocal_36 = 0;
	int iLocal_37 = 0;
	int iLocal_38 = 0;
	int iLocal_39 = 0;
	int iLocal_40 = 0;
	var uLocal_41 = 0;
	var uLocal_42 = 0;
	var uLocal_43 = 0;
	var uLocal_44 = 0;
	var uLocal_45 = 0;
	int iLocal_46 = 0;
	int iLocal_47 = 0;
	int iLocal_48 = 0;
	int iLocal_49 = 0;
	int iLocal_50 = 0;
	int iLocal_51 = 0;
	var uLocal_52 = 0;
	var uLocal_53 = 0;
	var uLocal_54 = 0;
	var uLocal_55 = 0;
	int iLocal_56 = 0;
	int iLocal_57 = 0;
	var uLocal_58 = 0;
	BOOL bLocal_59 = 0;
	var uLocal_60 = 0;
	BOOL bLocal_61 = 0;
	BOOL bLocal_62 = 0;
	var uLocal_63 = 0;
	var uLocal_64 = 0;
	var uLocal_65 = 0;
	var uLocal_66 = 0;
	var uLocal_67 = 0;
	var uLocal_68 = 0;
	var uLocal_69 = 0;
	var uLocal_70 = 0;
	var uLocal_71 = 0;
	BOOL bLocal_72 = 0;
	BOOL bLocal_73 = 0;
	BOOL bLocal_74 = 0;
	BOOL bLocal_75 = 0;
	var uLocal_76 = 16;
	var uLocal_77 = 0;
	var uLocal_78 = 0;
	var uLocal_79 = 0;
	var uLocal_80 = 0;
	var uLocal_81 = 0;
	var uLocal_82 = 0;
	var uLocal_83 = 0;
	var uLocal_84 = 0;
	var uLocal_85 = 0;
	var uLocal_86 = 0;
	var uLocal_87 = 0;
	var uLocal_88 = 0;
	var uLocal_89 = 0;
	var uLocal_90 = 0;
	var uLocal_91 = 0;
	var uLocal_92 = 0;
	var uLocal_93 = 0;
	var uLocal_94 = 0;
	var uLocal_95 = 0;
	var uLocal_96 = 0;
	var uLocal_97 = 0;
	var uLocal_98 = 0;
	var uLocal_99 = 0;
	var uLocal_100 = 0;
	var uLocal_101 = 0;
	var uLocal_102 = 0;
	var uLocal_103 = 0;
	var uLocal_104 = 0;
	var uLocal_105 = 0;
	var uLocal_106 = 0;
	var uLocal_107 = 0;
	var uLocal_108 = 0;
	var uLocal_109 = 0;
	var uLocal_110 = 0;
	var uLocal_111 = 0;
	var uLocal_112 = 0;
	var uLocal_113 = 0;
	var uLocal_114 = 0;
	var uLocal_115 = 0;
	var uLocal_116 = 0;
	var uLocal_117 = 0;
	var uLocal_118 = 0;
	var uLocal_119 = 0;
	var uLocal_120 = 0;
	var uLocal_121 = 0;
	var uLocal_122 = 0;
	var uLocal_123 = 0;
	var uLocal_124 = 0;
	var uLocal_125 = 0;
	var uLocal_126 = 0;
	var uLocal_127 = 0;
	var uLocal_128 = 0;
	var uLocal_129 = 0;
	var uLocal_130 = 0;
	var uLocal_131 = 0;
	var uLocal_132 = 0;
	var uLocal_133 = 0;
	var uLocal_134 = 0;
	var uLocal_135 = 0;
	var uLocal_136 = 0;
	var uLocal_137 = 0;
	var uLocal_138 = 0;
	var uLocal_139 = 0;
	var uLocal_140 = 0;
	var uLocal_141 = 0;
	var uLocal_142 = 0;
	var uLocal_143 = 0;
	var uLocal_144 = 0;
	var uLocal_145 = 0;
	var uLocal_146 = 0;
	var uLocal_147 = 0;
	var uLocal_148 = 0;
	var uLocal_149 = 0;
	var uLocal_150 = 0;
	var uLocal_151 = 0;
	var uLocal_152 = 0;
	var uLocal_153 = 0;
	var uLocal_154 = 0;
	var uLocal_155 = 0;
	var uLocal_156 = 0;
	var uLocal_157 = 0;
	var uLocal_158 = 0;
	var uLocal_159 = 0;
	var uLocal_160 = 0;
	var uLocal_161 = 0;
	var uLocal_162 = 0;
	var uLocal_163 = 0;
	var uLocal_164 = 0;
	var uLocal_165 = 0;
	var uLocal_166 = 0;
	var uLocal_167 = 0;
	var uLocal_168 = 0;
	var uLocal_169 = 0;
	var uLocal_170 = 0;
	var uLocal_171 = 0;
	var uLocal_172 = 0;
	var uLocal_173 = 0;
	var uLocal_174 = 0;
	var uLocal_175 = 0;
	var uLocal_176 = 0;
	var uLocal_177 = 0;
	var uLocal_178 = 0;
	var uLocal_179 = 0;
	var uLocal_180 = 0;
	var uLocal_181 = 0;
	var uLocal_182 = 0;
	var uLocal_183 = 0;
	var uLocal_184 = 0;
	var uLocal_185 = 0;
	var uLocal_186 = 0;
	var uLocal_187 = 0;
	var uLocal_188 = 0;
	var uLocal_189 = 0;
	var uLocal_190 = 0;
	var uLocal_191 = 0;
	var uLocal_192 = 0;
	var uLocal_193 = 0;
	var uLocal_194 = 0;
	var uLocal_195 = 0;
	var uLocal_196 = 0;
	var uLocal_197 = 0;
	var uLocal_198 = 0;
	var uLocal_199 = 0;
	var uLocal_200 = 0;
	var uLocal_201 = 0;
	var uLocal_202 = 0;
	var uLocal_203 = 0;
	var uLocal_204 = 0;
	var uLocal_205 = 0;
	var uLocal_206 = 0;
	var uLocal_207 = 0;
	var uLocal_208 = 0;
	var uLocal_209 = 0;
	var uLocal_210 = 0;
	var uLocal_211 = 0;
	var uLocal_212 = 0;
	var uLocal_213 = 0;
	var uLocal_214 = 0;
	var uLocal_215 = 0;
	var uLocal_216 = 0;
	var uLocal_217 = 0;
	var uLocal_218 = 0;
	var uLocal_219 = 0;
	var uLocal_220 = 0;
	var uLocal_221 = 0;
	var uLocal_222 = 0;
	var uLocal_223 = 0;
	var uLocal_224 = 0;
	var uLocal_225 = 0;
	var uLocal_226 = 0;
	var uLocal_227 = 0;
	var uLocal_228 = 0;
	var uLocal_229 = 0;
	var uLocal_230 = 0;
	var uLocal_231 = 0;
	var uLocal_232 = 0;
	var uLocal_233 = 0;
	var uLocal_234 = 0;
	var uLocal_235 = 0;
	var uLocal_236 = 0;
	var uLocal_237 = 0;
	var uLocal_238 = 0;
	var uLocal_239 = 0;
	var uLocal_240 = 0;
	int iLocal_241 = 0;
	int iLocal_242 = 0;
	int iLocal_243 = 0;
	int iLocal_244 = 0;
	int iLocal_245 = 0;
	int iLocal_246 = 0;
	int iLocal_247 = 0;
	float fLocal_248 = 0f;
	var uLocal_249 = 0;
	var uLocal_250 = 0;
	var uLocal_251 = 0;
	int iLocal_252 = 0;
	int iLocal_253 = 0;
	char* sLocal_254 = 0;
	char* sLocal_255 = 0;
	char* sLocal_256 = 0;
	char* sLocal_257 = 0;
	char* sLocal_258 = 0;
	BOOL bLocal_259 = 0;
	BOOL bLocal_260 = 0;
	BOOL bLocal_261 = 0;
	int iLocal_262 = 0;
	char* sLocal_263 = 0;
	char* sLocal_264 = 0;
	char* sLocal_265 = 0;
	char* sLocal_266 = 0;
	char* sLocal_267 = 0;
	char* sLocal_268 = 0;
	char* sLocal_269 = 0;
	char* sLocal_270 = 0;
	char* sLocal_271 = 0;
	char* sLocal_272 = 0;
	int iLocal_273 = 0;
	var uLocal_274 = 0;
	var uLocal_275 = 0;
	BOOL bLocal_276 = 0;
	var uLocal_277 = 0;
	var uLocal_278 = 0;
	var uLocal_279 = 0;
	var uLocal_280 = 0;
	var uLocal_281 = 0;
	var uLocal_282 = 0;
	int iLocal_283 = 0;
	int iLocal_284 = 0;
	BOOL bLocal_285 = 0;
	BOOL bLocal_286 = 0;
	BOOL bLocal_287 = 0;
	var uLocal_288 = 0;
	var uLocal_289 = 0;
	var uLocal_290 = 0;
	BOOL bLocal_291 = 0;
	BOOL bLocal_292 = 0;
	var uLocal_293 = 0;
	var uLocal_294 = 0;
	var uLocal_295 = 0;
	BOOL bLocal_296 = 0;
	var uScriptParam_0 = 0;
	var uScriptParam_1 = 5;
	var uScriptParam_2 = 0;
	var uScriptParam_3 = 0;
	var uScriptParam_4 = 0;
	var uScriptParam_5 = 0;
	var uScriptParam_6 = 0;
	var uScriptParam_7 = 0;
	var uScriptParam_8 = 0;
	var uScriptParam_9 = 0;
	var uScriptParam_10 = 0;
	var uScriptParam_11 = 0;
	var uScriptParam_12 = 0;
	var uScriptParam_13 = 0;
	var uScriptParam_14 = 0;
	var uScriptParam_15 = 0;
	var uScriptParam_16 = 0;
	var uScriptParam_17 = 5;
	var uScriptParam_18 = 0;
	var uScriptParam_19 = 0;
	var uScriptParam_20 = 0;
	var uScriptParam_21 = 0;
	var uScriptParam_22 = 0;
#endregion

void main() // Position - 0x0
{
	iLocal_2 = 1;
	iLocal_3 = 134;
	iLocal_4 = 134;
	iLocal_5 = 1;
	iLocal_6 = 1;
	iLocal_7 = 1;
	iLocal_8 = 134;
	iLocal_9 = 1;
	iLocal_10 = 12;
	iLocal_11 = 12;
	fLocal_12 = 0.001f;
	iLocal_15 = -1;
	sLocal_18 = "NULL";
	fLocal_19 = 0f;
	fLocal_23 = -0.0375f;
	fLocal_24 = 0.17f;
	iLocal_26 = 3;
	fLocal_29 = 80f;
	fLocal_30 = 140f;
	fLocal_31 = 180f;
	iLocal_37 = 1;
	iLocal_38 = 65;
	iLocal_39 = 49;
	iLocal_40 = 64;
	iLocal_242 = 8000;
	iLocal_245 = 8;
	iLocal_247 = 1;
	sLocal_255 = "S_M_Y_RANGER_01_WHITE_FULL_01";
	sLocal_256 = "RANDOM@ARRESTS";
	sLocal_257 = "RANDOM@ARRESTS@BUSTED";
	bLocal_261 = 1;
	uLocal_63 = { uScriptParam_0.f_1[0 /*3*/] };

	if (unk_0x96CFB880BAC634CE(11))
	{
		bLocal_74 = true;
		func_208();
		func_178();
	}

	if (SYSTEM::VDIST(2411.32f, 4958.76f, 45.19f, uLocal_63) < 10f)
		iLocal_51 = 1;
	else
		iLocal_51 = 2;

	if (func_135(uLocal_63, 15, iLocal_51, false, false))
		func_130(15);
	else
		unk_0xBBC29EBE6E1A48FA();

	while (true)
	{
		SYSTEM::WAIT(0);
	
		if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
			unk_0x9C931A1395CC1AE9(unk_0x259BE71D8A81D4FA());
	
		switch (iLocal_46)
		{
			case 0:
				if (unk_0x173751E886F8E9AB() && !func_129() && !func_116())
					if (func_113())
						iLocal_46 = 1;
				else
					func_178();
				break;
		
			case 1:
				if (func_109())
				{
					unk_0x5E08BBBF87BC4886(2551.0378f, 4708.6133f, 32.6775f, 2536.979f, 5022.1787f, 43.8519f, 300f, 0, 1, 1);
					unk_0x425BBE19F25A57AB(0f);
					func_98(true);
					iLocal_46 = 2;
				}
				else if (!func_97(200f))
				{
					func_178();
				}
				break;
		
			case 2:
				if (func_97(1128792064))
					func_1();
				else
					func_178();
				break;
		}
	}

	return;
}

void func_1() // Position - 0x1D1
{
	var unk;
	var unk2;
	float num;
	BOOL flag;
	float num2;

	unk_0x0B0C9A0F9AAEB7F0(&unk, 3);
	unk_0x0B0C9A0F9AAEB7F0(&unk, 4);
	flag = false;
	func_96();

	if (!unk_0x4FAFF4BCB7633475(uLocal_53))
		unk_0x0FB8E752BCC547A9(uLocal_53, 129, 1);

	if (_DOES_ENTITY_EXIST_AND_IS_ALIVE(uLocal_53))
		if (unk_0x11BF418D1AFD7970(uLocal_53, 0))
			bLocal_296 = true;

	if (!unk_0x4FAFF4BCB7633475(uLocal_52) && iLocal_47 != 2 && iLocal_47 != 1)
	{
		if (!unk_0x4FAFF4BCB7633475(uLocal_53))
		{
			switch (iLocal_50)
			{
				case 0:
					unk_0xBBB7593C7244A846("copChasesCriminal", 0.02f, 0.1f, 0f, 0, 0, 255, 255);
				
					if (SYSTEM::TIMERA() > 10)
						if (!bLocal_72)
							if (unk_0x9B5C1660CCDF7189(uLocal_52, joaat("SCRIPT_TASK_GO_TO_ENTITY")) == 7)
								unk_0xAB3658A740EED98E(uLocal_52, uLocal_53, -1, 1f, 3f, 1073741824, 0);
				
					if (SYSTEM::TIMERA() > 4000)
					{
						if (!bLocal_72)
						{
							if (unk_0x9B5C1660CCDF7189(uLocal_53, joaat("SCRIPT_TASK_SMART_FLEE_PED")) == 7)
							{
								if (unk_0x9B5C1660CCDF7189(uLocal_53, joaat("SCRIPT_TASK_PLAY_ANIM")) == 1)
									unk_0xE1C7B23ECC5080B5(uLocal_53, 2, 1);
							
								unk_0xD844F5E50DAB6FF7(uLocal_53, uLocal_52, 150f, -1, 0, 0);
							}
						}
					}
				
					num = unk_0xED977E2AE2CB16EE(unk_0xD1A6A821F5AC81DB(uLocal_52, 1), unk_0xD1A6A821F5AC81DB(uLocal_53, 1), 1);
				
					if (SYSTEM::TIMERA() > 8000 || num < 1.5f)
					{
						flag = func_91();
					
						if (flag == true)
							unk_0xAB3658A740EED98E(uLocal_52, uLocal_53, -1, 1f, 0.01f, 1073741824, 0);
					}
				
					if (SYSTEM::TIMERA() > 30000 && num < 10f)
						flag = true;
				
					if (SYSTEM::TIMERA() > 2000)
					{
						if (flag || bLocal_276 == true)
						{
							if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
							{
								_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PCUFF", 4, iLocal_252, 0, 0);
								bLocal_292 = unk_0x8BF5256C439DF778(uLocal_53) || unk_0x0B3FC0E7676E79BF(uLocal_53);
							
								if (!unk_0x66B90BA528CFEBCE(uLocal_52, joaat("WEAPON_PISTOL"), 0))
									unk_0xB41DEC3AAC1AA107(uLocal_52, joaat("WEAPON_PISTOL"), -1, 0, 1);
							
								unk_0x3C0F448853B71C92(uLocal_52, joaat("WEAPON_PISTOL"), 1);
								uLocal_66 = { unk_0xD1A6A821F5AC81DB(uLocal_53, 1) };
								unk_0x974022927CB47E68(uLocal_52);
								unk_0xB5396F1FB088FE38(&uLocal_71);
							
								if (!bLocal_292 && num > 2f)
									unk_0x6C4E9ADFB1521AAC(0, uLocal_66, uLocal_66.f_1, uLocal_66.f_2 + 4f, 1000, joaat("FIRING_PATTERN_SINGLE_SHOT"));
							
								unk_0xE67051907958B5EB(0, uLocal_53, -1, 2048, 2);
								unk_0x2433FB137DF847D2(0, uLocal_53, uLocal_53, 1f, 0, 3f, 1082130432, 1, 0, joaat("FIRING_PATTERN_FULL_AUTO"));
								unk_0x646DE293036A3499(0, uLocal_53, 5000, 0);
								unk_0x93C0674FC00824D0(uLocal_71);
								unk_0x4BD42B0527065BB6(uLocal_52, uLocal_71);
								unk_0xD0557B139A542F12(&uLocal_71);
								unk_0xE4DC7B3DD712372B(uLocal_52);
								unk_0x44FB298D6382876D(uLocal_52, 1);
								SYSTEM::SETTIMERA(0);
								iLocal_244 = unk_0x1DD05E817C89C737();
							
								if (bLocal_292)
								{
									unk_0x974022927CB47E68(uLocal_53);
									unk_0x08AABCACF8F4828E(uLocal_53);
									iLocal_50 = 4;
								}
								else
								{
									iLocal_50 = 2;
								}
							}
							else if (!bLocal_276)
							{
								unk_0xAB3658A740EED98E(uLocal_52, uLocal_53, -1, 1f, 0.01f, 1073741824, 0);
								bLocal_276 = true;
							}
						}
						else
						{
							func_74();
						}
					}
					else
					{
						unk_0xBBB7593C7244A846("copChasesCriminal TIMERA() < 2000", 0.02f, 0.1f, 0f, 0, 0, 255, 255);
					}
				
					if (unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
					{
						if (!unk_0x4FAFF4BCB7633475(uLocal_52) && !unk_0x1C2F771CDC87A3A5(unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0), 0))
						{
							if (unk_0x685171EED42BC4DF(unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0), uLocal_52))
							{
								unk_0x974022927CB47E68(uLocal_52);
								iLocal_50 = 1;
								unk_0x0FD8B5F4BB15CD71(uLocal_52, 500);
							}
						}
					}
					break;
			
				case 1:
					if (unk_0x9B5C1660CCDF7189(uLocal_52, joaat("SCRIPT_TASK_STAND_STILL")) == 7)
					{
						unk_0xAB3658A740EED98E(uLocal_52, uLocal_53, -1, 1f, 3f, 1073741824, 0);
						iLocal_50 = 0;
					}
					break;
			
				case 2:
					if (unk_0x1DD05E817C89C737() - iLocal_244 > 1000)
					{
						unk2 = { 0f, 0f, unk_0xCFC0C995455A6204(uLocal_53) * 3f };
						unk_0xA966E518B752B92A(uLocal_53, unk2, 2f, -1, 1f, 1024, 1193033728);
						iLocal_50 = 3;
					}
					break;
			
				case 3:
					if (unk_0x1DD05E817C89C737() - iLocal_244 > 4500)
						iLocal_50 = 4;
					break;
			
				case 4:
					bLocal_72 = true;
					unk_0xB5396F1FB088FE38(&uLocal_71);
					unk_0x10425721983AE158(0, sLocal_256, "idle_2_hands_up", 2f, -8f, -1, 0, 0, 0, 0, 0);
					unk_0x10425721983AE158(0, sLocal_256, "kneeling_arrest_idle", 8f, -8f, -1, 1, 0, 0, 0, 0);
					unk_0x93C0674FC00824D0(uLocal_71);
					unk_0x4BD42B0527065BB6(uLocal_53, uLocal_71);
					unk_0xD0557B139A542F12(&uLocal_71);
					unk_0xE4DC7B3DD712372B(uLocal_53);
					unk_0x44FB298D6382876D(uLocal_53, 1);
					unk_0x1834D30866818A23(uLocal_53, 1);
					iLocal_50 = 5;
					break;
			
				case 5:
					if (iLocal_48 == 3)
					{
						if (unk_0x9B5C1660CCDF7189(uLocal_52, joaat("SCRIPT_TASK_PERFORM_SEQUENCE")) == 7)
						{
							unk_0x974022927CB47E68(uLocal_52);
						
							if (!unk_0x66B90BA528CFEBCE(uLocal_52, joaat("WEAPON_PISTOL"), 0))
								unk_0xB41DEC3AAC1AA107(uLocal_52, joaat("WEAPON_PISTOL"), -1, 0, 1);
						
							unk_0x3C0F448853B71C92(uLocal_52, joaat("WEAPON_PISTOL"), 1);
							unk_0xB5396F1FB088FE38(&uLocal_71);
							unk_0x646DE293036A3499(0, uLocal_53, -1, 0);
							unk_0x93C0674FC00824D0(uLocal_71);
							unk_0x4BD42B0527065BB6(uLocal_52, uLocal_71);
							unk_0xD0557B139A542F12(&uLocal_71);
							unk_0x44FB298D6382876D(uLocal_52, 1);
							iLocal_50 = 6;
						}
					}
					break;
			
				case 6:
					if (unk_0xD9F5E1FEFD1329E4(uLocal_55, 0) && !unk_0x4FAFF4BCB7633475(uLocal_54))
					{
						unk_0x40891A689350468D(uLocal_55, 1);
					
						if (unk_0x9B5C1660CCDF7189(uLocal_52, joaat("SCRIPT_TASK_PLAY_ANIM")) == 7)
						{
							if (unk_0x9B5C1660CCDF7189(uLocal_54, joaat("SCRIPT_TASK_VEHICLE_MISSION")) == 7)
							{
								if (unk_0x848DE0A81098ECCB(uLocal_53, uLocal_55, 2, 0, 0))
								{
									if (!unk_0x4FAFF4BCB7633475(uLocal_53))
									{
										if (!unk_0xF4244288C3EF3306(uLocal_53, uLocal_55))
										{
											if (!unk_0x1B32E388988DD296(uLocal_53, 0))
											{
												if (unk_0x9B5C1660CCDF7189(uLocal_53, joaat("SCRIPT_TASK_ENTER_VEHICLE")) == 7)
												{
													unk_0xB5396F1FB088FE38(&uLocal_71);
													unk_0x10425721983AE158(0, sLocal_256, "kneeling_arrest_get_up", 8f, -2f, -1, 0, 0, 0, 0, 0);
													unk_0x93C0674FC00824D0(uLocal_71);
													unk_0x4BD42B0527065BB6(uLocal_53, uLocal_71);
													unk_0xD0557B139A542F12(&uLocal_71);
													unk_0xD414C47AFF81382A(0, uLocal_70, uLocal_69);
													unk_0xD414C47AFF81382A(0, uLocal_69, uLocal_70);
												}
											}
										}
									}
								
									if (!unk_0x4FAFF4BCB7633475(uLocal_52))
									{
										unk_0xB739D4CB4D58B663(uLocal_52);
									
										if (!unk_0x1B32E388988DD296(uLocal_52, 0))
										{
											if (unk_0xD9F5E1FEFD1329E4(uLocal_55, 0))
											{
												if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
												{
													if (_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_INCAR", 4, iLocal_252, 0, 0))
													{
														if (func_73(uLocal_52, uLocal_55) == 2)
															if (unk_0x3644984C9D7B57EF(uLocal_54, uLocal_53, 180f))
																uLocal_293 = { 2.978f, -0.75f, 1f };
															else
																uLocal_293 = { 2.978f, 0.75f, 1f };
														else if (unk_0x3644984C9D7B57EF(uLocal_54, uLocal_53, 180f))
															uLocal_293 = { -2.978f, -0.75f, 1f };
														else
															uLocal_293 = { -2.978f, 0.75f, 1f };
													
														unk_0xB5396F1FB088FE38(&uLocal_71);
														unk_0xA68F93198FAC44EC(0, unk_0x0D1381B6E0F3987D(uLocal_55, uLocal_293), uLocal_53, 0.75f, 0, 0.5f, 4f, 1, 0, 0, joaat("FIRING_PATTERN_FULL_AUTO"), 60000);
														unk_0x646DE293036A3499(0, uLocal_53, -1, 0);
														unk_0x93C0674FC00824D0(uLocal_71);
														unk_0x4BD42B0527065BB6(uLocal_52, uLocal_71);
														unk_0xD0557B139A542F12(&uLocal_71);
														unk_0x44FB298D6382876D(uLocal_52, 1);
														unk_0x4539850624F18A9E(uLocal_55, 0);
														iLocal_242 = 8000;
														SYSTEM::SETTIMERB(0);
														uLocal_275 = unk_0x1DD05E817C89C737();
														iLocal_50 = 7;
													}
												}
											}
										}
									}
								}
								else
								{
									func_72();
								}
							}
						}
					}
					break;
			
				case 7:
					if (!unk_0x4FAFF4BCB7633475(uLocal_52))
					{
						if (unk_0xFAA48325A90263BE(unk_0x4A8C381C258A124D(), uLocal_52, 35f, 35f, 35f, 0, 1, 0))
						{
							if (unk_0x1DD05E817C89C737() - uLocal_275 > 8000)
							{
								if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
								{
									unk_0xE67051907958B5EB(uLocal_52, unk_0x4A8C381C258A124D(), 6000, 2048, 2);
									_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PLEAVE", 4, iLocal_252, 0, 0);
									iLocal_242 = 8000;
									uLocal_275 = unk_0x1DD05E817C89C737();
								}
							}
						}
					
						if (unk_0xD9F5E1FEFD1329E4(uLocal_55, 0))
						{
							if (bLocal_62)
							{
								if (func_73(uLocal_52, uLocal_55) == 2)
									unk_0xEBA229B2E0BB05E0(uLocal_53, uLocal_55, -1, 2, 1f, 1, 0, 0);
								else
									unk_0xEBA229B2E0BB05E0(uLocal_53, uLocal_55, -1, 1, 1f, 1, 0, 0);
							
								iLocal_50 = 8;
							}
							else if (bLocal_61)
							{
								num2 = unk_0xED977E2AE2CB16EE(unk_0xD1A6A821F5AC81DB(uLocal_53, 1), unk_0xD1A6A821F5AC81DB(uLocal_55, 1), 1);
							
								if (num2 < 4f && iLocal_283 == 0)
								{
									unk_0x35CBF41D628A3871(uLocal_53, uLocal_55, 60000, 0.5f, uLocal_293, uLocal_293.f_1, 1f, 1);
									iLocal_283 = 1;
								}
								else if (num2 < 3f)
								{
									unk_0x974022927CB47E68(uLocal_53);
									bLocal_62 = true;
								}
							}
							else if (!unk_0x13CCB1AD131C1082(uLocal_53, sLocal_256, "kneeling_arrest_get_up", 3))
							{
								if (func_73(uLocal_52, uLocal_55) == 2)
									uLocal_293 = { 1.578f, -0.5f, 1f };
								else
									uLocal_293 = { -1.578f, -0.5f, 1f };
							
								unk_0x35CBF41D628A3871(uLocal_53, uLocal_55, 60000, 0.5f, uLocal_293, uLocal_293.f_1, 1f, 1);
							
								if (bLocal_296)
									unk_0x4669032A1DFBB449(uLocal_53, "MOVE_M@BAIL_BOND_TAZERED", 1048576000);
								else
									unk_0x4669032A1DFBB449(uLocal_53, "MOVE_M@BAIL_BOND_NOT_TAZERED", 1048576000);
							
								bLocal_61 = true;
							}
						}
					}
					break;
			
				case 8:
					if (unk_0xD9F5E1FEFD1329E4(uLocal_55, 0))
					{
						if (!unk_0x4FAFF4BCB7633475(uLocal_54))
						{
							if (unk_0xF4244288C3EF3306(uLocal_53, uLocal_55))
							{
								if (unk_0xC450B06E5AAA0985(iLocal_56))
									unk_0xFE54B8568B2ABD12(&iLocal_56);
							
								if (unk_0xC450B06E5AAA0985(iLocal_57))
									unk_0xFE54B8568B2ABD12(&iLocal_57);
							
								if (unk_0xF4244288C3EF3306(uLocal_52, uLocal_55))
								{
									unk_0x7C8E9DE09D4AD3FF(uLocal_54, uLocal_55, 10f, 786603);
									unk_0x44FB298D6382876D(uLocal_54, 1);
									unk_0xAAA71DD7E9059338(uLocal_52, 0);
									func_178();
								}
								else if (unk_0x9B5C1660CCDF7189(uLocal_52, joaat("SCRIPT_TASK_ENTER_VEHICLE")) == 7)
								{
									unk_0x974022927CB47E68(uLocal_52);
									unk_0xEBA229B2E0BB05E0(uLocal_52, uLocal_55, &_CAN_ENTER_FREEROAM_STATE, 0, 1f, 1, 0, 0);
									unk_0x44FB298D6382876D(uLocal_52, 1);
								}
							}
						}
					}
					break;
			}
		}
	}

	func_58();
	unk_0x4FAFF4BCB7633475(uLocal_53);

	if (unk_0x4FAFF4BCB7633475(uLocal_52))
	{
		if (!unk_0x4FAFF4BCB7633475(uLocal_53))
		{
			if (iLocal_47 == 2)
			{
				func_12();
			}
			else
			{
				if (iLocal_47 == 1 || iLocal_47 == 3)
					if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
						if (unk_0xED977E2AE2CB16EE(unk_0xD1A6A821F5AC81DB(uLocal_53, 1), unk_0xD1A6A821F5AC81DB(unk_0x4A8C381C258A124D(), 1), 1) < 20f)
							_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_272, 4, iLocal_252, 0, 0);
			
				if (unk_0x13CCB1AD131C1082(uLocal_53, sLocal_256, "kneeling_arrest_idle", 3))
				{
					unk_0xB5396F1FB088FE38(&uLocal_71);
					unk_0x10425721983AE158(0, sLocal_256, "kneeling_arrest_get_up", 2f, -2f, -1, 0, 0, 0, 0, 0);
					unk_0xD844F5E50DAB6FF7(0, unk_0x4A8C381C258A124D(), 150f, -1, 0, 0);
					unk_0x93C0674FC00824D0(uLocal_71);
					unk_0x4BD42B0527065BB6(uLocal_53, uLocal_71);
					unk_0xD0557B139A542F12(&uLocal_71);
				}
				else
				{
					unk_0xD844F5E50DAB6FF7(uLocal_53, unk_0x4A8C381C258A124D(), 150f, -1, 0, 0);
					unk_0x44FB298D6382876D(uLocal_53, 1);
				}
			
				SYSTEM::WAIT(0);
			
				if (iLocal_47 == 0)
					iLocal_47 = 2;
			
				func_178();
			}
		}
	}

	if (unk_0x4FAFF4BCB7633475(uLocal_53))
		if (!bLocal_59)
			if (func_11())
				func_3(false);

	if (iLocal_50 == 5 || iLocal_50 == 6)
		if (!bLocal_59)
			if (iLocal_47 == 3)
				if (func_11())
					func_3(false);
			else
				func_3(true);

	func_2();
	return;
}

void func_2() // Position - 0xBEE
{
	int num;
	var unk;

	unk = { 40f, 40f, 20f };

	if (unk_0x1DD05E817C89C737() > iLocal_253 + 1000)
	{
		if (!unk_0x4FAFF4BCB7633475(uLocal_52))
			if (!unk_0xFAA48325A90263BE(uLocal_52, unk_0x4A8C381C258A124D(), unk, 0, 1, 0))
				num = num + 1;
		else
			num = num + 1;
	
		if (!unk_0x4FAFF4BCB7633475(uLocal_53))
			if (!unk_0xFAA48325A90263BE(uLocal_53, unk_0x4A8C381C258A124D(), unk, 0, 1, 0))
				num = num + 1;
		else
			num = num + 1;
	
		if (num == 2)
			iLocal_252 = 1;
		else
			iLocal_252 = 0;
	
		iLocal_253 = unk_0x1DD05E817C89C737();
	}

	return;
}

void func_3(BOOL bParam0) // Position - 0xC86
{
	char* str;
	char* str2;
	char* str3;

	if (bParam0)
	{
		str = "radio_enter";
		str2 = "radio_chatter";
		str3 = "radio_exit";
	}
	else
	{
		str = "generic_radio_enter";
		str2 = "generic_radio_chatter";
		str3 = "generic_radio_exit";
	}

	if (!unk_0x4FAFF4BCB7633475(uLocal_52))
	{
		switch (iLocal_48)
		{
			case 0:
				if (iLocal_241 == 3 || iLocal_50 == 5 || iLocal_50 == 6)
				{
					if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
					{
						unk_0x974022927CB47E68(uLocal_52);
						unk_0xB5396F1FB088FE38(&uLocal_71);
						unk_0xE237FA90A8AFEE59(0, unk_0xD1A6A821F5AC81DB(uLocal_53, 0), 10000, 0, 2);
						unk_0x10425721983AE158(0, sLocal_256, str, 8f, -4f, -1, 0, 0, 0, 0, 0);
						unk_0x10425721983AE158(0, sLocal_256, str2, 16f, -8f, -1, 1, 0, 0, 0, 0);
						unk_0x93C0674FC00824D0(uLocal_71);
						unk_0x4BD42B0527065BB6(uLocal_52, uLocal_71);
						unk_0xD0557B139A542F12(&uLocal_71);
						iLocal_48 = 1;
					}
				}
				break;
		
			case 1:
				if (unk_0x13CCB1AD131C1082(uLocal_52, sLocal_256, str2, 3))
					if (!unk_0x4FAFF4BCB7633475(uLocal_53))
						if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
							if (_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PRAD1", 4, iLocal_252, 0, 0))
								iLocal_48 = 2;
					else if (func_10())
						if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
							if (_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PRAD2", 4, iLocal_252, 0, 0))
								iLocal_48 = 2;
				break;
		
			case 2:
				if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
				{
					if (func_10())
					{
						if (unk_0xD9F5E1FEFD1329E4(uLocal_55, 0))
						{
							unk_0xBC5009F300C00558(uLocal_55, 0);
							unk_0x4539850624F18A9E(uLocal_55, 1);
						}
					
						unk_0x974022927CB47E68(uLocal_52);
						unk_0xB5396F1FB088FE38(&uLocal_71);
						unk_0x10425721983AE158(0, sLocal_256, str3, 8f, -1.5f, -1, 0, 0, 0, 0, 0);
						unk_0x93C0674FC00824D0(uLocal_71);
						unk_0x4BD42B0527065BB6(uLocal_52, uLocal_71);
						unk_0xD0557B139A542F12(&uLocal_71);
						unk_0xD8ED11B32DF72E0B(uLocal_52, 0, 0);
						func_8();
						iLocal_48 = 3;
					}
				}
				break;
		
			case 3:
				if (!bLocal_72)
					func_7();
			
				if (unk_0xFC8BFE4B41177C22(uLocal_53) || !unk_0x1C2F771CDC87A3A5(uLocal_53, 0))
					if (unk_0x685171EED42BC4DF(unk_0x4A8C381C258A124D(), uLocal_53))
						if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
							_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PWTF", 4, iLocal_252, 0, 0);
			
				if (unk_0xFAA48325A90263BE(unk_0x4A8C381C258A124D(), uLocal_52, 35f, 35f, 35f, 0, 1, 0))
				{
					unk_0xF9F66C74E3167674(uLocal_52, 1, unk_0x4A8C381C258A124D(), 31086, 0f, 0f, 0f, 0, -1, -1);
					unk_0xF9F66C74E3167674(unk_0x4A8C381C258A124D(), 1, uLocal_52, 31086, 0f, 0f, 0f, 0, -1, -1);
				
					if (SYSTEM::TIMERB() > iLocal_242 || bLocal_260)
					{
						if (iLocal_47 == 3 || iLocal_47 == 0)
						{
							if (!unk_0x4FAFF4BCB7633475(uLocal_53))
							{
								if (iLocal_242 == 8000 || bLocal_260)
								{
									if (bLocal_259)
										func_6();
									else
										func_4();
								}
								else if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
								{
									unk_0xE67051907958B5EB(uLocal_52, unk_0x4A8C381C258A124D(), 500, 2048, 2);
									_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PLEAVE", 4, iLocal_252, 0, 0);
									iLocal_242 = 8000;
								}
							}
							else if (iLocal_242 == 8000)
							{
								if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
								{
									_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PRA2", 4, iLocal_252, 0, 0);
									iLocal_242 = &_CAN_ENTER_FREEROAM_STATE;
								}
							}
							else if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
							{
								_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PRA2", 4, iLocal_252, 0, 0);
								iLocal_242 = 8000;
							}
						
							SYSTEM::SETTIMERB(0);
						}
						else
						{
							if (unk_0x4FAFF4BCB7633475(uLocal_53))
							{
								if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
								{
									_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PRA2", 4, iLocal_252, 0, 0);
									iLocal_242 = 30000;
								}
							}
							else if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
							{
								unk_0xE67051907958B5EB(uLocal_52, unk_0x4A8C381C258A124D(), 6000, 2048, 2);
								_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PLEAVE", 4, iLocal_252, 0, 0);
								iLocal_242 = unk_0xC5935DFB3F39785A(8, 22);
								iLocal_242 = iLocal_242 * 30000;
							}
						
							SYSTEM::SETTIMERB(0);
						}
					}
				}
			
				if (iLocal_50 != 8)
					if (unk_0xD9F5E1FEFD1329E4(uLocal_55, 0))
						if (!unk_0x4FAFF4BCB7633475(uLocal_53))
							if (!unk_0xCECDBB848D53DEB2(uLocal_53, uLocal_55, 0))
								func_72();
					else
						func_72();
				break;
		}
	}

	return;
}

void func_4() // Position - 0x1056
{
	if (!bLocal_260)
	{
		switch (unk_0xC5935DFB3F39785A(0, 3))
		{
			case 0:
				sLocal_258 = "idle_a";
				break;
		
			case 1:
				sLocal_258 = "idle_b";
				break;
		
			case 2:
				sLocal_258 = "idle_c";
				break;
		}
	
		unk_0xB5396F1FB088FE38(&uLocal_71);
		unk_0x10425721983AE158(0, sLocal_257, "enter", 8f, -4f, -1, 0, 0, 0, 0, 0);
		unk_0x10425721983AE158(0, sLocal_257, sLocal_258, 8f, -4f, -1, 0, 0, 0, 0, 0);
		unk_0x10425721983AE158(0, sLocal_257, "exit", 8f, -4f, -1, 0, 0, 0, 0, 0);
		unk_0x10425721983AE158(0, sLocal_256, "kneeling_arrest_idle", 8f, -8f, -1, 9, 0, 0, 0, 0);
		unk_0x93C0674FC00824D0(uLocal_71);
		unk_0x4BD42B0527065BB6(uLocal_53, uLocal_71);
		unk_0xD0557B139A542F12(&uLocal_71);
		bLocal_260 = true;
	}
	else if (unk_0x13CCB1AD131C1082(uLocal_53, sLocal_257, "enter", 3))
	{
		if (func_5())
			_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PRAND", 4, iLocal_252, 0, 0);
	}
	else if (unk_0x13CCB1AD131C1082(uLocal_53, sLocal_257, "exit", 3))
	{
	}
	else if (unk_0x13CCB1AD131C1082(uLocal_53, sLocal_257, sLocal_258, 3))
	{
		_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_270, 4, iLocal_252, 0, 0);
	}
	else
	{
		func_8();
	}

	return;
}

BOOL func_5() // Position - 0x119B
{
	if (unk_0x1DD05E817C89C737() > iLocal_262 + 6000)
	{
		iLocal_262 = unk_0x1DD05E817C89C737();
		return true;
	}

	return false;
}

void func_6() // Position - 0x11BC
{
	if (!bLocal_260)
	{
		unk_0xB5396F1FB088FE38(&uLocal_71);
		unk_0x10425721983AE158(0, sLocal_256, "radio_enter", 8f, -4f, -1, 0, 0, 0, 0, 0);
		unk_0x10425721983AE158(0, sLocal_256, "radio_chatter", 8f, -4f, -1, 0, 0, 0, 0, 0);
		unk_0x10425721983AE158(0, sLocal_256, "radio_exit", 8f, -1.5f, -1, 0, 0, 0, 0, 0);
		unk_0x646DE293036A3499(0, uLocal_53, -1, 0);
		unk_0x93C0674FC00824D0(uLocal_71);
		unk_0x4BD42B0527065BB6(uLocal_52, uLocal_71);
		unk_0xD0557B139A542F12(&uLocal_71);
		bLocal_260 = true;
	}
	else if (unk_0x13CCB1AD131C1082(uLocal_52, sLocal_256, "radio_chatter", 3))
	{
		if (func_5())
			_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PRAD3", 4, iLocal_252, 0, 0);
	}
	else if (unk_0x13CCB1AD131C1082(uLocal_52, sLocal_256, "radio_exit", 3))
	{
		bLocal_261 = false;
		func_8();
	}

	return;
}

void func_7() // Position - 0x1290
{
	switch (iLocal_49)
	{
		case 0:
			if (!bLocal_72)
			{
				if (iLocal_47 == 3)
					iLocal_49 = 1;
				else if (iLocal_47 == 0)
					iLocal_49 = 1;
			
				unk_0x1C2F771CDC87A3A5(uLocal_52, 0);
				unk_0x1C2F771CDC87A3A5(uLocal_53, 0);
			
				if (unk_0xFC8BFE4B41177C22(uLocal_52) && unk_0xFC8BFE4B41177C22(uLocal_53))
					unk_0x35CBF41D628A3871(uLocal_52, uLocal_53, -1, 3f, 0f, 0f, 1f, 0);
			
				SYSTEM::SETTIMERB(0);
			}
			break;
	
		case 1:
			if (unk_0xC450B06E5AAA0985(iLocal_56))
				unk_0xFE54B8568B2ABD12(&iLocal_56);
		
			iLocal_49 = 2;
			iLocal_242 = 8000;
			uLocal_289 = unk_0x1DD05E817C89C737();
			break;
	
		case 2:
			if (unk_0xD9F5E1FEFD1329E4(uLocal_55, 0))
			{
				if (unk_0x1DD05E817C89C737() - uLocal_289 > &_CAN_ENTER_FREEROAM_STATE)
				{
					if (!unk_0x4FAFF4BCB7633475(uLocal_54))
					{
						if (unk_0x9B5C1660CCDF7189(uLocal_54, joaat("SCRIPT_TASK_VEHICLE_MISSION")) == 7)
						{
							unk_0xEBA229B2E0BB05E0(uLocal_52, uLocal_55, -1, 0, 1f, 1, 0, 0);
							iLocal_49 = 3;
						}
					}
				}
			}
			break;
	
		case 3:
			if (unk_0xD9F5E1FEFD1329E4(uLocal_55, 0))
			{
				if (!unk_0x4FAFF4BCB7633475(uLocal_54))
				{
					if (unk_0xF4244288C3EF3306(uLocal_52, uLocal_55))
					{
						unk_0x7C8E9DE09D4AD3FF(uLocal_54, uLocal_55, 10f, 786603);
						unk_0x44FB298D6382876D(uLocal_54, 1);
						func_178();
					}
				}
			}
			break;
	}

	return;
}

void func_8() // Position - 0x13B0
{
	bLocal_259 = _GET_RANDOM_BOOL();
	bLocal_259 = bLocal_261;
	iLocal_242 = &_CAN_ENTER_FREEROAM_STATE;
	bLocal_260 = false;
	return;
}

BOOL _GET_RANDOM_BOOL() // Position - 0x13CE
{
	if (IS_BIT_SET(unk_0xC5935DFB3F39785A(0, 65535), 0))
		return 1;

	return 0;
}

BOOL func_10() // Position - 0x13EC
{
	var unk;
	var unk4;
	var unk7;
	var unk10;
	int num;

	num = joaat("pranger");

	if (unk_0xD9F5E1FEFD1329E4(uLocal_55, 0))
		return true;

	unk_0xEC9DAA34BBB4658C(num);

	if (unk_0x6252BC0DD8A320DB(num))
	{
		if (unk_0x8B777C12D64DF209(unk_0xD1A6A821F5AC81DB(uLocal_52, 0), 1, &unk, 1, 1077936128, 0))
		{
			if (unk_0x8D6ED2284F9E6186(&unk, &unk7, &unk10, 0f, 180f, 50f, 1, 1, 1))
			{
				uLocal_55 = unk_0x5779387E956077A6(num, unk7, 0, 1, 1, 0);
				uLocal_54 = unk_0x8728A378EF2B46B2(uLocal_55, 6, joaat("S_M_Y_Ranger_01"), -1, 1, 1);
				unk_0x0428AFDCAA63B06E(uLocal_54, 294, 1);
			
				if (!unk_0x66B90BA528CFEBCE(uLocal_54, joaat("WEAPON_PISTOL"), 0))
					unk_0xB41DEC3AAC1AA107(uLocal_54, joaat("WEAPON_PISTOL"), -1, 0, 1);
			
				unk_0x3C0F448853B71C92(uLocal_54, joaat("WEAPON_PISTOL"), 1);
				unk_0x3CEA1FD137ACE2D9(uLocal_54, uLocal_69);
				unk_0x0B74F181ADFC39BF(uLocal_55, 3);
				unk_0xAAA71DD7E9059338(uLocal_54, 1);
				unk_0x1DE99C193C7EC64B(uLocal_55, 1084227584);
				unk_0x3B1F6012B676860B(uLocal_55, 1);
				unk_0xA80E7D11DB73C8BA(uLocal_55, 1);
				unk = { unk_0xD1A6A821F5AC81DB(uLocal_55, 1) };
				unk4 = { unk_0xD1A6A821F5AC81DB(unk_0x4A8C381C258A124D(), 1) - unk };
				unk_0x5C96CEA06531AB03(uLocal_55, unk_0x97BC40FFA2FFCCD2(unk4, unk4.f_1));
				unk_0x8B777C12D64DF209(unk_0xD1A6A821F5AC81DB(uLocal_52, 0), 1, &unk, 1, 1077936128, 0);
			
				if (unk_0xED977E2AE2CB16EE(unk, 2528.5625f, 2639.1147f, 36.9446f, 1) < 75f)
					unk = { 2473.6006f, 2496.7646f, 40.87f };
			
				if (unk_0xED977E2AE2CB16EE(2538.7097f, 2592.0073f, 36.9446f, unk, 1) < 16f)
					unk = { 2473.6006f, 2496.7646f, 40.87f };
			
				unk_0x30CCF17FEE4BDA53(uLocal_54, uLocal_55, unk, 4, 7f, 786471, 5f, -1f, 1);
				unk_0x93C337B66C95C99B(uLocal_55, 4f);
				unk_0x4539850624F18A9E(uLocal_55, 1);
				bLocal_75 = true;
				unk_0xAE03F542B985A69E(0);
				return true;
			}
		}
	}

	return false;
}

BOOL func_11() // Position - 0x15AE
{
	if (iLocal_47 != 3)
		iLocal_241 = 3;

	switch (iLocal_241)
	{
		case 0:
			if (unk_0x9B5C1660CCDF7189(uLocal_52, joaat("SCRIPT_TASK_PERFORM_SEQUENCE")) == 7 || unk_0x1DD05E817C89C737() > iLocal_244 + 15000)
			{
				if (unk_0xFAA48325A90263BE(unk_0x4A8C381C258A124D(), uLocal_52, 25f, 25f, 25f, 0, 1, 0))
				{
					if (!unk_0x4FAFF4BCB7633475(uLocal_52))
					{
						unk_0x974022927CB47E68(uLocal_52);
						unk_0xB5396F1FB088FE38(&uLocal_71);
					
						if (!unk_0x1C2F771CDC87A3A5(uLocal_53, 0))
							unk_0xAB3658A740EED98E(0, uLocal_53, -1, 3.5f, 2f, 1073741824, 0);
					
						unk_0x0E95B96CFEFE7B61(0, unk_0x4A8C381C258A124D(), 2000);
						unk_0x93C0674FC00824D0(uLocal_71);
						unk_0x4BD42B0527065BB6(uLocal_52, uLocal_71);
						unk_0xD0557B139A542F12(&uLocal_71);
						iLocal_241 = 1;
					}
				}
				else
				{
					iLocal_241 = 3;
				}
			}
			break;
	
		case 1:
			if (unk_0x9B5C1660CCDF7189(uLocal_52, joaat("SCRIPT_TASK_PERFORM_SEQUENCE")) == 1)
			{
				if (unk_0x89EC60A4485FD199(uLocal_52) > 0)
				{
					if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
					{
						if (iLocal_243 < 2)
						{
							_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PTHANK", 4, iLocal_252, 0, 0);
						
							if (unk_0xC450B06E5AAA0985(iLocal_56))
								unk_0x6A52036D51C7E18E(iLocal_56, 1);
						}
						else
						{
							_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PHELP", 4, iLocal_252, 0, 0);
						}
					
						iLocal_241 = 2;
					}
				}
			}
			else if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
			{
				if (iLocal_243 < 2)
				{
					_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PTHANK", 4, iLocal_252, 0, 0);
				
					if (unk_0xC450B06E5AAA0985(iLocal_56))
						unk_0x6A52036D51C7E18E(iLocal_56, 1);
				}
				else
				{
					_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PHELP", 4, iLocal_252, 0, 0);
				}
			
				iLocal_241 = 2;
			}
			break;
	
		case 2:
			if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
				iLocal_241 = 3;
		
			unk_0x0E95B96CFEFE7B61(uLocal_52, unk_0x4A8C381C258A124D(), 100);
			break;
	
		case 3:
			return true;
	}

	return false;
}

void func_12() // Position - 0x1753
{
	BOOL flag;
	BOOL flag2;

	flag = unk_0x9B3D4335E0EDB0BE(uLocal_53, unk_0x4A8C381C258A124D(), 1);
	flag2 = func_57();
	flag2;

	if (!unk_0x4FAFF4BCB7633475(uLocal_53))
	{
		if (!flag)
		{
			if (!flag2)
			{
				if (iLocal_47 == 2 || bLocal_72 == true)
				{
					switch (iLocal_245)
					{
						case 8:
							unk_0xBBB7593C7244A846("criminal_thanks 8", 0.02f, 0.1f, 0f, 0, 0, 255, 255);
							unk_0x6A52036D51C7E18E(iLocal_57, 1);
						
							if (!unk_0x1C2F771CDC87A3A5(uLocal_53, 0) && unk_0x13CCB1AD131C1082(uLocal_53, sLocal_256, "kneeling_arrest_idle", 3))
							{
								unk_0xB5396F1FB088FE38(&uLocal_71);
								unk_0x10425721983AE158(0, sLocal_256, "kneeling_arrest_get_up", 8f, -8f, -1, 0, 0, 0, 0, 0);
								unk_0x93C0674FC00824D0(uLocal_71);
								unk_0x4BD42B0527065BB6(uLocal_53, uLocal_71);
								unk_0xD0557B139A542F12(&uLocal_71);
								iLocal_245 = 9;
							}
							else
							{
								iLocal_245 = 3;
							}
							break;
					
						case 9:
							unk_0xBBB7593C7244A846("criminal_thanks 9", 0.02f, 0.1f, 0f, 0, 0, 255, 255);
						
							if (unk_0x9B5C1660CCDF7189(uLocal_53, joaat("SCRIPT_TASK_PERFORM_SEQUENCE")) == 7)
								iLocal_245 = 0;
							break;
					
						case 0:
							unk_0xBBB7593C7244A846("criminal_thanks 0", 0.02f, 0.1f, 0f, 0, 0, 255, 255);
						
							if (unk_0xFAA48325A90263BE(uLocal_53, unk_0x4A8C381C258A124D(), 75f, 75f, 75f, 0, 1, 0))
							{
								unk_0xFD61BB3B8F1CDB6D(uLocal_53, 1);
								iLocal_245 = 3;
							}
							break;
					
						case 1:
							unk_0xBBB7593C7244A846("criminal_thanks 1", 0.02f, 0.1f, 0f, 0, 0, 255, 255);
						
							if (unk_0x65FFA94B82A71741(uLocal_53, unk_0xD1A6A821F5AC81DB(uLocal_52, 0), 4f, 4f, 3f, 0, 1, 0))
							{
								iLocal_245 = 2;
							}
							else
							{
								unk_0x974022927CB47E68(uLocal_53);
								unk_0xB5396F1FB088FE38(&uLocal_71);
								unk_0xE237FA90A8AFEE59(0, unk_0xD1A6A821F5AC81DB(uLocal_52, 0), 1000, 0, 2);
								unk_0xA966E518B752B92A(0, unk_0xD1A6A821F5AC81DB(uLocal_52, 0), 2f, -1, 3f, 0, 1193033728);
								unk_0x93C0674FC00824D0(uLocal_71);
								unk_0x4BD42B0527065BB6(uLocal_53, uLocal_71);
								unk_0xD0557B139A542F12(&uLocal_71);
								iLocal_245 = 2;
							}
							break;
					
						case 2:
							unk_0xBBB7593C7244A846("criminal_thanks 2", 0.02f, 0.1f, 0f, 0, 0, 255, 255);
						
							if (unk_0x65FFA94B82A71741(uLocal_53, unk_0xD1A6A821F5AC81DB(uLocal_52, 0), 4f, 4f, 3f, 0, 1, 0))
							{
								if (unk_0x9B5C1660CCDF7189(uLocal_53, joaat("SCRIPT_TASK_PERFORM_SEQUENCE")) == 7 || _CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_269, 4, iLocal_252, 0, 0))
									iLocal_245 = 6;
							}
							else if (unk_0x9B5C1660CCDF7189(uLocal_53, joaat("SCRIPT_TASK_PERFORM_SEQUENCE")) == 7)
							{
								unk_0xB5396F1FB088FE38(&uLocal_71);
								unk_0xE237FA90A8AFEE59(0, unk_0xD1A6A821F5AC81DB(uLocal_52, 0), 1000, 0, 2);
								unk_0xA966E518B752B92A(0, unk_0xD1A6A821F5AC81DB(uLocal_52, 0), 2f, -1, 3f, 0, 1193033728);
								unk_0x93C0674FC00824D0(uLocal_71);
								unk_0x4BD42B0527065BB6(uLocal_53, uLocal_71);
								unk_0xD0557B139A542F12(&uLocal_71);
							}
							break;
					
						case 3:
							unk_0xBBB7593C7244A846("criminal_thanks 3", 0.02f, 0.1f, 0f, 0, 0, 255, 255);
						
							if (unk_0xFAA48325A90263BE(uLocal_53, unk_0x4A8C381C258A124D(), 5f, 5f, 3f, 0, 1, 0))
							{
								iLocal_245 = 4;
							}
							else if (unk_0xFAA48325A90263BE(uLocal_53, unk_0x4A8C381C258A124D(), 75f, 75f, 75f, 0, 1, 0))
							{
								if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
								{
									unk_0x974022927CB47E68(uLocal_53);
									unk_0xB5396F1FB088FE38(&uLocal_71);
									unk_0xAB3658A740EED98E(0, unk_0x4A8C381C258A124D(), -1, 8f, 1073741824, 1073741824, 0);
									unk_0x93C0674FC00824D0(uLocal_71);
									unk_0x4BD42B0527065BB6(uLocal_53, uLocal_71);
									unk_0xD0557B139A542F12(&uLocal_71);
									iLocal_245 = 4;
								}
							}
							else
							{
								iLocal_245 = 6;
							}
							break;
					
						case 4:
							unk_0xBBB7593C7244A846("criminal_thanks 4", 0.02f, 0.1f, 0f, 0, 0, 255, 255);
						
							if (unk_0xFAA48325A90263BE(uLocal_53, unk_0x4A8C381C258A124D(), 9f, 9f, 9f, 0, 1, 0))
							{
								uLocal_290 = unk_0x1DD05E817C89C737();
								iLocal_245 = 7;
							}
							break;
					
						case 7:
							unk_0xBBB7593C7244A846("criminal_thanks 7", 0.02f, 0.1f, 0f, 0, 0, 255, 255);
						
							if (!unk_0x9E437CA2E2CAD79D(uLocal_53, unk_0x4A8C381C258A124D()))
								unk_0xE67051907958B5EB(uLocal_53, unk_0x4A8C381C258A124D(), -1, 2064, 2);
						
							if (!unk_0x3644984C9D7B57EF(uLocal_53, unk_0x4A8C381C258A124D(), 30f))
								unk_0x0E95B96CFEFE7B61(uLocal_53, unk_0x4A8C381C258A124D(), -1);
						
							if (unk_0x1DD05E817C89C737() - uLocal_290 > 2000)
								iLocal_245 = 5;
							break;
					
						case 5:
							unk_0xBBB7593C7244A846("criminal_thanks 5", 0.02f, 0.1f, 0f, 0, 0, 255, 255);
						
							if (unk_0x13CCB1AD131C1082(uLocal_53, sLocal_256, "Thanks_Male_05", 3) || unk_0x9B5C1660CCDF7189(uLocal_53, joaat("SCRIPT_TASK_PERFORM_SEQUENCE")) == 7)
							{
								if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
								{
									if (iLocal_243 < 2)
									{
										_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_264, 4, iLocal_252, 0, 0);
										unk_0x10425721983AE158(uLocal_53, sLocal_256, "Thanks_Male_05", 16f, -16f, -1, 16, 0, 0, 0, 0);
									}
									else
									{
										_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_268, 4, iLocal_252, 0, 0);
									}
								
									func_15(_GET_CURRENT_PLAYER_CHARACTER(), 1, 250, false, false);
									unk_0x84B06A81C98DA4B8(uLocal_53);
									iLocal_245 = 6;
								}
							}
							break;
					
						case 6:
							unk_0xBBB7593C7244A846("criminal_thanks 6", 0.02f, 0.1f, 0f, 0, 0, 255, 255);
						
							if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
							{
								unk_0xD844F5E50DAB6FF7(uLocal_53, unk_0x4A8C381C258A124D(), 150f, -1, 0, 0);
								func_178();
							}
							break;
					}
				}
			}
			else
			{
				if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
					_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_271, 4, iLocal_252, 0, 0);
			
				unk_0xD844F5E50DAB6FF7(uLocal_53, unk_0x4A8C381C258A124D(), 150f, -1, 0, 0);
				func_178();
			}
		}
		else
		{
			func_13();
			unk_0xD844F5E50DAB6FF7(uLocal_53, unk_0x4A8C381C258A124D(), 150f, -1, 0, 0);
		
			if (flag == true)
				iLocal_47 = 1;
		
			if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
				_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_268, 4, iLocal_252, 0, 0);
		
			func_178();
		}
	}

	return;
}

void func_13() // Position - 0x1C96
{
	Global_21291 = 0;
	func_14();
	return;
}

void func_14() // Position - 0x1CA6
{
	if (unk_0x1F9EB85925C3ECD7())
	{
		unk_0xC78B293A5F4EACF9();
		Global_23436 = 0;
		unk_0x0F15249D24BC5ADA(1);
		Global_22425 = 6;
		return;
	}

	return;
}

void func_15(eCharacter echParam0, int iParam1, int iParam2, BOOL bParam3, BOOL bParam4) // Position - 0x1CCA
{
	int num;
	int num2;

	if (func_50(echParam0) == 3)
		return;

	if (func_50(echParam0) == 4)
		return;

	func_16(func_50(echParam0), 1, iParam1, iParam2, false);

	if (bParam3)
	{
		num = 0;
	
		if (bParam4)
		{
			switch (echParam0)
			{
				case CHAR_MICHAEL:
					num2 = joaat("SP0_MONEY_MADE_FROM_RANDOM_PEDS");
					break;
			
				case CHAR_FRANKLIN:
					num2 = joaat("SP1_MONEY_MADE_FROM_RANDOM_PEDS");
					break;
			
				case CHAR_TREVOR:
					num2 = joaat("SP2_MONEY_MADE_FROM_RANDOM_PEDS");
					break;
			
				default:
					return;
			}
		}
		else
		{
			switch (echParam0)
			{
				case CHAR_MICHAEL:
					num2 = joaat("SP0_MONEY_MADE_FROM_MISSIONS");
					break;
			
				case CHAR_FRANKLIN:
					num2 = joaat("SP1_MONEY_MADE_FROM_MISSIONS");
					break;
			
				case CHAR_TREVOR:
					num2 = joaat("SP2_MONEY_MADE_FROM_MISSIONS");
					break;
			
				default:
					return;
			}
		}
	
		unk_0xDF7F16323520B858(num2, &num, -1);
		num = num + iParam2;
		unk_0x1164A75E490C27B6(num2, num, 1);
	}

	return;
}

int func_16(int iParam0, int iParam1, int iParam2, int iParam3, BOOL bParam4) // Position - 0x1D9C
{
	float num;
	int num2;
	int num3;
	int num4;
	int num5;
	int num6;

	func_49();

	if (iParam3 < 1)
		return 0;

	num = 1f;

	switch (iParam1)
	{
		case 0:
			switch (iParam0)
			{
				case 0:
					func_48(99, 1);
					func_47(joaat("SP0_MONEY_TOTAL_SPENT"), iParam3);
					break;
			
				case 1:
					func_47(joaat("SP1_MONEY_TOTAL_SPENT"), iParam3);
					break;
			
				case 2:
					func_47(joaat("SP2_MONEY_TOTAL_SPENT"), iParam3);
					break;
			}
		
			func_31(false);
		
			switch (iParam2)
			{
				case 127:
				case 129:
				case 125:
				case 126:
				case 128:
					if (func_28(5))
					{
						num = 0.9f;
						num2 = 5;
					}
					break;
			
				case 63:
				case 64:
				case 65:
				case 66:
				case 67:
				case 68:
					switch (iParam0)
					{
						case 0:
							func_47(joaat("SP0_MONEY_SPENT_ON_TATTOOS"), iParam3);
							break;
					
						case 1:
							func_47(joaat("SP1_MONEY_SPENT_ON_TATTOOS"), iParam3);
							break;
					
						case 2:
							func_47(joaat("SP2_MONEY_SPENT_ON_TATTOOS"), iParam3);
							break;
					}
				
					if (func_28(1))
					{
						num = 0f;
						num2 = 1;
					}
					break;
			
				case 21:
					switch (iParam0)
					{
						case 0:
							func_47(joaat("SP0_MONEY_SPENT_ON_TAXIS"), iParam3);
							break;
					
						case 1:
							func_47(joaat("SP1_MONEY_SPENT_ON_TAXIS"), iParam3);
							break;
					
						case 2:
							func_47(joaat("SP2_MONEY_SPENT_ON_TAXIS"), iParam3);
							break;
					}
					break;
			
				case 25:
					switch (iParam0)
					{
						case 0:
							func_47(joaat("SP0_MONEY_SPENT_IN_STRIP_CLUBS"), iParam3);
							break;
					
						case 1:
							func_47(joaat("SP1_MONEY_SPENT_IN_STRIP_CLUBS"), iParam3);
							break;
					
						case 2:
							func_47(joaat("SP2_MONEY_SPENT_IN_STRIP_CLUBS"), iParam3);
							break;
					}
					break;
			
				case 99:
				case 100:
				case 101:
				case 102:
				case 104:
				case 105:
				case 106:
				case 107:
				case 108:
				case 109:
				case 110:
				case 111:
				case 112:
				case 113:
					switch (iParam0)
					{
						case 0:
							func_47(joaat("SP0_MONEY_SPENT_PROPERTY"), iParam3);
							break;
					
						case 1:
							func_47(joaat("SP1_MONEY_SPENT_PROPERTY"), iParam3);
							break;
					
						case 2:
							func_47(joaat("SP2_MONEY_SPENT_PROPERTY"), iParam3);
							break;
					}
					break;
			
				default:
					switch (unk_0x931D15B1D60C81B0())
					{
						case joaat("clothes_shop_sp"):
							switch (iParam0)
							{
								case 0:
									func_47(joaat("SP0_MONEY_SPENT_IN_CLOTHES"), iParam3);
									break;
							
								case 1:
									func_47(joaat("SP1_MONEY_SPENT_IN_CLOTHES"), iParam3);
									break;
							
								case 2:
									func_47(joaat("SP2_MONEY_SPENT_IN_CLOTHES"), iParam3);
									break;
							}
							break;
					
						case joaat("hairdo_shop_sp"):
							switch (iParam0)
							{
								case 0:
									func_47(joaat("SP0_MONEY_SPENT_ON_HAIRDOS"), iParam3);
									break;
							
								case 1:
									func_47(joaat("SP1_MONEY_SPENT_ON_HAIRDOS"), iParam3);
									break;
							
								case 2:
									func_47(joaat("SP2_MONEY_SPENT_ON_HAIRDOS"), iParam3);
									break;
							}
						
							if (func_28(0))
							{
								num = 0f;
								num2 = 0;
							}
							break;
					
						case joaat("gunclub_shop"):
							switch (iParam0)
							{
								case 0:
									func_47(joaat("SP0_MONEY_SPENT_IN_BUYING_GUNS"), iParam3);
									break;
							
								case 1:
									func_47(joaat("SP1_MONEY_SPENT_IN_BUYING_GUNS"), iParam3);
									break;
							
								case 2:
									func_47(joaat("SP2_MONEY_SPENT_IN_BUYING_GUNS"), iParam3);
									break;
							}
							break;
					
						case joaat("carmod_shop"):
							switch (iParam0)
							{
								case 0:
									func_47(joaat("SP0_MONEY_SPENT_CAR_MODS"), iParam3);
									break;
							
								case 1:
									func_47(joaat("SP1_MONEY_SPENT_CAR_MODS"), iParam3);
									break;
							
								case 2:
									func_47(joaat("SP2_MONEY_SPENT_CAR_MODS"), iParam3);
									break;
							}
						
							func_27(iParam3);
							break;
					}
					break;
			}
			break;
	
		case 1:
			switch (iParam0)
			{
				case 0:
					func_48(95, iParam3);
					break;
			
				case 1:
					func_48(97, iParam3);
					break;
			
				case 2:
					func_48(96, iParam3);
					break;
			}
		
			func_48(98, iParam3);
			break;
	}

	num3 = iParam0;
	iParam3 = SYSTEM::FLOOR(num * SYSTEM::TO_FLOAT(iParam3));
	num4 = 0;
	num5 = iParam3;

	if (num == 0f)
	{
		func_19(num2);
		return 1;
	}
	else if (num != 1f)
	{
		func_19(num2);
	}

	num6 = Global_61482[num3] + iParam3;

	switch (iParam1)
	{
		case 1:
			if (Global_61482[num3] >= 0 && iParam3 > 0)
				if (num6 <= 0)
					Global_61482[num3] = 2147483647;
				else
					Global_61482[num3] = Global_61482[num3] + iParam3;
		
			switch (iParam0)
			{
				case 0:
					func_47(joaat("SP0_TOTAL_CASH_EARNED"), iParam3);
					break;
			
				case 1:
					func_47(joaat("SP1_TOTAL_CASH_EARNED"), iParam3);
					break;
			
				case 2:
					func_47(joaat("SP2_TOTAL_CASH_EARNED"), iParam3);
					break;
			}
			break;
	
		case 0:
			if (!bParam4)
				if (Global_61482[num3] - iParam3 < 0)
					return 0;
		
			num4 = Global_61482[num3];
			Global_61482[num3] = Global_61482[num3] - iParam3;
		
			if (bParam4)
				num5 = num4;
			break;
	}

	if (iParam2 == 1)
	{
		num5 > 20;
	}
	else
	{
		Global_114135.f_20568.f_233[num3 /*69*/].f_2[Global_114135.f_20568.f_233[num3 /*69*/].f_1 /*6*/] = iParam1;
		Global_114135.f_20568.f_233[num3 /*69*/].f_2[Global_114135.f_20568.f_233[num3 /*69*/].f_1 /*6*/].f_1 = iParam2;
		Global_114135.f_20568.f_233[num3 /*69*/].f_2[Global_114135.f_20568.f_233[num3 /*69*/].f_1 /*6*/].f_2 = iParam3;
		Global_114135.f_20568.f_233[num3 /*69*/] = Global_114135.f_20568.f_233[num3 /*69*/] + 1;
		Global_114135.f_20568.f_233[num3 /*69*/].f_1 = Global_114135.f_20568.f_233[num3 /*69*/].f_1 + 1;
	
		if (Global_114135.f_20568.f_233[num3 /*69*/].f_1 > 10)
			Global_114135.f_20568.f_233[num3 /*69*/].f_1 = 0;
	}

	func_18(iParam0);

	if (Global_44181 == 15)
		func_17(false);

	return 1;
}

void func_17(BOOL bParam0) // Position - 0x239B
{
	int i;
	int j;

	i = 0;
	j = 0;

	for (i = 0; i < 3; i = i + 1)
	{
		for (j = 0; j < 11; j = j + 1)
		{
			Global_114135.f_20568.f_233[i /*69*/].f_2[j /*6*/].f_3 = Global_114135.f_20568.f_233[i /*69*/].f_2[j /*6*/];
			Global_114135.f_20568.f_233[i /*69*/].f_2[j /*6*/].f_4 = Global_114135.f_20568.f_233[i /*69*/].f_2[j /*6*/].f_1;
			Global_114135.f_20568.f_233[i /*69*/].f_2[j /*6*/].f_5 = Global_114135.f_20568.f_233[i /*69*/].f_2[j /*6*/].f_2;
		}
	}

	for (i = 0; i < 10; i = i + 1)
	{
		Global_61490[i /*3*/][0] = Global_114135.f_20568[i];
		Global_61490.f_31[i /*3*/][0] = Global_114135.f_20568.f_11[i];
		Global_61490.f_62[i /*3*/][0] = Global_114135.f_20568.f_22[i];
		Global_61490.f_93[i /*3*/][0] = Global_114135.f_20568.f_33[i];
		Global_61490.f_124[i /*3*/][0] = Global_114135.f_20568.f_44[i];
		Global_61490.f_155[i /*3*/][0] = Global_114135.f_20568.f_55[i];
		Global_61490.f_186[i /*3*/][0] = Global_114135.f_20568.f_66[i];
		Global_61490.f_217[i /*3*/][0] = Global_114135.f_20568.f_77[i];
		Global_61490.f_248[i /*3*/][0] = Global_114135.f_20568.f_88[i];
	
		if (!bParam0)
		{
			Global_61490[i /*3*/][1] = Global_114135.f_20568[i];
			Global_61490.f_31[i /*3*/][1] = Global_114135.f_20568.f_11[i];
			Global_61490.f_62[i /*3*/][1] = Global_114135.f_20568.f_22[i];
			Global_61490.f_93[i /*3*/][1] = Global_114135.f_20568.f_33[i];
			Global_61490.f_124[i /*3*/][1] = Global_114135.f_20568.f_44[i];
			Global_61490.f_155[i /*3*/][1] = Global_114135.f_20568.f_55[i];
			Global_61490.f_186[i /*3*/][1] = Global_114135.f_20568.f_66[i];
			Global_61490.f_217[i /*3*/][1] = Global_114135.f_20568.f_77[i];
			Global_61490.f_248[i /*3*/][1] = Global_114135.f_20568.f_88[i];
		}
	}

	return;
}

void func_18(int iParam0) // Position - 0x261D
{
	int num;

	num = Global_61482[iParam0];

	switch (iParam0)
	{
		case 0:
			unk_0x1164A75E490C27B6(joaat("SP0_TOTAL_CASH"), num, 1);
			break;
	
		case 1:
			unk_0x1164A75E490C27B6(joaat("SP1_TOTAL_CASH"), num, 1);
			break;
	
		case 2:
			unk_0x1164A75E490C27B6(joaat("SP2_TOTAL_CASH"), num, 1);
			break;
	}

	return;
}

void func_19(int iParam0) // Position - 0x2677
{
	BOOL flag;
	var unk;

	if (iParam0 == 8)
	{
		_STAT_SET_PACKED_BOOL(129, 0, -1);
		return;
	}

	if (iParam0 == 9)
	{
		_STAT_SET_PACKED_BOOL(135, 0, -1);
		return;
	}

	if (iParam0 == 10)
	{
		_STAT_SET_PACKED_BOOL(136, 0, -1);
		return;
	}

	if (iParam0 == 11)
	{
		_STAT_SET_PACKED_BOOL(137, 0, -1);
		return;
	}

	if (iParam0 == 12)
	{
		func_22(8272, 0, -1, 1);
		return;
	}

	if (iParam0 == 13)
	{
		func_22(8273, 0, -1, 1);
		return;
	}

	if (iParam0 == 14)
	{
		func_22(8274, 0, -1, 1);
		return;
	}

	if (iParam0 == 15)
	{
		func_22(8275, 0, -1, 1);
		return;
	}

	if (iParam0 == 16)
	{
		func_22(8276, 0, -1, 1);
		return;
	}

	if (iParam0 == 17)
	{
		func_22(8277, 0, -1, 1);
		return;
	}

	flag = false;

	if (!unk_0x76CD105BCAC6EB9F())
	{
		if (IS_BIT_SET(Global_114135.f_20568.f_471, iParam0))
		{
			flag = true;
			unk_0x8744D2E3FC95740E(&(Global_114135.f_20568.f_471), iParam0);
		}
	}
	else if (IS_BIT_SET(Global_114135.f_20568.f_471, iParam0) || IS_BIT_SET(Global_2359296[func_21() /*5571*/].f_681.f_10, iParam0))
	{
		flag = true;
		unk_0x8744D2E3FC95740E(&(Global_114135.f_20568.f_471), iParam0);
		unk_0x8744D2E3FC95740E(&(Global_2359296[func_21() /*5571*/].f_681.f_10), iParam0);
	}

	if (flag)
	{
		TEXT_LABEL_ASSIGN_STRING(&unk, "CHAR_LIFEINVADER", 64);
		unk_0xCCDB0041859B85A6("COUP_RED" /*You have redeemed your promotion for ~a~*/);
		unk_0xACF853FB3F6EA7D4(AUDIO_SPEECH_GET_PARAM_STRING_FROM_ENUM(iParam0));
		unk_0x5C7E2225D7451992(&unk, &unk, 1, 0, "", 0);
	}

	return;
}

char* AUDIO_SPEECH_GET_PARAM_STRING_FROM_ENUM(int iParam0) // Position - 0x27F1
{
	switch (iParam0)
	{
		case 0:
			return "COUP_HAIRC" /*one free haircut.*/;
	
		case 1:
			return "COUP_TATTOO" /*one free tattoo.*/;
	
		case 2:
			return "COUP_WARSTOCK" /*10% off your next purchase at Warstock-Cache-and-Carry.com.*/;
	
		case 3:
			return "COUP_MOSPORT" /*10 percent off your next purchase at LegendaryMotorsport.net.*/;
	
		case 4:
			return "COUP_ELITAS" /*10 percent off your next purchase at ElitasTravel.com.*/;
	
		case 5:
			return "COUP_MEDSPENS" /*10 percent off your next medical expenses.*/;
	
		case 6:
			return "COUP_SPRUNK" /*one free can of Sprunk.*/;
	
		case 7:
			return "COUP_RESPRAY" /*one free respray.*/;
	
		case 8:
			return "COUP_XMAS2017";
	
		case 9:
			return "COUP_CAR_XMAS2018";
	
		case 10:
			return "COUP_HELI_XMAS2018";
	
		case 11:
			return "COUP_CAR2_XMAS2018";
	
		case 12:
			return "COUP_CAS_ELITAS" /*10 percent off your next purchase at ElitasTravel.com.*/;
	
		case 13:
			return "COUP_CAS_DOCKTEASE" /*10 percent off your next purchase at DockTease.com.*/;
	
		case 14:
			return "COUP_CAS_MOSPORT" /*10 percent off your next purchase at LegendaryMotorsport.net.*/;
	
		case 15:
			return "COUP_CAS_SSASA" /*10 percent off your next purchase at southernsanandreassuperautos.com.*/;
	
		case 16:
			return "COUP_CAS_WARSTOCK" /*10 percent off your next purchase at Warstock-Cache-and-Carry.com.*/;
	
		case 17:
			return "COUP_CAS_PANDM" /*10 percent off your next purchase at pandmcycles.com.*/;
	
		default:
			break;
	}

	return "";
}

int func_21() // Position - 0x28F4
{
	int num;

	num = 0;
	return num;
}

void func_22(int iParam0, int iParam1, int iParam2, int iParam3) // Position - 0x2901
{
	int num;

	if (iParam0 != 15468)
	{
		num = func_23(iParam0, iParam2);
	
		if (num != 0)
			unk_0x1164A75E490C27B6(num, iParam1, iParam3);
	}

	return;
}

int func_23(int iParam0, int iParam1) // Position - 0x292F
{
	return unk_0xD69CE161FE614531(0, iParam0, func_24(iParam1));
}

int func_24(int iParam0) // Position - 0x2944
{
	int num;
	int num2;

	num = iParam0;

	if (num == -1)
	{
		num2 = func_25();
	
		if (num2 > -1)
		{
			Global_2751934 = 0;
			num = num2;
		}
		else
		{
			num = 0;
			Global_2751934 = 1;
		}
	}

	return num;
}

int func_25() // Position - 0x2978
{
	return Global_1574926;
}

void _STAT_SET_PACKED_BOOL(int iParam0, int iParam1, int iParam2) // Position - 0x2984
{
	if (iParam2 == -1)
		iParam2 = func_25();

	unk_0xA595AA1819B05EA0(iParam0, iParam1, iParam2);
	return;
}

void func_27(int iParam0) // Position - 0x29A2
{
	func_48(93, iParam0);
	func_48(29, iParam0);
	func_48(30, iParam0);
	return;
}

BOOL func_28(int iParam0) // Position - 0x29C2
{
	int num;
	int num2;
	int num3;
	int num4;
	int num5;
	int num6;
	int num7;
	int num8;
	int num9;
	int num10;
	int num11;
	int num12;

	if (iParam0 == 8)
		return func_30(129, -1);

	if (iParam0 == 9)
		return func_30(135, -1);

	if (iParam0 == 10)
		return func_30(136, -1);

	if (iParam0 == 11)
		return func_30(137, -1);

	if (iParam0 == 12)
	{
		num = unk_0x39D1D336459711BE();
		num2 = func_29(8272, -1);
	
		if (num2 == 0 || num >= num2)
			return false;
	
		return true;
	}

	if (iParam0 == 13)
	{
		num3 = unk_0x39D1D336459711BE();
		num4 = func_29(8273, -1);
	
		if (num4 == 0 || num3 >= num4)
			return false;
	
		return true;
	}

	if (iParam0 == 14)
	{
		num5 = unk_0x39D1D336459711BE();
		num6 = func_29(8274, -1);
	
		if (num6 == 0 || num5 >= num6)
			return false;
	
		return true;
	}

	if (iParam0 == 15)
	{
		num7 = unk_0x39D1D336459711BE();
		num8 = func_29(8275, -1);
	
		if (num8 == 0 || num7 >= num8)
			return false;
	
		return true;
	}

	if (iParam0 == 16)
	{
		num9 = unk_0x39D1D336459711BE();
		num10 = func_29(8276, -1);
	
		if (num10 == 0 || num9 >= num10)
			return false;
	
		return true;
	}

	if (iParam0 == 17)
	{
		num11 = unk_0x39D1D336459711BE();
		num12 = func_29(8277, -1);
	
		if (num12 == 0 || num11 >= num12)
			return false;
	
		return true;
	}

	if (!unk_0x76CD105BCAC6EB9F())
		return IS_BIT_SET(Global_114135.f_20568.f_471, iParam0);

	return IS_BIT_SET(Global_2359296[func_21() /*5571*/].f_681.f_10, iParam0);
}

int func_29(int iParam0, int iParam1) // Position - 0x2B62
{
	var unk;
	int num;

	if (iParam0 != 15468)
	{
		unk = func_23(iParam0, iParam1);
	
		if (unk_0xDF7F16323520B858(unk, &num, -1))
			return num;
	}

	return 0;
}

BOOL func_30(int iParam0, int iParam1) // Position - 0x2B91
{
	if (iParam1 == -1)
		iParam1 = func_25();

	return unk_0xA6D3C21763E25496(iParam0, iParam1);
}

int func_31(BOOL bParam0) // Position - 0x2BAD
{
	var unk;
	int num;
	int num2;

	num = 0;

	if (unk_0x37088D2C63AC4C01(27))
		return 0;

	if (unk_0xDF7F16323520B858(joaat("SP0_MONEY_TOTAL_SPENT"), &unk, -1))
		num = num + unk;

	if (unk_0xDF7F16323520B858(joaat("SP1_MONEY_TOTAL_SPENT"), &unk, -1))
		num = num + unk;

	if (unk_0xDF7F16323520B858(joaat("SP2_MONEY_TOTAL_SPENT"), &unk, -1))
		num = num + unk;

	bParam0;
	num2 = 0;
	unk_0xDF7F16323520B858(joaat("NUM_CASH_SPENT"), &num2, -1);

	if (num > 0 && num2 / 2000000 != num / 2000000)
	{
		unk_0x1164A75E490C27B6(joaat("NUM_CASH_SPENT"), num, 1);
		func_46(27, num);
	}

	if (num < 200000000)
		return 0;

	func_32(27, 1);
	return 1;
}

int func_32(int iParam0, int iParam1) // Position - 0x2C64
{
	if (iParam0 >= 78)
		return 0;

	return func_33(iParam0, iParam1);
}

int func_33(int iParam0, int iParam1) // Position - 0x2C7F
{
	if (func_45(14) && !func_44(iParam0))
		return 0;

	if (unk_0x37088D2C63AC4C01(iParam0) && iParam1 == 1)
		return 0;

	if (Global_33338 != 0 && !Global_79539)
		return 0;

	if (func_43(&Global_4546323))
	{
		if (func_41(&Global_4546323, iParam0))
			return 0;
	
		if (func_34(&Global_4546323, iParam0))
			return 1;
	}
	else
	{
		if (!unk_0x584E143398F9661C(iParam0))
			return 0;
	
		if (unk_0x37088D2C63AC4C01(iParam0))
			return 1;
	
		return 0;
	}

	return 0;
}

BOOL func_34(var uParam0, int iParam1) // Position - 0x2D1C
{
	int i;
	var unk;

	unk = 78;

	if (unk_0x37088D2C63AC4C01(iParam1))
		return false;

	if (func_45(14) && !func_44(iParam1))
		return false;

	if (func_41(uParam0, iParam1))
		return false;

	if (func_40(uParam0) < 0f)
		func_39(uParam0, 0);

	func_37(&unk);
	i = 0;

	for (i = 0; i < *uParam0 - 1; i = i + 1)
	{
		unk[i + 1] = uParam0->[i];
	}

	func_35(&unk, iParam1);
	i = 0;

	for (i = 0; i < *uParam0; i = i + 1)
	{
		uParam0->[i] = unk[i];
	}

	return true;
}

int func_35(var uParam0, int iParam1) // Position - 0x2DCD
{
	int i;

	if (unk_0x37088D2C63AC4C01(iParam1))
		return 0;

	if (func_45(14) && !func_44(iParam1))
		return 0;

	if (func_41(uParam0, iParam1))
		return 0;

	if (func_40(uParam0) < 0f)
		func_39(uParam0, 0);

	for (i = 0; i < *uParam0; i = i + 1)
	{
		if (func_36(uParam0, i))
		{
			uParam0->[i] = iParam1;
			return 1;
		}
	}

	return 0;
}

BOOL func_36(var uParam0, int iParam1) // Position - 0x2E48
{
	return uParam0->[iParam1] == 78;
}

void func_37(var uParam0) // Position - 0x2E59
{
	int i;

	for (i = 0; i < *uParam0; i = i + 1)
	{
		func_38(uParam0, i);
	}

	func_39(uParam0, Global_4546322 - 0.5f);
	return;
}

void func_38(var uParam0, int iParam1) // Position - 0x2E8D
{
	uParam0->[iParam1] = 78;
	return;
}

void func_39(var uParam0, int iParam1) // Position - 0x2E9D
{
	if (iParam1 == 0f)
		uParam0->f_80 = 0f;
	else
		uParam0->f_80 = iParam1;

	return;
}

float func_40(var uParam0) // Position - 0x2EBA
{
	return uParam0->f_80;
}

BOOL func_41(var uParam0, int iParam1) // Position - 0x2EC6
{
	return func_42(uParam0, iParam1) != -1;
}

int func_42(var uParam0, int iParam1) // Position - 0x2ED8
{
	int i;

	for (i = 0; i < *uParam0; i = i + 1)
	{
		if (uParam0->[i] == iParam1)
			return i;
	}

	return -1;
}

BOOL func_43(var uParam0) // Position - 0x2F05
{
	return uParam0->f_79 == 1;
}

BOOL func_44(int iParam0) // Position - 0x2F13
{
	switch (iParam0)
	{
		case 60:
		case 61:
		case 62:
		case 63:
		case 64:
		case 65:
		case 66:
		case 67:
		case 68:
		case 69:
			return true;
	
		default:
		
	}

	return false;
}

BOOL func_45(int iParam0) // Position - 0x2F63
{
	return Global_44181 == iParam0;
}

int func_46(int iParam0, int iParam1) // Position - 0x2F71
{
	int num;

	if (iParam0 < 0)
		return 0;

	if (iParam0 > 78)
		return 0;

	if (iParam1 <= 0 || iParam1 > 100)
		return 0;

	num = unk_0xB3E8CE9ABB5AD331(iParam0);

	if (iParam1 > num)
		return unk_0xE3872E56266EDEDC(iParam0, iParam1);

	return 0;
}

void func_47(int iParam0, int iParam1) // Position - 0x2FC2
{
	int num;

	unk_0xDF7F16323520B858(iParam0, &num, -1);
	num = num + iParam1;
	unk_0x1164A75E490C27B6(iParam0, num, 1);
	return;
}

void func_48(int iParam0, int iParam1) // Position - 0x2FE5
{
	int num;

	if (iParam1 < 1)
		return;

	if (Global_60050[iParam0 /*7*/].f_2)
		return;

	if (unk_0x76CD105BCAC6EB9F())
		return;

	if (Global_60050[iParam0 /*7*/])
	{
		unk_0xDF7F16323520B858(Global_60050[iParam0 /*7*/].f_1, &num, -1);
		num = num + iParam1;
		unk_0x1164A75E490C27B6(Global_60050[iParam0 /*7*/].f_1, num, 1);
	}

	return;
}

void func_49() // Position - 0x3042
{
	int unk;

	if (unk_0x5F9F81C08516558E())
	{
		unk_0xDF7F16323520B858(joaat("SP0_TOTAL_CASH"), &unk, -1);
	
		if (!(Global_61482[0] == unk))
			Global_61482[0] = unk;
	
		unk_0xDF7F16323520B858(joaat("SP1_TOTAL_CASH"), &unk, -1);
	
		if (!(Global_61482[1] == unk))
			Global_61482[1] = unk;
	
		unk_0xDF7F16323520B858(joaat("SP2_TOTAL_CASH"), &unk, -1);
	
		if (!(Global_61482[2] == unk))
			Global_61482[2] = unk;
	}

	return;
}

int func_50(eCharacter echParam0) // Position - 0x30B7
{
	return Global_2201[echParam0 /*29*/].f_17;
}

eCharacter _GET_CURRENT_PLAYER_CHARACTER() // Position - 0x30C8
{
	func_52();
	return Global_114135.f_2367.f_539.f_4321;
}

void func_52() // Position - 0x30E1
{
	eCharacter character;

	if (unk_0xFC8BFE4B41177C22(unk_0x4A8C381C258A124D()))
	{
		if (_GET_CHARACTER_MODEL(Global_114135.f_2367.f_539.f_4321) != unk_0x4B423FAA24E8ABF0(unk_0x4A8C381C258A124D()))
		{
			character = _GET_PLAYER_CHARACTER_FROM_PED(unk_0x4A8C381C258A124D());
		
			if (func_53(character) && !func_45(14) || Global_113083)
			{
				if (Global_114135.f_2367.f_539.f_4321 != character && func_53(Global_114135.f_2367.f_539.f_4321))
					Global_114135.f_2367.f_539.f_4322 = Global_114135.f_2367.f_539.f_4321;
			
				Global_114135.f_2367.f_539.f_4323 = character;
				Global_114135.f_2367.f_539.f_4321 = character;
				return;
			}
		}
		else
		{
			if (Global_114135.f_2367.f_539.f_4321 != _CHAR_NULL)
				Global_114135.f_2367.f_539.f_4323 = Global_114135.f_2367.f_539.f_4321;
		
			return;
		}
	}

	Global_114135.f_2367.f_539.f_4321 = 145;
	return;
}

BOOL func_53(eCharacter echParam0) // Position - 0x31DE
{
	return echParam0 < CHAR_MULTIPLAYER;
}

eCharacter _GET_PLAYER_CHARACTER_FROM_PED(var uParam0) // Position - 0x31EA
{
	eCharacter i;
	int num;

	if (unk_0xFC8BFE4B41177C22(uParam0))
	{
		num = unk_0x4B423FAA24E8ABF0(uParam0);
	
		for (i = CHAR_MICHAEL; i <= CHAR_TREVOR; i = i + 1)
		{
			if (_GET_CHARACTER_MODEL(i) == num)
				return i;
		}
	}

	return _CHAR_NULL;
}

int _GET_CHARACTER_MODEL(eCharacter character) // Position - 0x3227
{
	if (func_53(character))
		return func_56(character);
	else
		character != _CHAR_NULL;

	return 0;
}

int func_56(eCharacter echParam0) // Position - 0x324C
{
	return Global_2201[echParam0 /*29*/];
}

BOOL func_57() // Position - 0x325B
{
	float num;
	var unk;
	var unk2;

	num = 50f;

	if (unk_0xD9F5E1FEFD1329E4(uLocal_55, 0))
		if (unk_0xFAA48325A90263BE(uLocal_55, uLocal_53, num, num, num, 0, 1, 0))
			return 1;

	unk = unk_0xFA4D35AD36BDA1FE(unk_0xD1A6A821F5AC81DB(uLocal_53, 0), num, num, num, -1);

	if (!unk_0x4FAFF4BCB7633475(unk))
		if (unk_0x4B423FAA24E8ABF0(unk) == joaat("S_F_Y_Cop_01") || unk_0x4B423FAA24E8ABF0(unk) == joaat("S_M_Y_Cop_01"))
			return 1;

	unk = unk_0xFA4D35AD36BDA1FE(unk_0xD1A6A821F5AC81DB(unk_0x4A8C381C258A124D(), 0), num, num, num, -1);

	if (!unk_0x4FAFF4BCB7633475(unk))
		if (unk_0x4B423FAA24E8ABF0(unk) == joaat("S_F_Y_Cop_01") || unk_0x4B423FAA24E8ABF0(unk) == joaat("S_M_Y_Cop_01"))
			return 1;

	unk2 = unk_0x66D94742BB3D032F(unk_0xD1A6A821F5AC81DB(uLocal_53, 0), num, 0, 1024);

	if (unk_0xD9F5E1FEFD1329E4(unk2, 0))
		return 1;

	unk2 = unk_0x66D94742BB3D032F(unk_0xD1A6A821F5AC81DB(unk_0x4A8C381C258A124D(), 0), num, 0, 1024);

	if (unk_0xD9F5E1FEFD1329E4(unk2, 0))
		return 1;

	return 0;
}

void func_58() // Position - 0x3359
{
	BOOL flag;
	BOOL flag2;
	int num;

	if (!unk_0x4FAFF4BCB7633475(uLocal_53))
	{
		flag = unk_0x685171EED42BC4DF(unk_0x4A8C381C258A124D(), uLocal_53);
		flag2 = unk_0x9B3D4335E0EDB0BE(uLocal_53, unk_0x4A8C381C258A124D(), 1);
	
		if (flag2 || func_71() || func_69() || flag)
		{
			func_68();
		
			if (bLocal_72)
			{
				if (flag || flag2 && !unk_0x8BF5256C439DF778(uLocal_53) && !unk_0x7F420695E3F776FB(uLocal_53, 0))
				{
					unk_0x974022927CB47E68(uLocal_53);
					unk_0xB1C2DC5C115FA50D(uLocal_53, 500, 2000, 0, 0, 0, 0);
					unk_0xAFC7A89C990C4339(uLocal_53, -1);
					unk_0x44FB298D6382876D(uLocal_53, 1);
				}
				else if (!unk_0x7F420695E3F776FB(uLocal_53, 0) && !unk_0x8BF5256C439DF778(uLocal_53))
				{
					unk_0x974022927CB47E68(uLocal_53);
					unk_0xAFC7A89C990C4339(uLocal_53, -1);
					unk_0x44FB298D6382876D(uLocal_53, 1);
				}
			
				func_64(0);
			
				if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
					if (!unk_0x4FAFF4BCB7633475(uLocal_52) && iLocal_284 == 0)
						if (_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PWTF", 4, iLocal_252, 0, 0))
							iLocal_284 = 1;
			
				iLocal_47 = 1;
			}
			else
			{
				iLocal_47 = 3;
			}
		}
	}
	else
	{
		if (unk_0xC450B06E5AAA0985(iLocal_57))
		{
			unk_0xFE54B8568B2ABD12(&iLocal_57);
			func_63(&uLocal_76, 2);
		}
	
		func_68();
	
		if (unk_0x9B3D4335E0EDB0BE(uLocal_53, unk_0x4A8C381C258A124D(), 1))
		{
			if (bLocal_72)
			{
				func_64(1);
				iLocal_47 = 1;
			}
			else
			{
				iLocal_47 = 3;
			}
		}
	}

	if (!unk_0x4FAFF4BCB7633475(uLocal_52))
	{
		if (bLocal_59)
			if (!unk_0x3EE1A6D743332852(unk_0x259BE71D8A81D4FA(), 0))
				func_178();
	
		unk_0x3EE1A6D743332852(unk_0x259BE71D8A81D4FA(), 0);
		num = 0;
	
		if (unk_0xFC8BFE4B41177C22(uLocal_54))
		{
			if (!unk_0x4FAFF4BCB7633475(uLocal_54))
			{
				if (unk_0x9B3D4335E0EDB0BE(uLocal_54, unk_0x4A8C381C258A124D(), 1) && unk_0x8D91ADE44AC79BC9(uLocal_54) < 190 || unk_0x9B3D4335E0EDB0BE(uLocal_54, unk_0x4A8C381C258A124D(), 1) && unk_0x8BF5256C439DF778(uLocal_54))
					num = 1;
			
				if (unk_0xFC8BFE4B41177C22(uLocal_55) && unk_0x9B3D4335E0EDB0BE(uLocal_55, unk_0x4A8C381C258A124D(), 1))
					num = 1;
			}
			else
			{
				num = 1;
			}
		}
	
		if (unk_0x9B3D4335E0EDB0BE(uLocal_52, unk_0x4A8C381C258A124D(), 1) && unk_0x8D91ADE44AC79BC9(uLocal_52) < 190 || func_62() || unk_0x3EE1A6D743332852(unk_0x259BE71D8A81D4FA(), 0) || func_61() || unk_0x9B3D4335E0EDB0BE(uLocal_52, unk_0x4A8C381C258A124D(), 1) && unk_0x8BF5256C439DF778(uLocal_52) || num == 1)
		{
			func_64(1);
		
			if (iLocal_47 == 3 || iLocal_47 == 1)
				iLocal_47 = 1;
			else
				iLocal_47 = 2;
		}
	}
	else
	{
		if (unk_0x9B3D4335E0EDB0BE(uLocal_52, unk_0x4A8C381C258A124D(), 1))
		{
			if (bLocal_285 == false)
			{
				bLocal_285 = true;
				uLocal_288 = unk_0x1DD05E817C89C737();
			}
		
			if (iLocal_47 == 3 || iLocal_47 == 1)
				iLocal_47 = 1;
			else
				iLocal_47 = 2;
		}
	
		if (!unk_0x4FAFF4BCB7633475(uLocal_53))
			unk_0xE4DC7B3DD712372B(uLocal_53);
	
		if (unk_0xC450B06E5AAA0985(iLocal_56))
		{
			unk_0xFE54B8568B2ABD12(&iLocal_56);
			func_63(&uLocal_76, 1);
		}
	}

	if (bLocal_285)
	{
		if (unk_0x1DD05E817C89C737() - uLocal_288 > 5000)
		{
			unk_0xF165EAD0AA08F3B6(unk_0x259BE71D8A81D4FA(), 3, 0);
			unk_0x42C9A22D6724F283(unk_0x259BE71D8A81D4FA(), 0);
			bLocal_285 = false;
		}
	}

	if (!unk_0x4FAFF4BCB7633475(uLocal_52) && !unk_0x4FAFF4BCB7633475(uLocal_53))
	{
		if (unk_0x9B3D4335E0EDB0BE(uLocal_52, unk_0x4A8C381C258A124D(), 1) && unk_0x9B3D4335E0EDB0BE(uLocal_53, unk_0x4A8C381C258A124D(), 1))
		{
			if (unk_0x8D91ADE44AC79BC9(uLocal_52) < 190 && unk_0x8D91ADE44AC79BC9(uLocal_53) < 190)
			{
				func_68();
				func_64(1);
				iLocal_47 = 1;
			}
		}
	
		if (!unk_0xFAA48325A90263BE(uLocal_53, uLocal_52, 110f, 110f, 40f, 0, 1, 0))
		{
			if (unk_0xFAA48325A90263BE(unk_0x4A8C381C258A124D(), uLocal_52, 20f, 20f, 20f, 0, 1, 0))
			{
				func_59();
				SYSTEM::WAIT(0);
				_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PORUN", 4, iLocal_252, 0, 0);
			}
			else if (unk_0xFAA48325A90263BE(unk_0x4A8C381C258A124D(), uLocal_53, 40f, 40f, 40f, 0, 1, 0))
			{
				func_59();
				SYSTEM::WAIT(0);
				_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_265, 4, iLocal_252, 0, 0);
			}
		
			func_178();
		}
	}

	if (unk_0x4FAFF4BCB7633475(uLocal_52) && unk_0x4FAFF4BCB7633475(uLocal_53))
		func_178();

	if (unk_0x4FAFF4BCB7633475(uLocal_53))
		if (!unk_0x4FAFF4BCB7633475(uLocal_52))
			if (!unk_0xFAA48325A90263BE(unk_0x4A8C381C258A124D(), uLocal_52, 100f, 100f, 40f, 0, 1, 0))
				func_178();

	return;
}

void func_59() // Position - 0x379C
{
	Global_21291 = 0;
	func_60();
	return;
}

void func_60() // Position - 0x37AC
{
	unk_0xC78B293A5F4EACF9();
	Global_23436 = 0;

	if (unk_0xE87F28FD4128D063() || Global_21066.f_1 == 9 || Global_21065 == 1)
	{
		unk_0x0F15249D24BC5ADA(0);
		Global_22425 = 6;
		Global_21066.f_1 = 3;
		return;
	}

	if (unk_0x1F9EB85925C3ECD7())
	{
		unk_0x0F15249D24BC5ADA(1);
		Global_22425 = 6;
		return;
	}

	return;
}

BOOL func_61() // Position - 0x3803
{
	BOOL flag;

	flag = false;

	if (unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
	{
		if (unk_0x4B423FAA24E8ABF0(unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0)) == joaat("bulldozer"))
			if (!unk_0x4FAFF4BCB7633475(uLocal_52))
				if (unk_0x685171EED42BC4DF(unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0), uLocal_52))
					flag = true;
	
		if (unk_0xD9F5E1FEFD1329E4(uLocal_55, 0))
			if (unk_0x685171EED42BC4DF(unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0), uLocal_55))
				flag = true;
		else if (bLocal_75)
			if (unk_0x9B3D4335E0EDB0BE(uLocal_55, unk_0x4A8C381C258A124D(), 1))
				flag = true;
	}

	flag;
	return flag;
}

BOOL func_62() // Position - 0x388D
{
	var unk;
	var unk4;
	int num;

	unk = { 3f, 3f, 3f };
	unk4 = { -3f, -3f, -3f };

	if (unk_0x398315D0C90DE6F6(unk_0x83FDC027F0BEA202(uLocal_52, 31086, 0f, 0f, 0f), 0.5f, 1))
		if (unk_0x4FAFF4BCB7633475(uLocal_53))
			return true;
		else if (!unk_0x398315D0C90DE6F6(unk_0x83FDC027F0BEA202(uLocal_53, 31086, 0f, 0f, 0f), 2f, 1))
			return true;

	if (unk_0x5713DE6DB59E98EB(unk_0x83FDC027F0BEA202(uLocal_52, 31086, 0f, 0f, 0f), 3f))
		return true;

	if (unk_0xFAA48325A90263BE(unk_0x4A8C381C258A124D(), uLocal_52, 5f, 5f, 5f, 0, 1, 0))
	{
		if (!unk_0x4FAFF4BCB7633475(uLocal_52))
			if (unk_0x41754DC6001F6313(unk_0x259BE71D8A81D4FA(), uLocal_52) || unk_0xE5D813FA6F741B01(unk_0x259BE71D8A81D4FA(), uLocal_52))
				if (unk_0x23B29877D0BE9547(unk_0x4A8C381C258A124D(), &num, 1))
					if (num != joaat("WEAPON_UNARMED"))
						return true;
	
		if (!unk_0x4FAFF4BCB7633475(uLocal_53))
			if (unk_0x41754DC6001F6313(unk_0x259BE71D8A81D4FA(), uLocal_53) || unk_0xE5D813FA6F741B01(unk_0x259BE71D8A81D4FA(), uLocal_53))
				if (unk_0x23B29877D0BE9547(unk_0x4A8C381C258A124D(), &num, 1))
					if (num != joaat("WEAPON_UNARMED"))
						if (bLocal_72)
							return true;
	}

	unk = { unk + unk_0x83FDC027F0BEA202(uLocal_52, 31086, 0f, 0f, 0f) };
	unk4 = { unk4 + unk_0x83FDC027F0BEA202(uLocal_52, 31086, 0f, 0f, 0f) };

	if (unk_0x7A082DC02E5E00C1(unk4, unk, joaat("WEAPON_SMOKEGRENADE"), 1) || unk_0x7A082DC02E5E00C1(unk4, unk, joaat("WEAPON_GRENADE"), 1) || unk_0x7A082DC02E5E00C1(unk4, unk, joaat("WEAPON_STICKYBOMB"), 1) || unk_0x7A082DC02E5E00C1(unk4, unk, joaat("VEHICLE_WEAPON_TANK"), 1))
		return true;

	return false;
}

void func_63(var uParam0, int iParam1) // Position - 0x3A34
{
	if (uParam0->[iParam1 /*10*/].f_7 == 1)
		uParam0->[iParam1 /*10*/].f_7 = 0;

	return;
}

void func_64(int iParam0) // Position - 0x3A51
{
	if (!unk_0x4FAFF4BCB7633475(uLocal_52))
	{
		unk_0xE4DC7B3DD712372B(uLocal_52);
	
		if (!bLocal_59)
		{
			unk_0xF165EAD0AA08F3B6(unk_0x259BE71D8A81D4FA(), 2, 0);
			unk_0x42C9A22D6724F283(unk_0x259BE71D8A81D4FA(), 0);
			unk_0x3F58BFCF656F0DF1(uLocal_52, 1);
		
			if (unk_0xC450B06E5AAA0985(iLocal_56))
				unk_0x6A52036D51C7E18E(iLocal_56, 0);
		
			if (iParam0 == 1)
			{
				if (!unk_0x4FAFF4BCB7633475(uLocal_53))
				{
					if (unk_0x9B5C1660CCDF7189(uLocal_53, joaat("SCRIPT_TASK_SMART_FLEE_PED")) == 7)
					{
						if (unk_0x13CCB1AD131C1082(uLocal_53, sLocal_256, "kneeling_arrest_idle", 3))
						{
							unk_0xB5396F1FB088FE38(&uLocal_71);
							unk_0x10425721983AE158(0, sLocal_256, "kneeling_arrest_escape", 8f, -8f, -1, 4096, 0, 0, 0, 0);
							unk_0xD844F5E50DAB6FF7(0, uLocal_52, 150f, -1, 0, 0);
							unk_0x93C0674FC00824D0(uLocal_71);
							unk_0x4BD42B0527065BB6(uLocal_53, uLocal_71);
							unk_0xD0557B139A542F12(&uLocal_71);
							unk_0x717804C8C8DA67BF(uLocal_53, joaat("MotionState_Run"), 0, 0, 0);
							unk_0x44FB298D6382876D(uLocal_53, 1);
						}
						else
						{
							unk_0x974022927CB47E68(uLocal_53);
							unk_0xD844F5E50DAB6FF7(uLocal_53, uLocal_52, 150f, -1, 0, 0);
						}
					}
				}
			
				if (!unk_0x1C2F771CDC87A3A5(uLocal_53, 0) && !unk_0x4FAFF4BCB7633475(uLocal_53) && !unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
					if (unk_0xED977E2AE2CB16EE(unk_0xD1A6A821F5AC81DB(uLocal_53, 1), unk_0xD1A6A821F5AC81DB(unk_0x4A8C381C258A124D(), 1), 1) < 20f)
						if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
							_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_271, 4, iLocal_252, 0, 0);
			}
		
			unk_0x974022927CB47E68(uLocal_52);
			unk_0xB5396F1FB088FE38(&uLocal_71);
			unk_0xE67051907958B5EB(0, unk_0x4A8C381C258A124D(), -1, 2048, 2);
			unk_0x62A5310368A20EFA(0, unk_0x4A8C381C258A124D(), 0, 16);
			unk_0x93C0674FC00824D0(uLocal_71);
			unk_0x4BD42B0527065BB6(uLocal_52, uLocal_71);
			unk_0xD0557B139A542F12(&uLocal_71);
			unk_0x44FB298D6382876D(uLocal_52, 1);
			unk_0x87EDE48547CC8942(uLocal_52, 1, 0);
		
			if (unk_0xD9F5E1FEFD1329E4(uLocal_55, 0))
				unk_0x0B74F181ADFC39BF(uLocal_55, 1);
		
			func_66();
			SYSTEM::WAIT(0);
			SYSTEM::WAIT(0);
		
			if (!unk_0x4FAFF4BCB7633475(uLocal_52))
				!unk_0x8BF5256C439DF778(uLocal_52);
		
			if (unk_0xFC8BFE4B41177C22(uLocal_54))
				func_65();
		
			bLocal_59 = true;
		}
	}

	return;
}

void func_65() // Position - 0x3C28
{
	if (unk_0xFC8BFE4B41177C22(uLocal_54))
	{
		if (!unk_0x4FAFF4BCB7633475(uLocal_54))
		{
			unk_0xB5396F1FB088FE38(&uLocal_71);
			unk_0xE67051907958B5EB(0, unk_0x4A8C381C258A124D(), -1, 2048, 2);
			unk_0x62A5310368A20EFA(0, unk_0x4A8C381C258A124D(), 0, 16);
			unk_0x93C0674FC00824D0(uLocal_71);
			unk_0x4BD42B0527065BB6(uLocal_54, uLocal_71);
			unk_0xD0557B139A542F12(&uLocal_71);
			unk_0x44FB298D6382876D(uLocal_54, 1);
			unk_0x87EDE48547CC8942(uLocal_54, 1, 0);
		}
	}

	return;
}

void func_66() // Position - 0x3C86
{
	Global_21291 = 0;
	func_67();
	return;
}

void func_67() // Position - 0x3C96
{
	unk_0xC78B293A5F4EACF9();
	Global_23436 = 0;

	if (unk_0x1F9EB85925C3ECD7())
	{
		unk_0x0F15249D24BC5ADA(0);
		Global_22425 = 6;
	}

	return;
}

void func_68() // Position - 0x3CB7
{
	if (!unk_0x4FAFF4BCB7633475(uLocal_53))
	{
		if (unk_0xC450B06E5AAA0985(iLocal_57))
			unk_0x6A52036D51C7E18E(iLocal_57, 0);
	
		unk_0xE4DC7B3DD712372B(uLocal_53);
	}

	return;
}

BOOL func_69() // Position - 0x3CDF
{
	if (!unk_0x1C2F771CDC87A3A5(uLocal_52, 0))
		if (bLocal_72)
			if (unk_0x685171EED42BC4DF(unk_0x4A8C381C258A124D(), uLocal_52))
				if (unk_0x1DD05E817C89C737() - uLocal_274 > 1000)
					func_70();

	if (iLocal_273 >= 3)
		return true;

	return false;
}

void func_70() // Position - 0x3D22
{
	if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
	{
		if (iLocal_273 < 3)
		{
			unk_0xE67051907958B5EB(uLocal_52, unk_0x4A8C381C258A124D(), 6000, 2048, 2);
			_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PLEAVE", 4, iLocal_252, 0, 0);
		}
	}

	iLocal_273 = iLocal_273 + 1;
	uLocal_274 = unk_0x1DD05E817C89C737();
	return;
}

int func_71() // Position - 0x3D6B
{
	if (unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
		if (unk_0x685171EED42BC4DF(unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0), uLocal_53))
			if (bLocal_72)
				if (iLocal_50 != 7)
					if (SYSTEM::VDIST(unk_0xD1A6A821F5AC81DB(uLocal_53, 1), uLocal_66) > 2.5f)
						return 1;
				else
					return 1;
			else
				return 1;

	if (bLocal_72)
		if (unk_0x685171EED42BC4DF(unk_0x4A8C381C258A124D(), uLocal_53))
			if (unk_0x8BF5256C439DF778(uLocal_53))
				if (iLocal_50 != 7)
					if (SYSTEM::VDIST(unk_0xD1A6A821F5AC81DB(uLocal_53, 1), uLocal_66) > 2.5f)
						return 1;
				else
					return 1;
			else if (bLocal_72)
				if (unk_0x1DD05E817C89C737() - uLocal_274 > 1000)
					func_70();

	if (iLocal_273 >= 3)
		return 1;

	return 0;
}

void func_72() // Position - 0x3E30
{
	if (!bLocal_73)
	{
		switch (iLocal_243)
		{
			case 0:
				if (!bLocal_59)
				{
					if (SYSTEM::TIMERA() > 80000)
					{
						if (!unk_0x4FAFF4BCB7633475(uLocal_52))
						{
							if (!unk_0x4FAFF4BCB7633475(uLocal_53))
							{
								func_59();
								SYSTEM::WAIT(0);
								_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PRAD3", 4, iLocal_252, 0, 0);
							}
						}
					
						iLocal_243 = 1;
					}
				}
				break;
		
			case 1:
				if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
				{
					if (!unk_0x4FAFF4BCB7633475(uLocal_53))
					{
						if (unk_0x13CCB1AD131C1082(uLocal_53, sLocal_256, "kneeling_arrest_idle", 3))
						{
							unk_0xB5396F1FB088FE38(&uLocal_71);
							unk_0x10425721983AE158(0, sLocal_256, "kneeling_arrest_escape", 8f, -8f, -1, 4096, 0, 0, 0, 0);
							unk_0xD844F5E50DAB6FF7(0, uLocal_52, 150f, -1, 0, 0);
							unk_0x93C0674FC00824D0(uLocal_71);
							unk_0x4BD42B0527065BB6(uLocal_53, uLocal_71);
							unk_0xD0557B139A542F12(&uLocal_71);
							unk_0x717804C8C8DA67BF(uLocal_53, joaat("MotionState_Run"), 0, 0, 0);
							unk_0x44FB298D6382876D(uLocal_53, 1);
						}
						else
						{
							unk_0xD844F5E50DAB6FF7(uLocal_53, uLocal_52, 150f, -1, 0, 0);
						}
					
						unk_0x44FB298D6382876D(uLocal_53, 1);
						unk_0xD25E9BDC14A0B649(uLocal_53, 101, 0, 0);
						_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_267, 4, iLocal_252, 0, 0);
						iLocal_243 = 2;
					}
				
					if (!unk_0x4FAFF4BCB7633475(uLocal_52))
					{
						unk_0x974022927CB47E68(uLocal_52);
						iLocal_243 = 2;
					}
				}
				break;
		
			case 2:
				if (!unk_0x4FAFF4BCB7633475(uLocal_52))
				{
					if (!unk_0x4FAFF4BCB7633475(uLocal_53))
					{
						unk_0xD414C47AFF81382A(2, uLocal_70, uLocal_69);
						unk_0xD414C47AFF81382A(2, uLocal_69, uLocal_70);
						unk_0x62A5310368A20EFA(uLocal_52, uLocal_53, 0, 16);
					}
					else
					{
						unk_0x761F56E633460973(uLocal_52, 1193033728, 0);
						func_178();
					}
				
					unk_0x44FB298D6382876D(uLocal_52, 1);
				}
			
				bLocal_73 = true;
				break;
		}
	}

	return;
}

int func_73(var uParam0, var uParam1) // Position - 0x3FBD
{
	var unk;
	var unk4;
	var unk7;
	Vector3 vector;
	Vector3 vector2;

	if (unk_0xD9F5E1FEFD1329E4(uParam1, 0))
	{
		if (!unk_0x4FAFF4BCB7633475(uParam0))
			unk = { unk_0xD1A6A821F5AC81DB(uParam0, 1) };
		else
			return -1;
	
		unk4 = { unk_0x0D1381B6E0F3987D(uParam1, 1f, -0.5f, 0f) };
		vector = { unk4 - unk };
		unk7 = { unk_0x0D1381B6E0F3987D(uParam1, -1f, -0.5f, 0f) };
		vector2 = { unk7 - unk };
	
		if (SYSTEM::VMAG(vector) < SYSTEM::VMAG(vector2))
			return 2;
		else
			return 1;
	}

	return -1;
}

void func_74() // Position - 0x404C
{
	if (!_CONVERSATION_IS_DIALOGUE_IN_PROGRESS())
	{
		if (!bLocal_291)
		{
			if (unk_0xFAA48325A90263BE(unk_0x4A8C381C258A124D(), uLocal_52, 30f, 30f, 30f, 0, 1, 0))
			{
				_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PSTOP", 4, iLocal_252, 0, 0);
				bLocal_291 = true;
				SYSTEM::SETTIMERB(0);
			}
		}
		else if (SYSTEM::TIMERB() > 8000)
		{
			if (unk_0xC5935DFB3F39785A(0, 99) > 50)
				_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", "REARR_PRAND", 4, iLocal_252, 0, 0);
			else if (unk_0xFAA48325A90263BE(unk_0x4A8C381C258A124D(), uLocal_53, 10f, 10f, 10f, 0, 1, 0))
				_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_266, 4, iLocal_252, 0, 0);
			else if (unk_0xFAA48325A90263BE(unk_0x4A8C381C258A124D(), uLocal_52, 10f, 10f, 10f, 0, 1, 0))
				_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_266, 4, iLocal_252, 0, 0);
			else
				_CONVERSATION_ADD_LINE(&uLocal_76, "REARRAU", sLocal_270, 4, iLocal_252, 0, 0);
		
			SYSTEM::SETTIMERB(0);
		}
	}

	return;
}

BOOL _CONVERSATION_ADD_LINE(var uParam0, char* sParam1, char* sParam2, int iParam3, int iParam4, int iParam5, int iParam6) // Position - 0x414C
{
	func_89(uParam0, 145, sParam1, iParam4, iParam5, iParam6);

	if (iParam3 > 7)
		if (iParam3 < 12)
			iParam3 = 7;

	Global_22432 = 0;
	Global_22434 = 0;
	Global_22439 = 0;
	Global_23416 = 0;
	Global_23418 = 0;
	Global_23422 = 0;
	Global_2883585 = 0;
	return func_76(sParam2, iParam3, false);
}

BOOL func_76(char* sParam0, int iParam1, BOOL bParam2) // Position - 0x419A
{
	Global_22426 = 0;

	if (Global_22425 == 0 || Global_22427 == 2)
	{
		if (Global_22425 != 0)
		{
			if (iParam1 > Global_22427)
			{
				if (Global_22432 == 0)
				{
					unk_0x0F15249D24BC5ADA(0);
					Global_21066.f_1 = 3;
					Global_22425 = 0;
					Global_22426 = 1;
					Global_22478 = 0;
					Global_22421 = 0;
					Global_22422 = 0;
					Global_22436 = 0;
					Global_22435 = false;
					Global_21065 = 0;
				}
				else
				{
					func_60();
					return 0;
				}
			}
			else
			{
				return 0;
			}
		}
	
		if (unk_0x1F9EB85925C3ECD7())
			return 0;
	
		if (func_88(8, -1))
			return 0;
	
		Global_22501 = { Global_22495 };
		func_87();
		Global_21714 = { Global_21879 };
		Global_22431 = Global_22432;
		Global_22438 = Global_22439;
		Global_2883586 = Global_2883585;
		Global_22440 = { Global_22456 };
		Global_22433 = Global_22434;
		Global_23415 = Global_23416;
		Global_23423 = { Global_23429 };
		Global_23417 = Global_23418;
		Global_23419 = Global_23420;
		Global_23421 = Global_23422;
		Global_22044.f_370 = Global_23414;
		Global_22044.f_368 = Global_23412;
		Global_22044.f_369 = Global_23413;
		Global_22421 = Global_22422;
	
		if (Global_22431)
		{
			unk_0x8744D2E3FC95740E(&Global_8919, 20);
			unk_0x8744D2E3FC95740E(&Global_8920, 17);
			unk_0x8744D2E3FC95740E(&Global_8921, 0);
		
			if (bParam2)
			{
				_UPDATE_CURRENT_PLAYER_CHARACTER();
			
				if (Global_9741[Global_21066 /*2811*/][0 /*281*/].f_259 == 2)
				{
					if (iParam1 == 13)
					{
					}
					else
					{
						return 0;
					}
				}
			
				if (Global_21066.f_1 > 3)
					return 0;
			}
		
			if (Global_21032 == 1)
				return 0;
		
			if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
			{
				if (unk_0xFFAC548682B3D56E(unk_0x4A8C381C258A124D()))
					return 0;
			
				if (func_85())
					return 0;
			
				if (unk_0x6F21822F60DC0945(unk_0x4A8C381C258A124D()))
					return 0;
			
				if (unk_0x8BF5256C439DF778(unk_0x4A8C381C258A124D()))
					return 0;
			
				if (unk_0x333554781C7582D7(unk_0x4A8C381C258A124D()))
					return 0;
			
				if (unk_0x08DAD6B08438A17C(unk_0x4A8C381C258A124D(), joaat("GADGET_PARACHUTE")))
					return 0;
			
				if (!Global_79539)
				{
					if (unk_0x69799E0840A34AFB(unk_0x4A8C381C258A124D()))
						return 0;
				
					if (unk_0xB96BD3A32937D44E(unk_0x259BE71D8A81D4FA()))
						return 0;
				
					if (unk_0x74CD4FE549433E92(unk_0x4A8C381C258A124D()))
						return 0;
				
					if (unk_0xFD2D7766E325A3B2(unk_0x259BE71D8A81D4FA(), 0))
						return 0;
				}
			}
		
			if (func_84())
			{
				return 0;
			}
			else
			{
				switch (Global_21066.f_1)
				{
					case 7:
						return 0;
				
					case 8:
						return 0;
				
					case 9:
						break;
				
					case 10:
						break;
				
					default:
						break;
				}
			
				if (IS_BIT_SET(Global_8919, 9))
					return 0;
			}
		
			func_83();
			Global_22435 = bParam2;
		}
	
		Global_22427 = iParam1;
		TEXT_LABEL_ASSIGN_STRING(&Global_22044, sParam0, 24);
		Global_21291 = 0;
		func_82();
		func_77();
		return 1;
	}

	if (Global_22425 == 5)
		return 0;

	if (iParam1 < Global_22427 || iParam1 == Global_22427)
		return 0;

	if (iParam1 == 2)
	{
	}
	else
	{
		func_60();
	}

	return 0;
}

void func_77() // Position - 0x4468
{
	if (!func_78())
		return;

	if (Global_22431)
	{
		TEXT_LABEL_COPY(&(Global_1972230.f_1), { Global_22044 }, 4);
		Global_1972230 = Global_8234;
		Global_1972230.f_6 = Global_22435;
	}

	return;
}

BOOL func_78() // Position - 0x449F
{
	if (!Global_262145.f_28397)
		return false;

	if (!Global_79539)
		return false;

	if (unk_0x259BE71D8A81D4FA() == _INVALID_PLAYER_INDEX())
		return false;

	if (func_79(unk_0x259BE71D8A81D4FA()))
		return false;

	if (IS_BIT_SET(Global_1887549[unk_0x259BE71D8A81D4FA() /*611*/].f_1, 7))
		return false;

	if (unk_0x834C960822A4683F())
		return false;

	return true;
}

BOOL func_79(int iParam0) // Position - 0x4502
{
	return func_80(iParam0, 20);
}

BOOL func_80(int iParam0, int iParam1) // Position - 0x4512
{
	return IS_BIT_SET(Global_1887549[iParam0 /*611*/].f_10.f_4, iParam1);
}

Player _INVALID_PLAYER_INDEX() // Position - 0x452A
{
	return -1;
}

void func_82() // Position - 0x4533
{
	int i;

	for (i = 0; i <= 69; i = i + 1)
	{
		TEXT_LABEL_ASSIGN_STRING(&Global_21293[i /*6*/], "", 24);
	}

	unk_0x0F15249D24BC5ADA(0);
	Global_22425 = 1;
	return;
}

void func_83() // Position - 0x4565
{
	Global_22478 = Global_22477;
	Global_22472 = Global_22473;
	Global_22519 = { Global_22507 };
	Global_22525 = { Global_22513 };
	Global_22480 = Global_22479;
	Global_22549 = { Global_22531 };
	Global_22555 = { Global_22537 };
	Global_22561 = { Global_22543 };
	Global_22567 = { Global_22573 };
	Global_8234 = Global_8235;
	Global_8236 = Global_8237;
	Global_22436 = Global_22437;
	Global_22438 = Global_22439;
	Global_22440 = { Global_22456 };
	Global_22429 = Global_22430;
	Global_23441 = 0;
	Global_22474 = 0;
	Global_22475 = 0;
	unk_0x8744D2E3FC95740E(&Global_8920, 16);
	return;
}

BOOL func_84() // Position - 0x45FA
{
	if (Global_21066.f_1 == 1 || Global_21066.f_1 == 0)
		return true;

	return false;
}

BOOL func_85() // Position - 0x4621
{
	int num;
	int num2;

	if (Global_79539)
	{
		num = 0;
		unk_0x23B29877D0BE9547(unk_0x4A8C381C258A124D(), &num2, 1);
	
		if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
			if (num2 == joaat("WEAPON_SNIPERRIFLE") || num2 == joaat("WEAPON_HEAVYSNIPER") || num2 == joaat("WEAPON_REMOTESNIPER"))
				num = 1;
	
		if (unk_0x196704C916969409() && num == 1)
			return true;
		else
			return false;
	}

	if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
		if (unk_0x49F8918E2DC5FFBC(unk_0x4A8C381C258A124D(), 78, 1))
			return true;
		else
			return false;

	return true;
}

void _UPDATE_CURRENT_PLAYER_CHARACTER() // Position - 0x46BA
{
	if (func_45(14))
	{
		if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
			if (unk_0x4B423FAA24E8ABF0(unk_0x4A8C381C258A124D()) == Global_114135.f_28055[0 /*29*/])
				Global_21066 = CHAR_MICHAEL;
			else if (unk_0x4B423FAA24E8ABF0(unk_0x4A8C381C258A124D()) == Global_114135.f_28055[1 /*29*/])
				Global_21066 = CHAR_FRANKLIN;
			else if (unk_0x4B423FAA24E8ABF0(unk_0x4A8C381C258A124D()) == Global_114135.f_28055[2 /*29*/])
				Global_21066 = CHAR_TREVOR;
			else
				Global_21066 = CHAR_MICHAEL;
	}
	else
	{
		Global_21066 = _GET_CURRENT_PLAYER_CHARACTER();
	
		if (Global_21066 == _CHAR_NULL)
			Global_21066 = CHAR_MULTIPLAYER;
	
		if (Global_79539)
			Global_21066 = CHAR_MULTIPLAYER;
	
		if (Global_21066 > CHAR_MULTIPLAYER)
			Global_21066 = CHAR_MULTIPLAYER;
	}

	return;
}

void func_87() // Position - 0x475C
{
	int i;

	for (i = 0; i <= 15; i = i + 1)
	{
		Global_21714[i /*10*/] = 0;
		TEXT_LABEL_ASSIGN_STRING(&(Global_21714[i /*10*/].f_1), "", 24);
		Global_21714[i /*10*/].f_7 = 0;
		Global_21714[i /*10*/].f_8 = 0;
	}

	Global_21714.f_161 = -99;
	Global_21714.f_162 = { 0f, 0f, 0f };
	return;
}

BOOL func_88(int iParam0, int iParam1) // Position - 0x47B4
{
	switch (iParam0)
	{
		case 5:
			if (iParam1 > -1)
				return Global_1673654.f_203[iParam1];
			break;
	}

	return IS_BIT_SET(Global_1673654.f_1048, iParam0);
}

void func_89(var uParam0, int iParam1, char* sParam2, int iParam3, int iParam4, int iParam5) // Position - 0x47EC
{
	Global_21879 = { *uParam0 };
	Global_8235 = iParam1;
	TEXT_LABEL_ASSIGN_STRING(&Global_22495, sParam2, 24);
	Global_23414 = iParam5;

	if (iParam3 == 0)
	{
		Global_23412 = 1;
		Global_23410 = 0;
	}
	else
	{
		Global_23412 = 0;
		Global_23410 = 1;
	}

	if (iParam4 == 0)
	{
		Global_23413 = 1;
		Global_23411 = 0;
	}
	else
	{
		Global_23413 = 0;
		Global_23411 = 1;
	}

	return;
}

BOOL _CONVERSATION_IS_DIALOGUE_IN_PROGRESS() // Position - 0x4842
{
	if (Global_22425 != 0 || unk_0x1F9EB85925C3ECD7())
		return true;

	return false;
}

BOOL func_91() // Position - 0x4864
{
	if (unk_0xFAA48325A90263BE(uLocal_53, uLocal_52, 3.5f, 3.5f, 3.5f, 0, 1, 0) || func_94())
		if (!func_92(1063675494))
			return 1;

	return 0;
}

BOOL func_92(int iParam0) // Position - 0x48A5
{
	var unk;
	var unk2;

	if (unk_0x1C23EBEE3AABD4C7(unk_0xD1A6A821F5AC81DB(uLocal_53, 1), &unk, &unk2))
		if (func_93(unk2, 0f, 0f, 1f) < iParam0)
			return true;

	return false;
}

float func_93(float fParam0, var uParam1, var uParam2, float fParam3, float fParam4, float fParam5) // Position - 0x48D5
{
	return (fParam0 * fParam3) + (fParam0.f_1 * fParam3.f_1) + (fParam0.f_2 * fParam3.f_2);
}

int func_94() // Position - 0x48F6
{
	return 0;
}

BOOL _DOES_ENTITY_EXIST_AND_IS_ALIVE(var uParam0) // Position - 0x48FF
{
	if (unk_0xFC8BFE4B41177C22(uParam0))
		if (!unk_0x1C2F771CDC87A3A5(uParam0, 0))
			return true;

	return false;
}

void func_96() // Position - 0x4920
{
	if (unk_0x1DD05E817C89C737() - uLocal_251 > 500)
	{
		if (iLocal_47 != 1 && iLocal_47 != 3 && iLocal_47 != 2)
		{
			if (unk_0xC450B06E5AAA0985(iLocal_57))
			{
				unk_0x6A52036D51C7E18E(iLocal_57, iLocal_246);
			
				if (iLocal_246 == 1)
					iLocal_246 = 0;
				else
					iLocal_246 = 1;
			}
		}
		else
		{
			bLocal_286 = 1;
		}
	
		if (!bLocal_59)
		{
			if (unk_0xC450B06E5AAA0985(iLocal_56))
			{
				unk_0x6A52036D51C7E18E(iLocal_56, iLocal_247);
			
				if (iLocal_247 == 1)
					iLocal_247 = 0;
				else
					iLocal_247 = 1;
			}
		}
		else
		{
			bLocal_286 = 1;
		}
	
		uLocal_251 = unk_0x1DD05E817C89C737();
	}

	if (!(bLocal_287 == bLocal_286))
	{
		if (bLocal_286)
		{
			if (unk_0xC450B06E5AAA0985(iLocal_57))
				unk_0x89FE619BFBB2024B(iLocal_57, 1);
		
			if (unk_0xC450B06E5AAA0985(iLocal_56))
				unk_0x89FE619BFBB2024B(iLocal_56, 1);
		}
		else
		{
			if (unk_0xC450B06E5AAA0985(iLocal_57))
				unk_0x89FE619BFBB2024B(iLocal_57, 0);
		
			if (unk_0xC450B06E5AAA0985(iLocal_56))
				unk_0x89FE619BFBB2024B(iLocal_56, 0);
		}
	
		bLocal_287 = bLocal_286;
	}

	return;
}

BOOL func_97(int iParam0) // Position - 0x4A02
{
	BOOL num;
	int num2;
	var unk;

	num = 0;
	unk = { iParam0, iParam0, 50f };

	if (unk_0x5105BE70DEF1F5FB(unk_0x4A8C381C258A124D(), uLocal_277, uLocal_280, 194.75f, 0, 1, 0))
		return true;

	if (!unk_0x4FAFF4BCB7633475(uLocal_52))
		if (unk_0xFAA48325A90263BE(uLocal_52, unk_0x4A8C381C258A124D(), unk, 0, 1, 0))
			num = 1;
		else
			num2 = num2 + 1;
	else
		num2 = num2 + 1;

	if (!unk_0x4FAFF4BCB7633475(uLocal_53))
		if (unk_0xFAA48325A90263BE(uLocal_53, unk_0x4A8C381C258A124D(), unk, 0, 1, 0))
			num = 1;
		else
			num2 = num2 + 1;
	else
		num2 = num2 + 1;

	if (num2 == 2)
		num = 0;

	return num;
}

int func_98(BOOL bParam0) // Position - 0x4AAC
{
	if (func_103())
	{
		Global_114125 = true;
		Global_114122 = unk_0x1DD05E817C89C737();
	
		if (func_102(Global_114124))
			func_99(0);
	
		unk_0xB51A8DA1DD0D2E4E(1, "RE_TITLE" /*Random Event*/);
	
		if (bParam0 && func_102(Global_114124))
			unk_0x2716F9D3A99349AB();
	
		return 1;
	}

	return 0;
}

void func_99(int iParam0) // Position - 0x4AFF
{
	switch (iParam0)
	{
		case 0:
			if (Global_114135.f_24999.f_2 < 3)
			{
				if (!unk_0x3273F980CC5E02CD())
				{
					_DISPLAY_HELP_TEXT(func_101(iParam0), -1);
					Global_114135.f_24999.f_2 = Global_114135.f_24999.f_2 + 1;
					unk_0x0B0C9A0F9AAEB7F0(&Global_114131, 0);
				}
			}
			break;
	
		case 1:
			if (!IS_BIT_SET(Global_114131, 1))
			{
				if (!unk_0x3273F980CC5E02CD())
				{
					_DISPLAY_HELP_TEXT(func_101(iParam0), -1);
					Global_114135.f_24999.f_3 = Global_114135.f_24999.f_3 + 1;
					unk_0x0B0C9A0F9AAEB7F0(&Global_114131, 1);
				}
			}
			break;
	
		case 2:
			if (!IS_BIT_SET(Global_114131, 2))
			{
				if (!unk_0x3273F980CC5E02CD())
				{
					_DISPLAY_HELP_TEXT(func_101(iParam0), -1);
					Global_114135.f_24999.f_4 = Global_114135.f_24999.f_4 + 1;
					unk_0x0B0C9A0F9AAEB7F0(&Global_114131, 2);
				}
			}
			break;
	}

	return;
}

void _DISPLAY_HELP_TEXT(char* text, int iParam1) // Position - 0x4BDA
{
	unk_0xAC98CA65AD9A3215(text);
	unk_0x5E01B6B1F460FE3F(0, 0, 1, iParam1);
	return;
}

char* func_101(int iParam0) // Position - 0x4BF1
{
	char* str;

	str = "";

	switch (iParam0)
	{
		case 0:
			str = "AM_H_REFS" /*Various events unfold across San Andreas daily. These events will become blipped on the Radar when nearby.*/;
			break;
	
		case 1:
			str = "RE_FLASHBLIP" /*Flashing blue and red blips indicate situations around San Andreas that you can choose to help with.*/;
			break;
	
		case 2:
			str = "RE_HANDOVER" /*If you retrieve a stolen item, you can choose to keep it or return it for a reward.*/;
			break;
	}

	return str;
}

BOOL func_102(int iParam0) // Position - 0x4C35
{
	switch (iParam0)
	{
		case 29:
		case 30:
		case 2:
		case 18:
			return false;
	}

	return true;
}

BOOL func_103() // Position - 0x4C64
{
	switch (func_104(&Global_33147, 0, 5, false, unk_0x8F76B2250AC806FA()))
	{
		case 1:
			return true;
	
		case 0:
			return true;
	}

	return false;
}

int func_104(var uParam0, int iParam1, int iParam2, BOOL bParam3, int iParam4) // Position - 0x4C9A
{
	int i;

	if (iParam1 == 7)
		return 0;

	if (!bParam3)
		if (Global_99141.f_44 == 1)
			return 2;

	if (iParam1 == 0)
	{
		if (func_108(0))
			return 0;
	
		Global_44145 = Global_44145 + 1;
		*uParam0 = Global_44145;
		unk_0x2B52F77101390E6F(unk_0xB6B621402486C3E4(), 0);
		Global_23831.f_5 = 0;
	
		if (iParam2 != 5)
			unk_0xB1495D79F52B9EEE(8);
	
		Global_44181 = iParam2;
		Global_44143 = *uParam0;
		Global_44144 = iParam4;
		Global_44142 = 0;
		return 1;
	}

	if (*uParam0 != -1)
	{
		if (Global_44142 > 0)
		{
			i = 0;
		
			for (i = 0; i < Global_44142; i = i + 1)
			{
				if (Global_44148[i /*4*/] == *uParam0)
					return 2;
			}
		}
		else if (Global_44143 == *uParam0)
		{
			return 1;
		}
	
		*uParam0 = -1;
	}

	if (*uParam0 == -1)
	{
		if (!_CAN_ENTER_FREEROAM_STATE(iParam2))
			return 0;
	
		if (Global_44142 == 8)
			return 0;
	
		Global_44145 = Global_44145 + 1;
		*uParam0 = Global_44145;
		Global_44148[Global_44142 /*4*/] = Global_44145;
		Global_44148[Global_44142 /*4*/].f_1 = iParam1;
		Global_44148[Global_44142 /*4*/].f_2 = iParam2;
		Global_44148[Global_44142 /*4*/].f_3 = 0;
		Global_44142 = Global_44142 + 1;
	
		if (iParam4 != 0)
			func_105(uParam0, iParam4);
	}

	return 2;
}

void func_105(var uParam0, int iParam1) // Position - 0x4DD1
{
	int i;

	if (Global_44142 == 0)
		return;

	if (*uParam0 == -1)
		return;

	i = 0;

	for (i = 0; i < Global_44142; i = i + 1)
	{
		if (Global_44148[i /*4*/] == *uParam0)
			Global_44148[i /*4*/].f_3 = iParam1;
	}

	*uParam0 = -1;
	return;
}

BOOL _CAN_ENTER_FREEROAM_STATE(int iParam0) // Position - 0x4E20
{
	return func_107(iParam0, Global_44181);
}

BOOL func_107(int iParam0, int iParam1) // Position - 0x4E31
{
	if (iParam1 == 15)
		return 1;

	if (iParam0 == 15)
		return 0;

	switch (iParam0)
	{
		case 16:
			switch (iParam1)
			{
				case 9:
				case 10:
				case 7:
				case 13:
				case 14:
					return 0;
			}
		
			return 1;
	
		case 0:
			switch (iParam1)
			{
				case 5:
				case 17:
					return 1;
			}
			break;
	
		case 2:
		case 3:
			switch (iParam1)
			{
				case 5:
				case 6:
				case 8:
				case 17:
					return 1;
			}
			break;
	
		case 4:
			if (iParam1 == 17)
				return 1;
			break;
	
		case 5:
			break;
	
		case 6:
		case 8:
			if (iParam1 == 5)
				return 1;
			break;
	
		case 7:
			if (iParam1 == 6)
				return 1;
			break;
	
		case 9:
			if (iParam1 == 5)
				return 1;
			break;
	
		case 10:
			switch (iParam1)
			{
				case 5:
				case 6:
				case 17:
					return 1;
			}
			break;
	
		case 11:
			if (iParam1 == 5)
				return 1;
			break;
	
		case 17:
			switch (iParam1)
			{
				case 17:
				case 12:
				case 5:
					return 1;
			}
			break;
	
		case 18:
		case 12:
			switch (iParam1)
			{
				case 5:
				case 6:
				case 8:
					return 1;
			}
			break;
	
		case 13:
			switch (iParam1)
			{
				case 5:
					return 1;
			}
			break;
	
		case 14:
			switch (iParam1)
			{
				case 5:
					return 1;
			}
			break;
	}

	return 0;
}

BOOL func_108(int iParam0) // Position - 0x5012
{
	if (Global_44181 == 15)
		return false;

	if (_CAN_ENTER_FREEROAM_STATE(iParam0))
		return false;

	return true;
}

BOOL func_109() // Position - 0x5034
{
	float num;

	num = 50f;

	if (!unk_0x4FAFF4BCB7633475(uLocal_52))
	{
		if (!unk_0x4FAFF4BCB7633475(uLocal_53))
		{
			if (unk_0xFAA48325A90263BE(unk_0x4A8C381C258A124D(), uLocal_52, num, num, num, 0, 1, 0) || unk_0xFAA48325A90263BE(unk_0x4A8C381C258A124D(), uLocal_53, num, num, num, 0, 1, 0) || unk_0x5105BE70DEF1F5FB(unk_0x4A8C381C258A124D(), uLocal_277, uLocal_280, 194.75f, 0, 1, 0))
			{
				if (!unk_0xC450B06E5AAA0985(iLocal_57))
					iLocal_57 = func_110(uLocal_53, false, 145);
			
				if (!unk_0xC450B06E5AAA0985(iLocal_56))
					iLocal_56 = func_110(uLocal_52, false, 145);
			
				unk_0x89FE619BFBB2024B(iLocal_57, 0);
				unk_0x89FE619BFBB2024B(iLocal_56, 0);
				SYSTEM::SETTIMERA(0);
				return true;
			}
		}
		else
		{
			if (!unk_0x4FAFF4BCB7633475(uLocal_52))
				if (!unk_0xC450B06E5AAA0985(iLocal_56))
					iLocal_56 = func_110(uLocal_52, false, 145);
		
			if (unk_0xC450B06E5AAA0985(iLocal_57))
				unk_0xFE54B8568B2ABD12(&iLocal_57);
		
			return true;
		}
	}
	else
	{
		if (!unk_0x4FAFF4BCB7633475(uLocal_53))
			if (!unk_0xC450B06E5AAA0985(iLocal_57))
				iLocal_57 = func_110(uLocal_53, false, 145);
	
		if (unk_0xC450B06E5AAA0985(iLocal_56))
			unk_0xFE54B8568B2ABD12(&iLocal_56);
	
		return true;
	}

	return false;
}

int func_110(var uParam0, BOOL bParam1, int iParam2) // Position - 0x5152
{
	int num;

	num = func_111(uParam0, !bParam1, false);

	if (iParam2 != 145 && unk_0xC450B06E5AAA0985(num) && unk_0x6BA487C862DB8DDF(&(Global_2201[iParam2 /*29*/].f_3)))
		unk_0x594D5D0D7071B0DE(num, &(Global_2201[iParam2 /*29*/].f_3));

	return num;
}

int func_111(var uParam0, BOOL bParam1, BOOL bParam2) // Position - 0x519C
{
	int num;

	if (!unk_0xFC8BFE4B41177C22(uParam0))
		return 0;

	num = unk_0x18E23E031A9B798F(uParam0);

	if (unk_0x55B80B6E7AB61270(uParam0))
	{
		unk_0x5D3946F818C6B331(num, unk_0x76CD105BCAC6EB9F() ? 1f : 1f);
	
		if (!bParam2)
			unk_0x6A52036D51C7E18E(num, bParam1);
		else
			unk_0x61183D6239A9D7B8(num, 2);
	}
	else if (unk_0x0101C509A6E67F99(uParam0))
	{
		unk_0x5D3946F818C6B331(num, unk_0x76CD105BCAC6EB9F() ? 0.7f : 0.7f);
		unk_0x6A52036D51C7E18E(num, bParam1);
	}
	else if (unk_0xBE79A96C521F4432(uParam0))
	{
		unk_0x5D3946F818C6B331(num, unk_0x76CD105BCAC6EB9F() ? 0.7f : 0.7f);
	}

	return num;
}

var func_112(BOOL bParam0, var uParam1, var uParam2) // Position - 0x5240
{
	if (bParam0)
		return uParam1;

	return uParam2;
}

BOOL func_113() // Position - 0x5257
{
	int num;
	int num2;
	var unk;
	var unk4;
	float num3;
	float num4;

	num = joaat("S_M_Y_Ranger_01");

	if (iLocal_51 == 1)
	{
		unk = { 2429.822f, 4952.938f, 44.8892f };
		num3 = 218.3434f;
		unk4 = { 2431.685f, 4945.31f, 44.6996f };
		num4 = 220.0087f;
		uLocal_277 = { 2378.2844f, 4863.027f, 32.508835f };
		uLocal_280 = { 2512.7974f, 4998.5864f, 90.12283f };
		sLocal_254 = "A_M_M_HillBilly_02_WHITE_MINI_02";
		sLocal_263 = "arrestcriminal";
		sLocal_264 = "REARR_CTHANK";
		sLocal_265 = "REARR_CORUN";
		sLocal_266 = "REARR_CWTF";
		sLocal_267 = "REARR_CTURN";
		sLocal_268 = "REARR_CHELP";
		sLocal_269 = "REARR_HATE";
		sLocal_270 = "REARR_CRAND";
		sLocal_271 = "REARR_CRAND";
		sLocal_272 = "REARR_CRAND";
		num2 = joaat("A_M_M_Hillbilly_02");
	}
	else if (iLocal_51 == 2)
	{
		unk = { 2592.637f, 2540.0503f, 30.4162f };
		num3 = 90.5639f;
		unk4 = { 2586.0051f, 2541.4102f, 31.0379f };
		num4 = 75.5943f;
		uLocal_277 = { 2543.1345f, 2683.9172f, 0.307461f };
		uLocal_280 = { 2597.6025f, 2408.885f, 121.90103f };
		sLocal_254 = "G_M_M_ArmGoon_01_White_Armenian_MINI_01";
		sLocal_263 = "arrestcriminal2";
		sLocal_264 = "REARR_THANK2";
		sLocal_265 = "REARR_CORUN2";
		sLocal_266 = "REARR_CWTF2";
		sLocal_267 = "REARR_CTURN2";
		sLocal_268 = "REARR_CHELP2";
		sLocal_269 = "REARR_HATE2";
		sLocal_270 = "REARR_CRAND2";
		sLocal_271 = "REARR_CRAND2";
		sLocal_272 = "REARR_CRAND2";
		num2 = joaat("G_M_M_ArmGoon_01");
	}

	unk_0xEC9DAA34BBB4658C(num);
	unk_0xEC9DAA34BBB4658C(num2);
	unk_0xEC9DAA34BBB4658C(joaat("prop_ld_binbag_01"));
	unk_0x80813AC549A1E8AE(sLocal_256);
	unk_0x80813AC549A1E8AE(sLocal_257);
	unk_0x73DEEAB0747FB17C("MOVE_M@BAIL_BOND_NOT_TAZERED");
	unk_0x73DEEAB0747FB17C("MOVE_M@BAIL_BOND_TAZERED");

	if (unk_0x6252BC0DD8A320DB(num) && unk_0x6252BC0DD8A320DB(num2) && unk_0x6252BC0DD8A320DB(joaat("prop_ld_binbag_01")) && unk_0xE100DD4F82A51BDE(sLocal_256) && unk_0xE100DD4F82A51BDE(sLocal_257) && unk_0x8175BC6D49412468("MOVE_M@BAIL_BOND_NOT_TAZERED") && unk_0x8175BC6D49412468("MOVE_M@BAIL_BOND_TAZERED"))
	{
		uLocal_52 = unk_0xB1DBFEB95C0EFB88(6, num, unk, num3, 1, 1);
		unk_0x0428AFDCAA63B06E(uLocal_52, 294, 1);
		unk_0x87EDE48547CC8942(uLocal_52, 1, 0);
		unk_0x1834D30866818A23(uLocal_52, 1);
		unk_0xAAA71DD7E9059338(uLocal_52, 1);
		_CONVERSATION_INITIALIZE_ACTOR(&uLocal_76, 1, uLocal_52, "ArrestCop", 0, 1);
		unk_0x788F35D395511DFE(uLocal_52, 1, 1);
		unk_0x570AAA413775DFFB(uLocal_52, 1);
		uLocal_53 = unk_0xB1DBFEB95C0EFB88(26, num2, unk4, num4, 1, 1);
		unk_0x87EDE48547CC8942(uLocal_53, 1, 0);
		unk_0x0428AFDCAA63B06E(uLocal_53, 42, 1);
		unk_0x0428AFDCAA63B06E(uLocal_53, 281, 1);
		unk_0xAAA71DD7E9059338(uLocal_53, 1);
		_CONVERSATION_INITIALIZE_ACTOR(&uLocal_76, 2, uLocal_53, sLocal_263, 0, 1);
		unk_0x788F35D395511DFE(uLocal_53, 1, 1);
	
		if (iLocal_51 == 1)
		{
			unk_0xD1C578C204015E1F(uLocal_53, 0, 0, 2, 0);
			unk_0xD1C578C204015E1F(uLocal_53, 2, 0, 0, 0);
			unk_0xD1C578C204015E1F(uLocal_53, 3, 2, 0, 0);
			unk_0xD1C578C204015E1F(uLocal_53, 4, 0, 0, 0);
		}
	
		func_114();
	
		if (_GET_CURRENT_PLAYER_CHARACTER() == CHAR_MICHAEL)
			_CONVERSATION_INITIALIZE_ACTOR(&uLocal_76, 0, unk_0x4A8C381C258A124D(), "MICHAEL", 0, 1);
		else if (_GET_CURRENT_PLAYER_CHARACTER() == CHAR_FRANKLIN)
			_CONVERSATION_INITIALIZE_ACTOR(&uLocal_76, 0, unk_0x4A8C381C258A124D(), "FRANKLIN", 0, 1);
		else if (_GET_CURRENT_PLAYER_CHARACTER() == CHAR_TREVOR)
			_CONVERSATION_INITIALIZE_ACTOR(&uLocal_76, 0, unk_0x4A8C381C258A124D(), "TREVOR", 0, 1);
	
		if (!unk_0x66B90BA528CFEBCE(uLocal_52, joaat("WEAPON_PISTOL"), 0))
			unk_0xB41DEC3AAC1AA107(uLocal_52, joaat("WEAPON_PISTOL"), -1, 0, 1);
	
		unk_0x3C0F448853B71C92(uLocal_52, joaat("WEAPON_PISTOL"), 1);
		uLocal_58 = unk_0x4E55EAB577C13329(joaat("prop_ld_binbag_01"), uLocal_63, uLocal_63.f_1, uLocal_63.f_2 + 100f, 1, 1, 0);
		unk_0x4285E11B28063EE0(uLocal_58, 0, 0);
	
		if (SYSTEM::VDIST(2411.32f, 4958.76f, 45.19f, uLocal_63) < 10f)
		{
			iLocal_51 = 1;
			unk_0xB5396F1FB088FE38(&uLocal_71);
			unk_0x505D1BE21E9DF7E0(0, 2473.2544f, 4901.6514f, 43.1488f, 3f, -1, 2f, 1, fLocal_248, 1193033728);
			unk_0xD844F5E50DAB6FF7(0, uLocal_52, 150f, -1, 0, 0);
			unk_0x93C0674FC00824D0(uLocal_71);
			unk_0x4BD42B0527065BB6(uLocal_53, uLocal_71);
			unk_0x44FB298D6382876D(uLocal_53, 1);
			unk_0xD0557B139A542F12(&uLocal_71);
			unk_0xA8C09F16998DFD28(2544.862f, 4729.8926f, 27.8986f, 2564.862f, 4749.8926f, 28.8986f, 1);
		}
		else
		{
			iLocal_51 = 2;
			unk_0xB5396F1FB088FE38(&uLocal_71);
			unk_0x505D1BE21E9DF7E0(0, 2531.445f, 2521.7493f, 38.2072f, 3f, -1, 2f, 1, fLocal_248, 1193033728);
			unk_0xD844F5E50DAB6FF7(0, uLocal_52, 150f, -1, 0, 0);
			unk_0x93C0674FC00824D0(uLocal_71);
			unk_0x4BD42B0527065BB6(uLocal_53, uLocal_71);
			unk_0x44FB298D6382876D(uLocal_53, 1);
			unk_0xD0557B139A542F12(&uLocal_71);
		}
	
		iLocal_51 == 1;
		unk_0xAB3658A740EED98E(uLocal_52, uLocal_53, -1, 1f, 3f, 1073741824, 0);
		unk_0x44FB298D6382876D(uLocal_52, 1);
		return true;
	}

	return false;
}

void func_114() // Position - 0x56D3
{
	fLocal_248 = 0f;
	fLocal_248.f_1 = 10f;
	unk_0xD0764B65C2DFEDCA(uLocal_52, 2);
	unk_0xBE8796DB2B90A437(uLocal_52, 13, 1);
	unk_0xBE8796DB2B90A437(uLocal_52, 2, 1);
	unk_0xBE8796DB2B90A437(uLocal_52, 1, 1);
	unk_0xBE8796DB2B90A437(uLocal_52, 11, 0);
	unk_0x1090EAA386FB31A8(uLocal_52, 0);
	unk_0xD0764B65C2DFEDCA(uLocal_53, 3);
	unk_0xBE8796DB2B90A437(uLocal_53, 17, 1);
	unk_0xBE8796DB2B90A437(uLocal_53, 6, 1);
	unk_0xBE8796DB2B90A437(uLocal_53, 1, 1);
	unk_0xBE8796DB2B90A437(uLocal_53, 11, 0);
	unk_0x1CA08719184AFC6F(uLocal_53, 128, 1);
	unk_0x1CA08719184AFC6F(uLocal_53, 1, 0);
	unk_0x06CD913C241C765E("RE_ARREST_COP", &uLocal_69);
	unk_0x06CD913C241C765E("RE_ARREST_CRIM", &uLocal_70);
	unk_0x3CEA1FD137ACE2D9(uLocal_52, uLocal_69);
	unk_0x3CEA1FD137ACE2D9(uLocal_53, uLocal_70);
	unk_0xD414C47AFF81382A(4, uLocal_70, uLocal_69);
	unk_0xD414C47AFF81382A(3, uLocal_69, joaat("PLAYER"));
	unk_0xD414C47AFF81382A(3, uLocal_70, joaat("PLAYER"));
	unk_0xD414C47AFF81382A(4, uLocal_70, joaat("COP"));
	unk_0x397CF4F4C8B17365(uLocal_52, sLocal_255);
	unk_0x397CF4F4C8B17365(uLocal_53, sLocal_254);
	return;
}

void _CONVERSATION_INITIALIZE_ACTOR(var uParam0, int iParam1, var uParam2, char* sParam3, int iParam4, int iParam5) // Position - 0x57B4
{
	uParam0->[iParam1 /*10*/].f_7 == 1;
	uParam0->[iParam1 /*10*/] = uParam2;
	TEXT_LABEL_ASSIGN_STRING(&(uParam0->[iParam1 /*10*/].f_1), sParam3, 24);
	uParam0->[iParam1 /*10*/].f_7 = 1;
	uParam0->[iParam1 /*10*/].f_8 = iParam4;
	uParam0->[iParam1 /*10*/].f_9 = iParam5;

	if (!Global_79539)
	{
		if (!unk_0x4FAFF4BCB7633475(uParam2))
			if (uParam0->[iParam1 /*10*/].f_8 == 0)
				unk_0xC1BC1B8A5AA67C6B(uParam2, 0);
			else
				unk_0xC1BC1B8A5AA67C6B(uParam2, 1);
	
		if (!unk_0x4FAFF4BCB7633475(uParam2))
			if (uParam0->[iParam1 /*10*/].f_9 == 0)
				unk_0x26C12212366CBF6E(uParam2, 0);
			else
				unk_0x26C12212366CBF6E(uParam2, 1);
	}

	return;
}

BOOL func_116() // Position - 0x584F
{
	if (!_CAN_ENTER_FREEROAM_STATE(5))
		return true;

	if (func_125())
		return true;

	if (!unk_0x4FAFF4BCB7633475(unk_0x4A8C381C258A124D()))
		if (SYSTEM::VMAG2(unk_0xE5741C6B6539231F(unk_0x4A8C381C258A124D())) > 1369f && !func_124())
			return false;

	if (func_117(100f, true) != -1)
		return true;

	return false;
}

int func_117(float fParam0, BOOL bParam1) // Position - 0x58B1
{
	var unk;
	int num;
	int num2;
	float num3;
	float num4;
	int num5;
	int i;
	BOOL flag;

	num2 = -1;
	num3 = fParam0;

	if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
	{
		if (func_53(_GET_CURRENT_PLAYER_CHARACTER()))
		{
			num5 = func_123();
			i = 0;
		
			for (i = 0; i < 63; i = i + 1)
			{
				num = i;
			
				if (IS_BIT_SET(Global_114135.f_18578[num /*6*/], 2) && !IS_BIT_SET(Global_114135.f_18578[num /*6*/], 3))
				{
					func_118(num, &unk);
					num4 = unk_0xED977E2AE2CB16EE(unk_0xD1A6A821F5AC81DB(unk_0x4A8C381C258A124D(), 0), unk.f_6, 1);
				
					if (num4 < num3)
					{
						flag = true;
					
						if (bParam1)
							if (num5 != unk.f_26)
								flag = false;
					
						if (flag)
						{
							num2 = num;
							num3 = num4;
						}
					}
				}
			}
		}
	}

	return num2;
}

void func_118(int iParam0, var uParam1) // Position - 0x5962
{
	switch (iParam0)
	{
		case 0:
			func_119(uParam1, "Abigail1", func_121(iParam0), 0, 0, 4, -1604.668f, 5239.1f, 3.01f, 66, "", 109, 0, "ambient_Diving", 0, 0, 1, 4, 1, 0, 2359, func_120(iParam0), 1, 0);
			break;
	
		case 1:
			func_119(uParam1, "Abigail2", func_121(iParam0), 0, 0, 4, -1592.84f, 5214.04f, 3.01f, 400, "", 110, 0, "", 0, 0, -1, 4, 1, 0, 2359, func_120(iParam0), 1, 0);
			break;
	
		case 2:
			func_119(uParam1, "Barry1", func_121(iParam0), 0, 1, 4, 190.26f, -956.35f, 29.63f, 381, "", 74, 0, "", 0, 1, -1, 4, 1, 0, 2359, func_120(iParam0), 1, 0);
			break;
	
		case 3:
			func_119(uParam1, "Barry2", func_121(iParam0), 0, 1, 4, 190.26f, -956.35f, 29.63f, 381, "", -1, 0, "", 0, 1, -1, 4, 4, 0, 2359, func_120(iParam0), 1, 1);
			break;
	
		case 4:
			func_119(uParam1, "Barry3", func_121(iParam0), 0, 1, 4, 414f, -761f, 29f, 381, "", -1, 0, "", 164, 1, -1, 0, 2, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 5:
			func_119(uParam1, "Barry3A", func_121(iParam0), 1, 1, 0, 1199.27f, -1255.63f, 34.23f, 381, "BARSTASH" /*Areas where you can find vehicles with a hidden stash have been marked on the map. Collect these vehicles for Barry.*/, 84, 0, "", 166, 0, 7, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 6:
			func_119(uParam1, "Barry3C", func_121(iParam0), 3, 1, 0, -468.9f, -1713.06f, 18.21f, 381, "", 84, 0, "", 166, 0, 7, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 7:
			func_119(uParam1, "Barry4", func_121(iParam0), 0, 1, 4, 237.65f, -385.41f, 44.4f, 381, "", 85, 0, "postRC_Barry4", 0, 0, -1, 4, 2, 800, 2000, func_120(iParam0), 0, 0);
			break;
	
		case 8:
			func_119(uParam1, "Dreyfuss1", func_121(iParam0), 0, 2, 4, -1458.97f, 485.99f, 115.38f, 66, "LETTERS_HINT" /*The killer's identity and location have been revealed.*/, 106, 0, "", 0, 0, -1, 4, 2, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 9:
			func_119(uParam1, "Epsilon1", func_121(iParam0), 0, 3, 4, -1622.89f, 4204.87f, 83.3f, 66, "", 86, 0, "", 0, 1, 10, 4, 1, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 10:
			func_119(uParam1, "Epsilon2", func_121(iParam0), 0, 3, 4, 242.7f, 362.7f, 104.74f, 206, "", 87, 16, "", 0, 0, 11, 4, 1, 0, 2359, func_120(iParam0), 1, 0);
			break;
	
		case 11:
			func_119(uParam1, "Epsilon3", func_121(iParam0), 0, 3, 4, 1835.53f, 4705.86f, 38.1f, 206, "", 88, 16, "epsCars", 0, 0, 12, 4, 1, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 12:
			func_119(uParam1, "Epsilon4", func_121(iParam0), 0, 3, 4, 1826.13f, 4698.88f, 38.92f, 206, "", 90, 16, "postRC_Epsilon4", 0, 0, 13, 4, 1, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 13:
			func_119(uParam1, "Epsilon5", func_121(iParam0), 0, 3, 4, 637.02f, 119.7093f, 89.5f, 206, "", 89, 16, "epsRobes", 0, 0, 14, 4, 1, 0, 2359, func_120(iParam0), 1, 0);
			break;
	
		case 14:
			func_119(uParam1, "Epsilon6", func_121(iParam0), 0, 3, 4, -2892.93f, 3192.37f, 11.66f, 206, "", 93, 0, "", 0, 0, 15, 4, 1, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 15:
			func_119(uParam1, "Epsilon7", func_121(iParam0), 0, 3, 4, 524.43f, 3079.82f, 39.48f, 206, "", -1, 16, "epsDesert", 0, 0, 16, 4, 1, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 16:
			func_119(uParam1, "Epsilon8", func_121(iParam0), 0, 3, 4, -697.75f, 45.38f, 43.03f, 206, "", 94, 16, "epsilonTract", 0, 0, -1, 4, 1, 0, 2359, func_120(iParam0), 1, 0);
			break;
	
		case 17:
			func_119(uParam1, "Extreme1", func_121(iParam0), 0, 4, 4, -188.22f, 1296.1f, 302.86f, 66, "", -1, 0, "", 4, 1, 18, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 18:
			func_119(uParam1, "Extreme2", func_121(iParam0), 0, 4, 4, -954.19f, -2760.05f, 14.64f, 382, "", 96, 0, "", 171, 0, 19, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 19:
			func_119(uParam1, "Extreme3", func_121(iParam0), 0, 4, 4, -63.8f, -809.5f, 321.8f, 382, "", 97, 0, "", 0, 0, 20, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 20:
			func_119(uParam1, "Extreme4", func_121(iParam0), 0, 4, 4, 1731.41f, 96.96f, 170.39f, 382, "", 98, 16, "", 0, 0, -1, 4, 2, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 21:
			func_119(uParam1, "Fanatic1", func_121(iParam0), 0, 5, 4, -1877.82f, -440.649f, 45.05f, 405, "", 74, 0, "", 0, 1, -1, 4, 1, 700, 2000, func_120(iParam0), 1, 0);
			break;
	
		case 22:
			func_119(uParam1, "Fanatic2", func_121(iParam0), 0, 5, 4, 809.66f, 1279.76f, 360.49f, 405, "", -1, 0, "", 0, 1, -1, 4, 4, 700, 2000, func_120(iParam0), 1, 0);
			break;
	
		case 23:
			func_119(uParam1, "Fanatic3", func_121(iParam0), 0, 5, 4, -915.6f, 6139.2f, 5.5f, 405, "", -1, 0, "", 0, 1, -1, 4, 2, 700, 2000, func_120(iParam0), 0, 1);
			break;
	
		case 24:
			func_119(uParam1, "Hao1", func_121(iParam0), 0, 6, 4, -72.29f, -1260.63f, 28.14f, 66, "", -1, 0, "controller_Races", 13, 1, -1, 4, 2, 2000, 500, func_120(iParam0), 0, 1);
			break;
	
		case 25:
			func_119(uParam1, "Hunting1", func_121(iParam0), 0, 7, 4, 1804.32f, 3931.33f, 32.82f, 66, "", -1, 0, "", 174, 1, 26, 4, 4, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 26:
			func_119(uParam1, "Hunting2", func_121(iParam0), 0, 7, 4, -684.17f, 5839.16f, 16.09f, 384, "", 99, 0, "", 7, 0, -1, 4, 4, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 27:
			func_119(uParam1, "Josh1", func_121(iParam0), 0, 8, 4, -1104.93f, 291.25f, 64.3f, 66, "", -1, 0, "forSaleSigns", 0, 1, 28, 4, 4, 0, 2359, func_120(iParam0), 1, 0);
			break;
	
		case 28:
			func_119(uParam1, "Josh2", func_121(iParam0), 0, 8, 4, 565.39f, -1772.88f, 29.77f, 385, "", 105, 0, "", 0, 0, 29, 4, 4, 0, 2359, func_120(iParam0), 1, 1);
			break;
	
		case 29:
			func_119(uParam1, "Josh3", func_121(iParam0), 0, 8, 4, 565.39f, -1772.88f, 29.77f, 385, "", -1, 16, "", 0, 0, 30, 4, 4, 0, 2359, func_120(iParam0), 1, 1);
			break;
	
		case 30:
			func_119(uParam1, "Josh4", func_121(iParam0), 0, 8, 4, -1104.93f, 291.25f, 64.3f, 385, "", -1, 36, "", 0, 0, -1, 4, 4, 0, 2359, func_120(iParam0), 1, 0);
			break;
	
		case 31:
			func_119(uParam1, "Maude1", func_121(iParam0), 0, 9, 4, 2726.1f, 4145f, 44.3f, 66, "", -1, 0, "BailBond_Launcher", 0, 1, -1, 4, 4, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 32:
			func_119(uParam1, "Minute1", func_121(iParam0), 0, 10, 4, 327.85f, 3405.7f, 35.73f, 66, "", -1, 0, "", 0, 1, 33, 4, 4, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 33:
			func_119(uParam1, "Minute2", func_121(iParam0), 0, 10, 4, 18f, 4527f, 105f, 386, "", -1, 10, "", 0, 0, 34, 4, 4, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 34:
			func_119(uParam1, "Minute3", func_121(iParam0), 0, 10, 4, -303.82f, 6211.29f, 31.05f, 386, "", -1, 10, "", 0, 0, -1, 4, 4, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 35:
			func_119(uParam1, "MrsPhilips1", func_121(iParam0), 0, 11, 4, 1972.59f, 3816.43f, 32.42f, 66, "", -1, 0, "ambient_MrsPhilips", 0, 1, -1, 4, 4, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 36:
			func_119(uParam1, "MrsPhilips2", func_121(iParam0), 0, 11, 4, 0f, 0f, 0f, -1, "", -1, 0, "", 0, 1, -1, 4, 4, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 37:
			func_119(uParam1, "Nigel1", func_121(iParam0), 0, 12, 4, -1097.16f, 790.01f, 164.52f, 66, "", -1, 0, "", 177, 1, -1, 1, 4, 0, 2359, func_120(iParam0), 1, 0);
			break;
	
		case 38:
			func_119(uParam1, "Nigel1A", func_121(iParam0), 0, 12, 1, -558.65f, 284.49f, 90.86f, 149, "NIGITEMS" /*Areas where you can find celebrity items have been marked on the map. Steal these items for Nigel and Mrs. Thornhill.*/, 100, 0, "", 0, 0, 42, 4, 4, 0, 2359, func_120(iParam0), 1, 1);
			break;
	
		case 39:
			func_119(uParam1, "Nigel1B", func_121(iParam0), 0, 12, 1, -1034.15f, 366.08f, 80.11f, 149, "", 100, 0, "", 0, 0, 42, 4, 4, 700, 2000, func_120(iParam0), 1, 1);
			break;
	
		case 40:
			func_119(uParam1, "Nigel1C", func_121(iParam0), 0, 12, 1, -623.91f, -266.17f, 37.76f, 149, "", 100, 0, "", 0, 0, 42, 4, 4, 700, 2000, func_120(iParam0), 1, 1);
			break;
	
		case 41:
			func_119(uParam1, "Nigel1D", func_121(iParam0), 0, 12, 1, -1096.85f, 67.68f, 52.95f, 149, "", 100, 0, "", 0, 0, 42, 4, 4, 700, 2000, func_120(iParam0), 1, 1);
			break;
	
		case 42:
			func_119(uParam1, "Nigel2", func_121(iParam0), 0, 12, 4, -1310.7f, -640.22f, 26.54f, 149, "", -1, 8, "", 0, 0, 43, 4, 4, 0, 2359, func_120(iParam0), 1, 1);
			break;
	
		case 43:
			func_119(uParam1, "Nigel3", func_121(iParam0), 0, 12, 4, -44.75f, -1288.67f, 28.21f, 149, "", -1, 16, "postRC_Nigel3", 0, 0, -1, 4, 4, 0, 2359, func_120(iParam0), 1, 1);
			break;
	
		case 44:
			func_119(uParam1, "Omega1", func_121(iParam0), 0, 13, 4, 2468.51f, 3437.39f, 49.9f, 66, "", -1, 0, "spaceshipParts", 0, 1, 45, 4, 2, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 45:
			func_119(uParam1, "Omega2", func_121(iParam0), 0, 13, 4, 2319.44f, 2583.58f, 46.76f, 387, "", 107, 0, "", 0, 0, -1, 4, 2, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 46:
			func_119(uParam1, "Paparazzo1", func_121(iParam0), 0, 14, 4, -149.75f, 285.81f, 93.67f, 66, "", -1, 0, "", 0, 1, 47, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 47:
			func_119(uParam1, "Paparazzo2", func_121(iParam0), 0, 14, 4, -70.71f, 301.43f, 106.79f, 389, "", -1, 8, "", 0, 0, 48, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 48:
			func_119(uParam1, "Paparazzo3", func_121(iParam0), 0, 14, 4, -257.22f, 292.85f, 90.63f, 389, "", -1, 8, "", 183, 1, -1, 2, 2, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 49:
			func_119(uParam1, "Paparazzo3A", func_121(iParam0), 0, 14, 2, 305.52f, 157.19f, 102.94f, 389, "PAPPHOTO" /*Areas where you can find celebrity photo opportunities have been marked on the map. Track down and photograph these celebrities for Beverly.*/, 102, 0, "", 0, 0, 51, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 50:
			func_119(uParam1, "Paparazzo3B", func_121(iParam0), 0, 14, 2, 1040.96f, -534.42f, 60.17f, 389, "", 102, 0, "", 0, 0, 51, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 51:
			func_119(uParam1, "Paparazzo4", func_121(iParam0), 0, 14, 4, -484.2f, 229.68f, 82.21f, 389, "", -1, 8, "", 0, 1, -1, 4, 2, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 52:
			func_119(uParam1, "Rampage1", func_121(iParam0), 0, 15, 4, 908f, 3643.7f, 32.2f, 66, "", -1, 0, "", 0, 1, 54, 4, 4, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 54:
			func_119(uParam1, "Rampage3", func_121(iParam0), 0, 15, 4, 465.1f, -1849.3f, 27.8f, 84, "", -1, 0, "", 0, 1, 55, 4, 4, 0, 2359, func_120(iParam0), 1, 0);
			break;
	
		case 55:
			func_119(uParam1, "Rampage4", func_121(iParam0), 0, 15, 4, -161f, -1669.7f, 33f, 84, "", -1, 0, "", 0, 0, 56, 4, 4, 0, 2359, func_120(iParam0), 1, 0);
			break;
	
		case 56:
			func_119(uParam1, "Rampage5", func_121(iParam0), 0, 15, 4, -1298.2f, 2504.14f, 21.09f, 84, "", -1, 0, "", 0, 0, 53, 4, 4, 0, 2359, func_120(iParam0), 0, 0);
			break;
	
		case 53:
			func_119(uParam1, "Rampage2", func_121(iParam0), 0, 15, 4, 1181.5f, -400.1f, 67.5f, 84, "", -1, 0, "rampage_controller", 0, 0, -1, 4, 4, 0, 2359, func_120(iParam0), 1, 0);
			break;
	
		case 57:
			func_119(uParam1, "TheLastOne", func_121(iParam0), 0, 16, 4, -1298.98f, 4640.16f, 105.67f, 66, "", 133, 1, "", 0, 1, -1, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 58:
			func_119(uParam1, "Tonya1", func_121(iParam0), 0, 17, 4, -14.39f, -1472.69f, 29.58f, 66, "AM_H_RCFS" /*Strangers and Freaks can be found throughout San Andreas at ~HUD_COLOUR_FRANKLIN~~BLIP_RANDOM_CHARACTER~~s~*/, -1, 0, "ambient_TonyaCall", 24, 1, 59, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 59:
			func_119(uParam1, "Tonya2", func_121(iParam0), 0, 17, 4, -14.39f, -1472.69f, 29.58f, 388, "", -1, 48, "ambient_Tonya", 185, 0, 60, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 60:
			func_119(uParam1, "Tonya3", func_121(iParam0), 0, 17, 4, 0f, 0f, 0f, -1, "", -1, 0, "", 187, 0, 61, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 61:
			func_119(uParam1, "Tonya4", func_121(iParam0), 0, 17, 4, 0f, 0f, 0f, -1, "", -1, 0, "", 0, 0, 62, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		case 62:
			func_119(uParam1, "Tonya5", func_121(iParam0), 0, 17, 4, -14.39f, -1472.69f, 29.58f, 388, "", -1, 48, "", 0, 0, -1, 4, 2, 0, 2359, func_120(iParam0), 0, 1);
			break;
	
		default:
			break;
	}

	return;
}

void func_119(var uParam0, char* sParam1, var uParam2, var uParam3, int iParam4, int iParam5, int iParam6, float fParam7, float fParam8, float fParam9, int iParam10, char* sParam11, int iParam12, int iParam13, char* sParam14, int iParam15, int iParam16, int iParam17, int iParam18, int iParam19, int iParam20, int iParam21, int iParam22, int iParam23, int iParam24) // Position - 0x6BA9
{
	uParam0->f_4 = iParam5;
	*uParam0 = sParam1;
	uParam0->f_1 = { uParam2 };
	uParam0->f_3 = iParam4;
	uParam0->f_5 = iParam6;
	uParam0->f_6 = { fParam7 };
	uParam0->f_9 = iParam10;
	TEXT_LABEL_ASSIGN_STRING(&(uParam0->f_10), sParam11, 16);
	uParam0->f_14 = iParam12;
	uParam0->f_15 = iParam13;
	TEXT_LABEL_ASSIGN_STRING(&(uParam0->f_16), sParam14, 24);
	uParam0->f_22 = iParam15;
	uParam0->f_23 = iParam16;
	uParam0->f_24 = iParam17;
	uParam0->f_25 = iParam18;
	uParam0->f_26 = iParam19;
	uParam0->f_27 = iParam20;
	uParam0->f_28 = iParam21;
	uParam0->f_29 = iParam22;
	uParam0->f_30 = iParam23;
	uParam0->f_31 = iParam24;
	return;
}

int func_120(int iParam0) // Position - 0x6C3A
{
	switch (iParam0)
	{
		case 0:
			return 0;
	
		case 1:
			return 0;
	
		case 2:
			return 1;
	
		case 3:
			return 1;
	
		case 4:
			return 0;
	
		case 5:
			return 1;
	
		case 6:
			return 1;
	
		case 7:
			return 0;
	
		case 8:
			return 1;
	
		case 9:
			return 0;
	
		case 10:
			return 0;
	
		case 11:
			return 0;
	
		case 12:
			return 1;
	
		case 13:
			return 0;
	
		case 14:
			return 1;
	
		case 15:
			return 0;
	
		case 16:
			return 1;
	
		case 17:
			return 1;
	
		case 18:
			return 1;
	
		case 19:
			return 1;
	
		case 20:
			return 1;
	
		case 21:
			return 1;
	
		case 22:
			return 1;
	
		case 23:
			return 1;
	
		case 24:
			return 1;
	
		case 25:
			return 1;
	
		case 26:
			return 1;
	
		case 27:
			return 0;
	
		case 28:
			return 1;
	
		case 29:
			return 1;
	
		case 30:
			return 1;
	
		case 31:
			return 0;
	
		case 32:
			return 1;
	
		case 33:
			return 1;
	
		case 34:
			return 1;
	
		case 35:
			return 0;
	
		case 36:
			return 0;
	
		case 37:
			return 0;
	
		case 38:
			return 1;
	
		case 39:
			return 1;
	
		case 40:
			return 1;
	
		case 41:
			return 1;
	
		case 42:
			return 1;
	
		case 43:
			return 1;
	
		case 44:
			return 0;
	
		case 45:
			return 0;
	
		case 46:
			return 1;
	
		case 47:
			return 1;
	
		case 48:
			return 0;
	
		case 49:
			return 1;
	
		case 50:
			return 1;
	
		case 51:
			return 1;
	
		case 52:
			return 1;
	
		case 54:
			return 1;
	
		case 55:
			return 1;
	
		case 56:
			return 1;
	
		case 53:
			return 1;
	
		case 57:
			return 1;
	
		case 58:
			return 1;
	
		case 59:
			return 1;
	
		case 60:
			return 1;
	
		case 61:
			return 1;
	
		case 62:
			return 1;
	
		default:
			break;
	}

	return 0;
}

struct<2> func_121(int iParam0) // Position - 0x6F80
{
	var unk;
	var unk3;

	TEXT_LABEL_ASSIGN_STRING(&unk, "", 8);
	unk3 = { func_122(iParam0) };

	if (unk_0xD6F9DEE4765092A9(&unk3))
	{
	}
	else
	{
		TEXT_LABEL_ASSIGN_STRING(&unk, "RC_", 8);
		TEXT_LABEL_APPEND_STRING(&unk, &unk3, 8);
	}

	return unk;
}

struct<2> func_122(int iParam0) // Position - 0x6FB8
{
	var unk;

	TEXT_LABEL_ASSIGN_STRING(&unk, "", 8);

	switch (iParam0)
	{
		case 0:
			TEXT_LABEL_ASSIGN_STRING(&unk, "ABI1", 8);
			break;
	
		case 1:
			TEXT_LABEL_ASSIGN_STRING(&unk, "ABI2", 8);
			break;
	
		case 2:
			TEXT_LABEL_ASSIGN_STRING(&unk, "BA1", 8);
			break;
	
		case 3:
			TEXT_LABEL_ASSIGN_STRING(&unk, "BA2", 8);
			break;
	
		case 4:
			TEXT_LABEL_ASSIGN_STRING(&unk, "BA3", 8);
			break;
	
		case 5:
			TEXT_LABEL_ASSIGN_STRING(&unk, "BA3A", 8);
			break;
	
		case 6:
			TEXT_LABEL_ASSIGN_STRING(&unk, "BA3C", 8);
			break;
	
		case 7:
			TEXT_LABEL_ASSIGN_STRING(&unk, "BA4", 8);
			break;
	
		case 8:
			TEXT_LABEL_ASSIGN_STRING(&unk, "DRE1", 8);
			break;
	
		case 9:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS1", 8);
			break;
	
		case 10:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS2", 8);
			break;
	
		case 11:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS3", 8);
			break;
	
		case 12:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS4", 8);
			break;
	
		case 13:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS5", 8);
			break;
	
		case 14:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS6", 8);
			break;
	
		case 15:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS7", 8);
			break;
	
		case 16:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EPS8", 8);
			break;
	
		case 17:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EXT1", 8);
			break;
	
		case 18:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EXT2", 8);
			break;
	
		case 19:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EXT3", 8);
			break;
	
		case 20:
			TEXT_LABEL_ASSIGN_STRING(&unk, "EXT4", 8);
			break;
	
		case 21:
			TEXT_LABEL_ASSIGN_STRING(&unk, "FAN1", 8);
			break;
	
		case 22:
			TEXT_LABEL_ASSIGN_STRING(&unk, "FAN2", 8);
			break;
	
		case 23:
			TEXT_LABEL_ASSIGN_STRING(&unk, "FAN3", 8);
			break;
	
		case 24:
			TEXT_LABEL_ASSIGN_STRING(&unk, "HAO1", 8);
			break;
	
		case 25:
			TEXT_LABEL_ASSIGN_STRING(&unk, "HUN1", 8);
			break;
	
		case 26:
			TEXT_LABEL_ASSIGN_STRING(&unk, "HUN2", 8);
			break;
	
		case 27:
			TEXT_LABEL_ASSIGN_STRING(&unk, "JOS1", 8);
			break;
	
		case 28:
			TEXT_LABEL_ASSIGN_STRING(&unk, "JOS2", 8);
			break;
	
		case 29:
			TEXT_LABEL_ASSIGN_STRING(&unk, "JOS3", 8);
			break;
	
		case 30:
			TEXT_LABEL_ASSIGN_STRING(&unk, "JOS4", 8);
			break;
	
		case 31:
			TEXT_LABEL_ASSIGN_STRING(&unk, "MAU1", 8);
			break;
	
		case 32:
			TEXT_LABEL_ASSIGN_STRING(&unk, "MIN1", 8);
			break;
	
		case 33:
			TEXT_LABEL_ASSIGN_STRING(&unk, "MIN2", 8);
			break;
	
		case 34:
			TEXT_LABEL_ASSIGN_STRING(&unk, "MIN3", 8);
			break;
	
		case 35:
			TEXT_LABEL_ASSIGN_STRING(&unk, "MRS1", 8);
			break;
	
		case 36:
			TEXT_LABEL_ASSIGN_STRING(&unk, "MRS2", 8);
			break;
	
		case 37:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI1", 8);
			break;
	
		case 38:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI1A", 8);
			break;
	
		case 39:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI1B", 8);
			break;
	
		case 40:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI1C", 8);
			break;
	
		case 41:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI1D", 8);
			break;
	
		case 42:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI2", 8);
			break;
	
		case 43:
			TEXT_LABEL_ASSIGN_STRING(&unk, "NI3", 8);
			break;
	
		case 44:
			TEXT_LABEL_ASSIGN_STRING(&unk, "OME1", 8);
			break;
	
		case 45:
			TEXT_LABEL_ASSIGN_STRING(&unk, "OME2", 8);
			break;
	
		case 46:
			TEXT_LABEL_ASSIGN_STRING(&unk, "PA1", 8);
			break;
	
		case 47:
			TEXT_LABEL_ASSIGN_STRING(&unk, "PA2", 8);
			break;
	
		case 48:
			TEXT_LABEL_ASSIGN_STRING(&unk, "PA3", 8);
			break;
	
		case 49:
			TEXT_LABEL_ASSIGN_STRING(&unk, "PA3A", 8);
			break;
	
		case 50:
			TEXT_LABEL_ASSIGN_STRING(&unk, "PA3B", 8);
			break;
	
		case 51:
			TEXT_LABEL_ASSIGN_STRING(&unk, "PA4", 8);
			break;
	
		case 52:
			TEXT_LABEL_ASSIGN_STRING(&unk, "RAM1", 8);
			break;
	
		case 53:
			TEXT_LABEL_ASSIGN_STRING(&unk, "RAM2", 8);
			break;
	
		case 54:
			TEXT_LABEL_ASSIGN_STRING(&unk, "RAM3", 8);
			break;
	
		case 55:
			TEXT_LABEL_ASSIGN_STRING(&unk, "RAM4", 8);
			break;
	
		case 56:
			TEXT_LABEL_ASSIGN_STRING(&unk, "RAM5", 8);
			break;
	
		case 57:
			TEXT_LABEL_ASSIGN_STRING(&unk, "SAS1", 8);
			break;
	
		case 58:
			TEXT_LABEL_ASSIGN_STRING(&unk, "TON1", 8);
			break;
	
		case 59:
			TEXT_LABEL_ASSIGN_STRING(&unk, "TON2", 8);
			break;
	
		case 60:
			TEXT_LABEL_ASSIGN_STRING(&unk, "TON3", 8);
			break;
	
		case 61:
			TEXT_LABEL_ASSIGN_STRING(&unk, "TON4", 8);
			break;
	
		case 62:
			TEXT_LABEL_ASSIGN_STRING(&unk, "TON5", 8);
			break;
	
		default:
			break;
	}

	return unk;
}

int func_123() // Position - 0x7405
{
	func_52();

	switch (Global_114135.f_2367.f_539.f_4321)
	{
		case CHAR_MICHAEL:
			return 1;
	
		case CHAR_FRANKLIN:
			return 2;
	
		case CHAR_TREVOR:
			return 4;
	}

	return 0;
}

BOOL func_124() // Position - 0x744B
{
	if (unk_0x761778199FE1211C())
		if (unk_0xAB154BFC15F33733() == 1f)
			return true;

	return false;
}

BOOL func_125() // Position - 0x7468
{
	if (func_128() && !func_124())
		return true;

	if (func_127() && func_126())
		return true;

	return false;
}

BOOL func_126() // Position - 0x749A
{
	return Global_113853 > 0;
}

BOOL func_127() // Position - 0x74A8
{
	if (Global_98585 != -1)
		return true;

	return false;
}

BOOL func_128() // Position - 0x74BD
{
	if (Global_98585 != -1)
		return IS_BIT_SET(Global_92451[Global_98585 /*34*/].f_15, 20);

	return false;
}

BOOL func_129() // Position - 0x74E0
{
	if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()) && !unk_0x4FAFF4BCB7633475(unk_0x4A8C381C258A124D()))
	{
		if (SYSTEM::VDIST2(unk_0xD1A6A821F5AC81DB(unk_0x4A8C381C258A124D(), 1), uLocal_42) < 75f * 75f)
			return true;
	
		if (SYSTEM::VMAG2(unk_0xE5741C6B6539231F(unk_0x4A8C381C258A124D())) > 1369f && !func_124())
			return false;
	}

	if (func_125())
		return true;

	if (func_117(100f, true) != -1)
		return true;

	return false;
}

void func_130(int iParam0) // Position - 0x7566
{
	if (iParam0 == -1)
		iParam0 = func_133();

	if (iParam0 == -1)
		return;

	func_132(iParam0);
	unk_0x21EA3DC8C16770EF(0);
	unk_0x7EA65A7D5C6CBDB5(1);
	Global_114121 = 0;
	func_131();
	return;
}

void func_131() // Position - 0x759C
{
	if (!unk_0x4FAFF4BCB7633475(unk_0x4A8C381C258A124D()))
	{
		if (unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
			unk_0xCCA6D8A84EE8C88A(unk_0x4B423FAA24E8ABF0(unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0)), 1);
	
		unk_0x0428AFDCAA63B06E(unk_0x4A8C381C258A124D(), 32, 0);
	}

	return;
}

void func_132(int iParam0) // Position - 0x75D9
{
	Global_114124 = iParam0;
	return;
}

int func_133() // Position - 0x75E7
{
	var unk;
	int num;

	TEXT_LABEL_ASSIGN_STRING(&unk, unk_0x1AF90EB93E0012D6(), 64);
	num = func_134(unk);
	return num;
}

int func_134(var uParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6, var uParam7, var uParam8, var uParam9, var uParam10, var uParam11, var uParam12, var uParam13, var uParam14, var uParam15) // Position - 0x7604
{
	switch (unk_0x70E57E9927B6BA58(&uParam0))
	{
		case joaat("re_abandonedcar"):
			return 23;
	
		case joaat("re_accident"):
			return 0;
	
		case joaat("re_arrests"):
			return 15;
	
		case joaat("re_atmrobbery"):
			return 1;
	
		case joaat("re_bikethief"):
			return 26;
	
		case joaat("re_border"):
			return 29;
	
		case joaat("re_burials"):
			return 24;
	
		case joaat("re_bus_tours"):
			return 2;
	
		case joaat("re_cartheft"):
			return 17;
	
		case joaat("re_chasethieves"):
			return 11;
	
		case joaat("re_crashrescue"):
			return 16;
	
		case joaat("re_cultshootout"):
			return 18;
	
		case joaat("re_dealgonewrong"):
			return 12;
	
		case joaat("re_domestic"):
			return 3;
	
		case joaat("re_drunkdriver"):
			return 27;
	
		case joaat("re_gang_intimidation"):
			return 20;
	
		case joaat("re_gangfight"):
			return 19;
	
		case joaat("re_getaway_driver"):
			return 4;
	
		case joaat("re_hitch_lift"):
			return 13;
	
		case joaat("re_homeland_security"):
			return 28;
	
		case joaat("re_lured"):
			return 7;
	
		case joaat("re_muggings"):
			return 25;
	
		case joaat("re_paparazzi"):
			return 10;
	
		case joaat("re_prisonerlift"):
			return 22;
	
		case joaat("re_prisonvanbreak"):
			return 21;
	
		case joaat("re_securityvan"):
			return 9;
	
		case joaat("re_shoprobbery"):
			return 5;
	
		case joaat("re_snatched"):
			return 6;
	
		case joaat("re_stag_do"):
			return 14;
	
		case joaat("re_yetarian"):
			return 30;
	
		case joaat("re_duel"):
			return 31;
	
		case joaat("re_seaplane"):
			return 32;
	
		case joaat("re_monkey"):
			return 33;
	}

	return -1;
}

BOOL func_135(var uParam0, var uParam1, var uParam2, int iParam3, int iParam4, BOOL bParam5, BOOL bParam6) // Position - 0x77DE
{
	BOOL flag;
	Vector3 vector;
	eCharacter i;
	Vector3 vector2;
	int num;

	if (!Global_152724)
		return false;

	if (iParam3 == -1)
		iParam3 = func_133();

	if (iParam3 == -1)
		return false;

	if (iParam3 == 31 || iParam3 == 32)
		if (!_IS_EXCLUSIVE_CONTENT_UNLOCKED())
			return false;

	uLocal_42 = { uParam0 };
	flag = false;

	if (!flag)
	{
		if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()) && !unk_0x4FAFF4BCB7633475(unk_0x4A8C381C258A124D()))
		{
			vector = { unk_0xD1A6A821F5AC81DB(unk_0x4A8C381C258A124D(), 1) };
		
			if (SYSTEM::VMAG2(unk_0xE5741C6B6539231F(unk_0x4A8C381C258A124D())) > 1369f && !func_124())
				return false;
		}
	
		if (!Global_114135.f_9089)
			return false;
	
		if (_IS_MISSION_REPEAT_ACTIVE(false))
			return false;
	
		if (func_125())
			return false;
	
		if (func_175())
			return false;
	
		if (Global_114124 != -1)
			return false;
	
		if (func_53(_GET_CURRENT_PLAYER_CHARACTER()))
			if (func_117(100f, true) != -1)
				return false;
	
		if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()) && !bParam6)
			if (vector.f_2 - uLocal_42.f_2 > 50f)
				return false;
	
		if (!func_174(iParam3))
			return false;
	
		if (func_53(_GET_CURRENT_PLAYER_CHARACTER()))
			if (func_173(_GET_CURRENT_PLAYER_CHARACTER()) == 4 || func_173(_GET_CURRENT_PLAYER_CHARACTER()) == 5)
				return false;
	
		if (func_53(_GET_CURRENT_PLAYER_CHARACTER()))
			if (!func_172(iParam3, iParam4, 145))
				return false;
	
		if (!func_171(Global_114135.f_24999.f_43[iParam3]))
			return false;
	
		if (unk_0x1DD05E817C89C737() - Global_114126 < 150000)
			if (iParam3 != 30)
				return false;
	
		if (func_170())
			return false;
	
		if (unk_0x3D2EB53CF281A77E())
			return false;
	
		if (unk_0x9FA91AA1AC724CD0())
			return false;
	
		if (!func_161(4))
			return false;
	
		if (!_CAN_ENTER_FREEROAM_STATE(5))
			return false;
	
		if (func_160(iParam3, iParam4) && !bParam5)
			return false;
	
		if (Global_4 && iParam3 != 10)
			return false;
	
		if (unk_0xF8A8852F99E201DD(unk_0xF8F35890F43ED2AE(unk_0x4A8C381C258A124D())))
			if (unk_0xF8F35890F43ED2AE(unk_0x4A8C381C258A124D()) == unk_0x0E171121A3A25363(377.153f, -717.567f, 10.0536f) || unk_0xF8F35890F43ED2AE(unk_0x4A8C381C258A124D()) == unk_0x0E171121A3A25363(320.9934f, 265.2515f, 82.1221f) || unk_0xF8F35890F43ED2AE(unk_0x4A8C381C258A124D()) == unk_0x0E171121A3A25363(-1425.5645f, -244.3f, 15.8053f))
				return false;
	
		if (iParam3 == 9 && iParam4 == 2 || iParam4 == 5 && !func_160(0, 0))
			return false;
	
		if (Global_33234)
			return false;
	
		if (func_174(30) && !func_160(30, 0))
			if (iParam3 != 30)
				if (SYSTEM::VDIST2(vector, -61.2745f, -1100.4675f, 25.3752f) < 176400f)
					return false;
	
		if (func_53(_GET_CURRENT_PLAYER_CHARACTER()))
		{
			for (i = CHAR_MICHAEL; i < CHAR_MULTIPLAYER; i = i + 1)
			{
				vector2 = { Global_114135.f_2367.f_539.f_2300[i /*3*/] };
				num = Global_114135.f_2367.f_539.f_2296[i];
			
				if (func_159(num))
					if (func_137(i))
						if (!func_136(vector2, 0f, 0f, 0f, false))
							if (SYSTEM::VDIST2(unk_0xD1A6A821F5AC81DB(unk_0x4A8C381C258A124D(), 0), vector2) < 210f * 210f)
								if (_GET_CURRENT_PLAYER_CHARACTER() != i)
									return false;
			}
		}
	}

	return true;
}

BOOL func_136(float fParam0, var uParam1, var uParam2, float fParam3, float fParam4, float fParam5, BOOL bParam6) // Position - 0x7B78
{
	if (bParam6)
		return fParam0 == fParam3 && fParam0.f_1 == fParam3.f_1;

	return fParam0 == fParam3 && fParam0.f_1 == fParam3.f_1 && fParam0.f_2 == fParam3.f_2;
}

BOOL func_137(eCharacter echParam0) // Position - 0x7BBF
{
	int num;

	num = Global_114135.f_2367.f_539.f_2296[echParam0];
	return func_138(num);
}

BOOL func_138(int iParam0) // Position - 0x7BE0
{
	return func_139(iParam0, 1);
}

int func_139(int iParam0, int iParam1) // Position - 0x7BEF
{
	var unk;
	var unk2;
	int num;
	int num2;
	int num3;
	int num4;

	if (!func_159(iParam0))
		return 0;

	func_140(iParam0, &unk, &unk2, &num, &num2, &num3, &num4);

	if (num4 > 0 || num3 > 0 || num2 > 0 || num >= iParam1)
		return 1;

	return 0;
}

void func_140(int iParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6) // Position - 0x7C42
{
	func_141(func_152(), iParam0, uParam1, uParam2, uParam3, uParam4, uParam5, uParam6);
	return;
}

void func_141(int iParam0, int iParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6, var uParam7) // Position - 0x7C60
{
	int num;
	int num2;
	int num3;
	int num4;
	int num5;
	int num6;
	int num7;
	int num8;

	if (func_151(iParam0, iParam1))
	{
		num = func_150(iParam1);
		num2 = func_148(iParam0);
		num3 = func_148(iParam0) - func_148(iParam1);
		num4 = func_150(iParam0) - func_150(iParam1);
		num5 = func_147(iParam0) - func_147(iParam1);
		num6 = func_146(iParam0) - func_146(iParam1);
		num7 = func_145(iParam0) - func_145(iParam1);
		num8 = func_144(iParam0) - func_144(iParam1);
	}
	else
	{
		num = func_150(iParam0);
		num2 = func_148(iParam1);
		num3 = func_148(iParam1) - func_148(iParam0);
		num4 = func_150(iParam1) - func_150(iParam0);
		num5 = func_147(iParam1) - func_147(iParam0);
		num6 = func_146(iParam1) - func_146(iParam0);
		num7 = func_145(iParam1) - func_145(iParam0);
		num8 = func_144(iParam1) - func_144(iParam0);
	}

	while (num8 < 0)
	{
		num8 = num8 + 60;
		num7 = num7 - 1;
	}

	while (num8 > 59)
	{
		num8 = num8 - 60;
		num7 = num7 + 1;
	}

	while (num7 < 0)
	{
		num7 = num7 + 60;
		num6 = num6 - 1;
	}

	while (num7 > 59)
	{
		num7 = num7 - 60;
		num6 = num6 + 1;
	}

	while (num6 < 0)
	{
		num6 = num6 + 24;
		num5 = num5 - 1;
	}

	while (num6 > 23)
	{
		num6 = num6 - 24;
		num5 = num5 + 1;
	}

	while (num5 < 0)
	{
		while (num4 < 0)
		{
			num4 = num4 + 12;
			num3 = num3 - 1;
		}
	
		num5 = num5 + func_143(num, num2);
		num4 = num4 - 1;
		num = SYSTEM::ROUND(func_142(SYSTEM::TO_FLOAT(num + 1), 0f, 12f));
	}

	while (num4 < 0)
	{
		num4 = num4 + 12;
		num3 = num3 - 1;
	}

	while (num4 > 12)
	{
		num4 = num4 - 12;
		num3 = num3 + 1;
	}

	*uParam2 = num8;
	*uParam3 = num7;
	*uParam4 = num6;
	*uParam5 = num5;
	*uParam6 = num4;
	*uParam7 = num3;
	return;
}

float func_142(float fParam0, float fParam1, float fParam2) // Position - 0x7E61
{
	float num;

	if (fParam1 == fParam2)
		return fParam1;

	num = fParam2 - fParam1;
	fParam0 = fParam0 - ((float)SYSTEM::ROUND((fParam0 - fParam1) / num) * num);

	if (fParam0 < fParam1)
		fParam0 = fParam0 + num;

	return fParam0;
}

int func_143(int iParam0, int iParam1) // Position - 0x7EA3
{
	if (iParam1 < 0)
		iParam1 = 0;

	switch (iParam0)
	{
		case 0:
		case 2:
		case 4:
		case 6:
		case 7:
		case 9:
		case 11:
			return 31;
	
		case 3:
		case 5:
		case 8:
		case 10:
			return 30;
	
		case 1:
			if (iParam1 % 4 == 0)
				if (iParam1 % 100 != 0)
					return 29;
				else if (iParam1 % 400 == 0)
					return 29;
		
			return 28;
	}

	return 30;
}

int func_144(int iParam0) // Position - 0x7F45
{
	return SYSTEM::SHIFT_RIGHT(iParam0, 20) & 63;
}

int func_145(int iParam0) // Position - 0x7F58
{
	return SYSTEM::SHIFT_RIGHT(iParam0, 14) & 63;
}

int func_146(int iParam0) // Position - 0x7F6B
{
	return SYSTEM::SHIFT_RIGHT(iParam0, 9) & 31;
}

int func_147(int iParam0) // Position - 0x7F7E
{
	return SYSTEM::SHIFT_RIGHT(iParam0, 4) & 31;
}

int func_148(int iParam0) // Position - 0x7F90
{
	return (SYSTEM::SHIFT_RIGHT(iParam0, 26) & 31 * IS_BIT_SET(iParam0, 31) ? -1 : 1) + 2011;
}

var func_149(BOOL bParam0, var uParam1, var uParam2) // Position - 0x7FB2
{
	if (bParam0)
		return uParam1;

	return uParam2;
}

int func_150(int iParam0) // Position - 0x7FC9
{
	return iParam0 & 15;
}

BOOL func_151(int iParam0, int iParam1) // Position - 0x7FD6
{
	int num;
	int num2;

	if (!func_159(iParam1) || !func_159(iParam0))
		return 1;

	num = func_148(iParam0);
	num2 = func_148(iParam1);

	if (num > num2)
		return 1;
	else if (num < num2)
		return 0;

	num = func_150(iParam0);
	num2 = func_150(iParam1);

	if (num > num2)
		return 1;
	else if (num < num2)
		return 0;

	num = func_147(iParam0);
	num2 = func_147(iParam1);

	if (num > num2)
		return 1;
	else if (num < num2)
		return 0;

	num = func_146(iParam0);
	num2 = func_146(iParam1);

	if (num > num2)
		return 1;
	else if (num < num2)
		return 0;

	num = func_145(iParam0);
	num2 = func_145(iParam1);

	if (num > num2)
		return 1;
	else if (num < num2)
		return 0;

	num = func_144(iParam0);
	num2 = func_144(iParam1);

	if (num > num2)
		return 1;

	return 0;
}

int func_152() // Position - 0x80E4
{
	var unk;

	func_158(&unk, unk_0x4BA5A16068183C5E());
	func_157(&unk, unk_0x18E502A71E28968C());
	func_156(&unk, unk_0x5295501D0862870D());
	func_155(&unk, unk_0xB12880C92EA6EE15());
	func_154(&unk, unk_0x8825A6E0A30BDCB8());
	func_153(&unk, unk_0x7598FE4545010A75());
	return unk;
}

void func_153(var uParam0, int iParam1) // Position - 0x812A
{
	if (iParam1 <= 0)
		return;

	if (iParam1 > 2043 || iParam1 < 1979)
		return;

	*uParam0 = *uParam0 - *uParam0 & 2080374784;

	if (iParam1 < 2011)
	{
		*uParam0 = *uParam0 || SYSTEM::SHIFT_LEFT(2011 - iParam1, 26);
		*uParam0 = *uParam0 | -2147483648;
	}
	else
	{
		*uParam0 = *uParam0 || SYSTEM::SHIFT_LEFT(iParam1 - 2011, 26);
		*uParam0 = *uParam0 - *uParam0 & -2147483648;
	}

	return;
}

void func_154(var uParam0, int iParam1) // Position - 0x81B0
{
	if (iParam1 < 0 || iParam1 > 11)
		return;

	*uParam0 = *uParam0 - *uParam0 & 15;
	*uParam0 = *uParam0 || iParam1;
	return;
}

void func_155(var uParam0, int iParam1) // Position - 0x81E3
{
	int num;
	int num2;

	num = func_150(*uParam0);
	num2 = func_148(*uParam0);

	if (iParam1 < 1 || iParam1 > func_143(num, num2))
		return;

	*uParam0 = *uParam0 - *uParam0 & 496;
	*uParam0 = *uParam0 || SYSTEM::SHIFT_LEFT(iParam1, 4);
	return;
}

void func_156(var uParam0, int iParam1) // Position - 0x8234
{
	if (iParam1 < 0 || iParam1 > 24)
		return;

	*uParam0 = *uParam0 - *uParam0 & 15872;
	*uParam0 = *uParam0 || SYSTEM::SHIFT_LEFT(iParam1, 9);
	return;
}

void func_157(var uParam0, int iParam1) // Position - 0x826E
{
	if (iParam1 < 0 || iParam1 >= 60)
		return;

	*uParam0 = *uParam0 - *uParam0 & 1032192;
	*uParam0 = *uParam0 || SYSTEM::SHIFT_LEFT(iParam1, 14);
	return;
}

void func_158(var uParam0, int iParam1) // Position - 0x82A9
{
	if (iParam1 < 0 || iParam1 >= 60)
		return;

	*uParam0 = *uParam0 - *uParam0 & 66060288;
	*uParam0 = *uParam0 || SYSTEM::SHIFT_LEFT(iParam1, 20);
	return;
}

BOOL func_159(int iParam0) // Position - 0x82E5
{
	int num;
	int num2;
	int num3;
	int num4;
	int num5;
	int num6;

	if (iParam0 == -15)
		return false;

	num = func_144(iParam0);

	if (num < 0 || num >= 60)
		return false;

	num2 = func_145(iParam0);

	if (num2 < 0 || num2 >= 60)
		return false;

	num3 = func_146(iParam0);

	if (num3 < 0 || num3 > 23)
		return false;

	num4 = func_148(iParam0);

	if (num4 <= 0 || num4 > 2043 || num4 < 1979)
		return false;

	num5 = func_150(iParam0);

	if (num5 < 0 || num5 > 11)
		return false;

	num6 = func_147(iParam0);

	if (num6 < 1 || num6 > func_143(num5, num4))
		return false;

	return true;
}

BOOL func_160(int iParam0, int iParam1) // Position - 0x83C1
{
	if (IS_BIT_SET(Global_114135.f_24999.f_8[iParam0], iParam1))
		return true;

	return false;
}

BOOL func_161(int iParam0) // Position - 0x83E1
{
	eCharacter character;

	if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
	{
		if (unk_0xFC8BFE4B41177C22(unk_0x4A8C381C258A124D()))
		{
			if (!unk_0x4FAFF4BCB7633475(unk_0x4A8C381C258A124D()))
			{
				character = _GET_CURRENT_PLAYER_CHARACTER();
			
				if (!func_53(character))
					return false;
			
				switch (iParam0)
				{
					case 9:
					case 0:
						if (!unk_0xFB1E434B5C3D0CEE(unk_0x259BE71D8A81D4FA()) || unk_0x3F18810075C77D41(unk_0x4A8C381C258A124D()) || unk_0x9D6DF8F3584AAC2B(unk_0x4A8C381C258A124D()) || unk_0x8BF5256C439DF778(unk_0x4A8C381C258A124D()) || unk_0x9857C978BD3CBEDA(unk_0x4A8C381C258A124D()) || unk_0x6652E84A43F51B8B(unk_0x259BE71D8A81D4FA(), 1) || unk_0xB96BD3A32937D44E(unk_0x259BE71D8A81D4FA()) || unk_0x1B32E388988DD296(unk_0x4A8C381C258A124D(), 0) || func_169() || Global_113182 || Global_33090 || func_168() || func_88(8, -1) || func_167() || func_166() || func_165() || func_164() || Global_114135.f_7692.f_919[character] == 5)
							return false;
						break;
				
					case 1:
						if (unk_0x6652E84A43F51B8B(unk_0x259BE71D8A81D4FA(), 1) || func_169() || Global_33090 || func_168() || func_88(8, -1) || func_165() || func_167() || func_166() || func_164() || Global_114135.f_7692.f_919[character] == 5)
							return false;
						break;
				
					case 2:
						if (!unk_0xFB1E434B5C3D0CEE(unk_0x259BE71D8A81D4FA()) || unk_0x3F18810075C77D41(unk_0x4A8C381C258A124D()) || unk_0x9D6DF8F3584AAC2B(unk_0x4A8C381C258A124D()) || unk_0x8BF5256C439DF778(unk_0x4A8C381C258A124D()) || unk_0x9857C978BD3CBEDA(unk_0x4A8C381C258A124D()) || unk_0x6652E84A43F51B8B(unk_0x259BE71D8A81D4FA(), 1) || unk_0xB96BD3A32937D44E(unk_0x259BE71D8A81D4FA()) || unk_0x1B32E388988DD296(unk_0x4A8C381C258A124D(), 0) || func_169() || Global_113182 || Global_33090 || func_168() || func_88(8, -1) || func_165() || func_167() || func_166() || func_164() || Global_114135.f_7692.f_919[character] == 5 || Global_44728 != -1)
							return false;
						break;
				
					case 3:
						if (unk_0x8BF5256C439DF778(unk_0x4A8C381C258A124D()) || unk_0x9857C978BD3CBEDA(unk_0x4A8C381C258A124D()) || unk_0x6652E84A43F51B8B(unk_0x259BE71D8A81D4FA(), 1) || unk_0x1B32E388988DD296(unk_0x4A8C381C258A124D(), 0) || func_169() || Global_113182 || Global_33090 || func_168() || func_88(8, -1) || func_167() || func_166() || func_164() || Global_114135.f_7692.f_919[character] == 5)
							return false;
						break;
				
					case 4:
						if (func_169() || unk_0xE7B45027762DEFE7(unk_0x259BE71D8A81D4FA()) > 0 || func_88(8, -1) || func_164() || func_163() || Global_114135.f_7692.f_919[character] == 5)
							return false;
						break;
				
					case 5:
						if (func_88(8, -1) || func_167() || func_166() || func_163() || func_162())
							return false;
					
						if (unk_0x3555462DB47B7AB1() && unk_0x1403FEB4554982F8() != 3 && unk_0x6D231A0D52134FC1() < 8)
							return false;
						break;
				
					case 6:
						if (unk_0xFC8BFE4B41177C22(unk_0x4A8C381C258A124D()))
							if (unk_0x1B32E388988DD296(unk_0x4A8C381C258A124D(), 0) || unk_0xE7B45027762DEFE7(unk_0x259BE71D8A81D4FA()) > 0 || unk_0x3F18810075C77D41(unk_0x4A8C381C258A124D()) || unk_0x8BF5256C439DF778(unk_0x4A8C381C258A124D()) || unk_0x9857C978BD3CBEDA(unk_0x4A8C381C258A124D()) || unk_0x6652E84A43F51B8B(unk_0x259BE71D8A81D4FA(), 1) || unk_0xB96BD3A32937D44E(unk_0x259BE71D8A81D4FA()) || func_169() || Global_33090 || func_168() || func_88(8, -1) || func_166() || func_165() || func_164() || Global_114135.f_7692.f_919[character] == 5)
								return false;
						break;
				
					case 7:
						if (unk_0x1B32E388988DD296(unk_0x4A8C381C258A124D(), 0) || !unk_0xE916D57851F785AB(unk_0x259BE71D8A81D4FA()) || !unk_0xFB1E434B5C3D0CEE(unk_0x259BE71D8A81D4FA()) || !unk_0x9390801B06EE998F() || unk_0x3F18810075C77D41(unk_0x4A8C381C258A124D()) || unk_0x8BF5256C439DF778(unk_0x4A8C381C258A124D()) || unk_0x9857C978BD3CBEDA(unk_0x4A8C381C258A124D()) || unk_0x6652E84A43F51B8B(unk_0x259BE71D8A81D4FA(), 1) || func_169() || func_166() || Global_113182 || Global_33090 || func_168() || Global_45392 || func_88(8, -1) || func_165() || func_163() || func_164() || Global_114135.f_7692.f_919[character] == 5)
							return false;
						break;
				
					case 8:
						if (unk_0x1B32E388988DD296(unk_0x4A8C381C258A124D(), 0) || !unk_0xE916D57851F785AB(unk_0x259BE71D8A81D4FA()) || !unk_0xFB1E434B5C3D0CEE(unk_0x259BE71D8A81D4FA()) || !unk_0x9390801B06EE998F() || unk_0x3EE1A6D743332852(unk_0x259BE71D8A81D4FA(), 0) || unk_0x3F18810075C77D41(unk_0x4A8C381C258A124D()) || unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 1) || unk_0x8BF5256C439DF778(unk_0x4A8C381C258A124D()) || unk_0x9857C978BD3CBEDA(unk_0x4A8C381C258A124D()) || unk_0x2CFBD7757B4D922F(unk_0x4A8C381C258A124D()) || unk_0x6652E84A43F51B8B(unk_0x259BE71D8A81D4FA(), 1) || unk_0xB96BD3A32937D44E(unk_0x259BE71D8A81D4FA()) || func_169() || Global_113182 || Global_33090 || func_168() || func_88(8, -1) || func_165() || func_163() || func_167() || func_166() || func_164())
							return false;
						break;
				}
			}
			else
			{
				return false;
			}
		}
		else
		{
			return false;
		}
	}
	else
	{
		return false;
	}

	return true;
}

BOOL func_162() // Position - 0x8AFE
{
	return Global_101727.f_1;
}

BOOL func_163() // Position - 0x8B0C
{
	if (Global_98585 != -1)
		return IS_BIT_SET(Global_92451[Global_98585 /*34*/].f_15, 13);

	return false;
}

BOOL func_164() // Position - 0x8B2F
{
	if (unk_0x486FF5D06E9659F1(joaat("player_timetable_scene")) > 0)
		return true;

	return false;
}

BOOL func_165() // Position - 0x8B49
{
	if (Global_79800)
		return true;
	else if (Global_64302 && !Global_64308)
		return true;

	return false;
}

BOOL func_166() // Position - 0x8B73
{
	return Global_101740.f_400 > 0;
}

BOOL func_167() // Position - 0x8B84
{
	return Global_101740.f_399 > 0;
}

BOOL func_168() // Position - 0x8B95
{
	return Global_1575084;
}

BOOL func_169() // Position - 0x8BA1
{
	if (!unk_0x76CD105BCAC6EB9F())
		return Global_99141.f_44 == 1;

	return false;
}

BOOL func_170() // Position - 0x8BBD
{
	_UPDATE_CURRENT_PLAYER_CHARACTER();

	if (Global_9741[Global_21066 /*2811*/][0 /*281*/].f_259 == 2)
		return true;

	return false;
}

BOOL func_171(int iParam0) // Position - 0x8BE5
{
	return func_151(func_152(), iParam0);
}

BOOL func_172(int iParam0, int iParam1, eCharacter echParam2) // Position - 0x8BF7
{
	BOOL num;
	eCharacter character;

	num = 0;
	character = _GET_CURRENT_PLAYER_CHARACTER();

	if (echParam2 != 145 && echParam2 == 0 || echParam2 == 1 || echParam2 == 2)
		character = echParam2;

	switch (iParam0)
	{
		case 18:
			if (character == CHAR_TREVOR)
				num = 1;
			break;
	
		case 19:
			if (character != CHAR_TREVOR)
				num = 1;
			break;
	
		case 20:
			if (character != CHAR_FRANKLIN)
				num = 1;
			break;
	
		case 28:
			if (character != CHAR_TREVOR)
				num = 1;
			break;
	
		case 13:
			if (character == CHAR_MICHAEL)
				if (iParam1 == 2)
					num = 0;
				else
					num = 1;
			else
				num = 1;
			break;
	
		case 22:
			if (iParam1 == 2 || character != CHAR_TREVOR)
				num = 1;
			break;
	
		case 30:
			if (character != CHAR_TREVOR)
				num = 1;
			break;
	
		default:
			num = 1;
			break;
	}

	return num;
}

int func_173(eCharacter echParam0) // Position - 0x8CDB
{
	if (!func_53(echParam0))
		return 7;

	return Global_114135.f_7692.f_919[echParam0];
}

BOOL func_174(int iParam0) // Position - 0x8CFF
{
	int num;
	BOOL flag;

	if (iParam0 == 31 || iParam0 == 32)
		if (!_IS_EXCLUSIVE_CONTENT_UNLOCKED())
			return false;

	num = iParam0;

	if (num < 31)
	{
		flag = IS_BIT_SET(Global_114135.f_24999, num);
	}
	else
	{
		num = num - 31;
		flag = IS_BIT_SET(Global_114135.f_24999.f_1, num);
	}

	return flag;
}

BOOL func_175() // Position - 0x8D57
{
	var unk;

	if (Global_33239)
	{
		if (unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
		{
			unk = unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0);
		
			if (unk_0xD9F5E1FEFD1329E4(unk, 0))
				if (!unk_0x4FAFF4BCB7633475(unk_0xFD5C5BBD1FE92BB7(unk, 0, 0)))
					return true;
		}
	}

	return false;
}

BOOL _IS_MISSION_REPEAT_ACTIVE(BOOL bExcludeBenchmark) // Position - 0x8D9B
{
	if (!bExcludeBenchmark && unk_0x486FF5D06E9659F1(joaat("benchmark")) > 0)
		return true;

	return IS_BIT_SET(Global_79788, 0);
}

BOOL _IS_EXCLUSIVE_CONTENT_UNLOCKED() // Position - 0x8DC3
{
	var unk;

	if (Global_153015 == 2)
		return true;
	else if (Global_153015 == 3)
		return false;

	if (unk_0x5F9F81C08516558E())
	{
		if (unk_0x1595D1B690089487())
		{
			if (unk_0x5BC93955B6EEBC0B())
			{
				unk_0xDF7F16323520B858(joaat("SP_UNLOCK_EXCLUS_CONTENT"), &unk, -1);
				unk_0x0B0C9A0F9AAEB7F0(&unk, 2);
				unk_0x0B0C9A0F9AAEB7F0(&unk, 4);
				unk_0x0B0C9A0F9AAEB7F0(&unk, 6);
				unk_0x0B0C9A0F9AAEB7F0(&Global_28, 2);
				unk_0x0B0C9A0F9AAEB7F0(&Global_28, 4);
				unk_0x0B0C9A0F9AAEB7F0(&Global_28, 6);
				unk_0x1164A75E490C27B6(joaat("SP_UNLOCK_EXCLUS_CONTENT"), unk, 1);
			
				if (unk_0x76CA59C648318506())
				{
					unk = unk_0x38640D2193CB547F(866);
					unk_0x0B0C9A0F9AAEB7F0(&unk, 0);
					unk_0xED11291F7127888E(unk);
				}
			
				return true;
			}
		}
	}

	if (unk_0x76CA59C648318506())
		if (IS_BIT_SET(unk_0x38640D2193CB547F(866), 0))
			return true;

	return false;
}

void func_178() // Position - 0x8E7B
{
	if (unk_0x9FA91AA1AC724CD0())
	{
		switch (iLocal_47)
		{
			case 0:
				break;
		
			case 1:
				break;
		
			case 2:
				func_207(6);
				func_192();
				break;
		
			case 3:
				func_207(5);
				func_192();
				break;
		}
	
		if (unk_0xFC8BFE4B41177C22(uLocal_53))
		{
			if (!unk_0x4FAFF4BCB7633475(uLocal_53) && !unk_0x1C2F771CDC87A3A5(uLocal_53, 0))
			{
				unk_0x1C2F771CDC87A3A5(uLocal_52, 0);
			
				if (unk_0xFC8BFE4B41177C22(uLocal_52))
					unk_0xFD251F92B546F389(uLocal_53, unk_0xD1A6A821F5AC81DB(uLocal_52, 1), 9000f, -1, 0, 0);
				else
					unk_0xFD251F92B546F389(uLocal_53, unk_0xD1A6A821F5AC81DB(uLocal_53, 1), 9000f, -1, 0, 0);
			
				unk_0xAAA71DD7E9059338(uLocal_53, 0);
				unk_0x44FB298D6382876D(uLocal_53, 1);
			}
		}
	
		if (unk_0xFC8BFE4B41177C22(uLocal_52))
		{
			if (!unk_0x4FAFF4BCB7633475(uLocal_52))
			{
				unk_0x44FB298D6382876D(uLocal_52, 1);
				unk_0xAAA71DD7E9059338(uLocal_52, 0);
				unk_0x91B8DEBF573007DE(uLocal_52, 1);
				unk_0x3F58BFCF656F0DF1(uLocal_52, 1);
			}
		}
	
		if (unk_0xFC8BFE4B41177C22(uLocal_54))
		{
			if (!unk_0x4FAFF4BCB7633475(uLocal_54))
			{
				if (unk_0x3EE1A6D743332852(unk_0x259BE71D8A81D4FA(), 0))
					func_65();
			
				unk_0xAAA71DD7E9059338(uLocal_54, 0);
				unk_0x91B8DEBF573007DE(uLocal_54, 1);
			}
		}
	
		if (func_191())
		{
			unk_0x425BBE19F25A57AB(1f);
			unk_0xD414C47AFF81382A(2, uLocal_70, uLocal_69);
			unk_0xD414C47AFF81382A(2, uLocal_69, uLocal_70);
		}
	
		if (bLocal_75)
			unk_0xAE03F542B985A69E(1);
	}

	unk_0x2AF2D6F164BD6F5A(2551.0378f, 4708.6133f, 32.6775f, 2536.979f, 5022.1787f, 43.8519f, 300f, 1);

	if (bLocal_285)
	{
		unk_0xF165EAD0AA08F3B6(unk_0x259BE71D8A81D4FA(), 3, 0);
		unk_0x42C9A22D6724F283(unk_0x259BE71D8A81D4FA(), 0);
	}

	func_179(-1);
	SYSTEM::WAIT(0);
	unk_0xBBC29EBE6E1A48FA();
	return;
}

void func_179(int iParam0) // Position - 0x900A
{
	var unk;

	if (iParam0 == -1)
		iParam0 = func_133();

	if (iParam0 == -1)
		return;

	if (func_191())
	{
		func_184(iParam0);
		unk_0xB51A8DA1DD0D2E4E(0, 0);
		Global_114126 = unk_0x1DD05E817C89C737();
		func_183(30000);
		TEXT_LABEL_ASSIGN_STRING(&unk, func_182(Global_114124, true), 64);
	
		if (func_181(Global_114124) > 0)
		{
			TEXT_LABEL_APPEND_STRING(&unk, " Variation ", 64);
			TEXT_LABEL_APPEND_INT(&unk, Global_114123, 64);
		}
	
		unk_0x3AF124228E3EAF07(&unk, Global_114121, unk_0x1DD05E817C89C737() - Global_114122, 0);
	}
	else if (IS_BIT_SET(Global_114131, 0) && Global_114135.f_24999.f_2 < 3)
	{
		unk_0x8744D2E3FC95740E(&Global_114131, 0);
	}

	func_180(&Global_33147);
	Global_114125 = false;
	func_132(-1);
	return;
}

void func_180(var uParam0) // Position - 0x90BC
{
	if (*uParam0 == -1)
		return;

	if (!(*uParam0 == Global_44143))
	{
		*uParam0 = -1;
		return;
	}

	*uParam0 = -1;
	Global_44142 = 0;
	Global_44144 = 0;
	Global_44181 = 15;
	Global_64305 = 0;
	Global_64306 = 0;
	return;
}

int func_181(int iParam0) // Position - 0x90F9
{
	int num;

	num = 1;

	switch (iParam0)
	{
		case 1:
			num = 5;
			break;
	
		case 5:
			num = 2;
			break;
	
		case 9:
			num = 10;
			break;
	
		case 11:
			num = 4;
			break;
	
		case 13:
			num = 4;
			break;
	
		case 15:
			num = 2;
			break;
	
		case 17:
			num = 2;
			break;
	
		case 22:
			num = 2;
			break;
	
		case 23:
			num = 2;
			break;
	
		case 25:
			num = 3;
			break;
	
		case 26:
			num = 2;
			break;
	
		case 27:
			num = 2;
			break;
	
		case 29:
			num = 3;
			break;
	}

	return num;
}

char* func_182(int iParam0, BOOL bParam1) // Position - 0x91AA
{
	switch (iParam0)
	{
		case 0:
			return "RE_ACCIDENT";
	
		case 1:
			return "RE_ATMROBBERY";
	
		case 2:
			return "RE_BUSTOUR";
	
		case 3:
			return "RE_DOMESTIC";
	
		case 4:
			return "RE_GETAWAYDRIVER";
	
		case 5:
			return "RE_SHOPROBBERY";
	
		case 6:
			return "RE_SNATCHED";
	
		case 7:
			return "RE_LURED";
	
		case 8:
			return "RE_BIKETHIEFSTAMP";
	
		case 9:
			return "RE_SECURITYVAN";
	
		case 10:
			return "RE_PAPARAZZI";
	
		case 11:
			return "RE_CHASETHIEVES";
	
		case 12:
			return "RE_DEALGONEWRONG";
	
		case 13:
			return "RE_HITCHLIFT";
	
		case 14:
			return "RE_STAG";
	
		case 15:
			return "RE_ARREST";
	
		case 16:
			return "RE_CRASHRESCUE";
	
		case 17:
			return "RE_CARTHEFT";
	
		case 18:
			return "RE_CULTSHOOTOUT";
	
		case 19:
			return "RE_GANGFIGHT";
	
		case 20:
			return "RE_GANGINTIMIDATION";
	
		case 21:
			return "RE_PRISONVANBREAK";
	
		case 22:
			return "RE_PRISONERLIFT";
	
		case 23:
			return "RE_ABANDONEDCAR";
	
		case 24:
			return "RE_BURIAL";
	
		case 25:
			return "RE_MUGGING";
	
		case 26:
			return "RE_BIKETHIEF";
	
		case 27:
			return "RE_DRUNKDRIVER";
	
		case 28:
			return "RE_HOMELANDSECURITY";
	
		case 29:
			return "RE_BORDERPATROL";
	
		case 30:
			return "RE_SIMEONYETARIAN";
	
		case 31:
			return "RE_DUEL";
	
		case 32:
			return "RE_SEAPLANE";
	
		case 33:
			return "RE_MONKEYPHOTO";
	
		case -1:
			return "RE_NONE";
	}

	!bParam1;
	return "UNKNOWN";
}

void func_183(int iParam0) // Position - 0x93F3
{
	Global_44732 = unk_0x1DD05E817C89C737() + iParam0;
	return;
}

void func_184(int iParam0) // Position - 0x9405
{
	func_185(iParam0, 0, func_190(iParam0));
	return;
}

void func_185(int iParam0, int iParam1, int iParam2) // Position - 0x941A
{
	int unk;
	var unk2;

	unk = func_152();
	func_188(&unk, 0, 0, iParam2, iParam1, 0, 0);
	func_187(iParam0, &unk);
	unk2 = { func_186(&unk) };
	return;
}

struct<16> func_186(var uParam0) // Position - 0x9449
{
	var unk;
	int num;

	TEXT_LABEL_ASSIGN_STRING(&unk, "", 64);
	num = func_146(*uParam0);

	if (num < 10)
		TEXT_LABEL_APPEND_INT(&unk, 0, 64);

	TEXT_LABEL_APPEND_INT(&unk, num, 64);
	TEXT_LABEL_APPEND_STRING(&unk, ":", 64);
	num = func_145(*uParam0);

	if (num < 10)
		TEXT_LABEL_APPEND_INT(&unk, 0, 64);

	TEXT_LABEL_APPEND_INT(&unk, num, 64);
	TEXT_LABEL_APPEND_STRING(&unk, ":", 64);
	num = func_144(*uParam0);

	if (num < 10)
		TEXT_LABEL_APPEND_INT(&unk, 0, 64);

	TEXT_LABEL_APPEND_INT(&unk, num, 64);
	TEXT_LABEL_APPEND_STRING(&unk, "  ", 64);
	num = func_147(*uParam0);

	if (num < 10)
		TEXT_LABEL_APPEND_INT(&unk, 0, 64);

	TEXT_LABEL_APPEND_INT(&unk, num, 64);
	TEXT_LABEL_APPEND_STRING(&unk, "/", 64);
	num = func_150(*uParam0);

	if (num < 9)
		TEXT_LABEL_APPEND_INT(&unk, 0, 64);

	TEXT_LABEL_APPEND_INT(&unk, num + 1, 64);
	TEXT_LABEL_APPEND_STRING(&unk, "/", 64);
	TEXT_LABEL_APPEND_INT(&unk, func_148(*uParam0), 64);
	return unk;
}

void func_187(int iParam0, var uParam1) // Position - 0x951A
{
	Global_114135.f_24999.f_43[iParam0] = *uParam1;
	return;
}

void func_188(var uParam0, int iParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6) // Position - 0x9532
{
	int num;
	int i;
	int num2;
	int j;
	int k;
	int l;
	int m;

	num = func_148(*uParam0);
	i = func_150(*uParam0);
	num2 = func_147(*uParam0);
	j = func_146(*uParam0);
	k = func_145(*uParam0);
	l = func_144(*uParam0);

	if (iParam6 == 0 && iParam5 == 0 && iParam4 == 0 && iParam3 == 0 && iParam2 == 0 && iParam1 == 0)
		return;

	if (iParam1 < 0)
		return;

	if (iParam2 < 0)
		return;

	if (iParam3 < 0)
		return;

	if (iParam4 < 0)
		return;

	if (iParam5 < 0)
		return;

	if (iParam6 < 0)
		return;

	for (l = l + iParam1; l >= 60; l = l - 60)
	{
		iParam2 = iParam2 + 1;
	}

	for (k = k + iParam2; k >= 60; k = k - 60)
	{
		iParam3 = iParam3 + 1;
	}

	for (j = j + iParam3; j >= 24; j = j - 24)
	{
		iParam4 = iParam4 + 1;
	}

	num2 = num2 + iParam4;

	for (m = func_143(i, num); num2 > m; m = func_143(i, num))
	{
		i = i + 1;
		num2 = num2 - m;
	
		if (i > 11)
		{
			num = num + 1;
			i = i - 12;
		}
	}

	for (i = i + iParam5; i > 11; i = i - 12)
	{
		iParam6 = iParam6 + 1;
	}

	num = num + iParam6;
	func_189(uParam0, l, k, j, num2, i, num);
	return;
}

void func_189(var uParam0, int iParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6) // Position - 0x96B4
{
	func_158(uParam0, iParam1);
	func_157(uParam0, iParam2);
	func_156(uParam0, iParam3);
	func_154(uParam0, iParam5);
	func_155(uParam0, iParam4);
	func_153(uParam0, iParam6);
	return;
}

int func_190(int iParam0) // Position - 0x96EC
{
	int num;

	switch (iParam0)
	{
		case 23:
			num = 30;
			break;
	
		case 0:
			num = 30;
			break;
	
		case 15:
			num = 30;
			break;
	
		case 1:
			num = 200;
			break;
	
		case 26:
			num = 30;
			break;
	
		case 8:
			num = 30;
			break;
	
		case 29:
			num = 30;
			break;
	
		case 24:
			num = 30;
			break;
	
		case 2:
			num = 0;
			break;
	
		case 17:
			num = 30;
			break;
	
		case 11:
			num = 30;
			break;
	
		case 16:
			num = 30;
			break;
	
		case 18:
			num = 30;
			break;
	
		case 12:
			num = 120;
			break;
	
		case 3:
			num = 60;
			break;
	
		case 27:
			num = 60;
			break;
	
		case 19:
			num = 30;
			break;
	
		case 20:
			num = 30;
			break;
	
		case 4:
			num = 60;
			break;
	
		case 28:
			num = 30;
			break;
	
		case 13:
			num = 35;
			break;
	
		case 7:
			num = 30;
			break;
	
		case 25:
			num = 40;
			break;
	
		case 10:
			num = 30;
			break;
	
		case 22:
			num = 30;
			break;
	
		case 21:
			num = 30;
			break;
	
		case 5:
			num = 30;
			break;
	
		case 30:
			num = 60;
			break;
	
		case 9:
			num = 60;
			break;
	
		case 6:
			num = 40;
			break;
	
		case 14:
			num = 40;
			break;
	}

	return num;
}

BOOL func_191() // Position - 0x988F
{
	if (Global_114124 == func_133() && unk_0x9FA91AA1AC724CD0() && Global_114125)
		return true;

	return false;
}

void func_192() // Position - 0x98BA
{
	if (!bLocal_74)
		func_206();

	func_195(15, iLocal_51);
	func_193();
	return;
}

void func_193() // Position - 0x98D9
{
	func_194();
	return;
}

int func_194() // Position - 0x98E6
{
	if (_IS_MISSION_REPEAT_ACTIVE(false))
		return 0;

	if (Global_101727.f_8)
		if (Global_101727.f_10 > 0)
			return 0;
	else if (Global_101727.f_10 > 1)
		return 0;

	Global_101727.f_10 = Global_101727.f_10 + 1;
	return 1;
}

void func_195(int iParam0, int iParam1) // Position - 0x9931
{
	if (iParam0 == -1)
		iParam0 = func_133();

	if (iParam0 == -1)
		return;

	if (iParam1 <= func_181(iParam0))
	{
		func_205(iParam0, iParam1);
	
		if (!func_204(51))
		{
			func_201("RE_REWARD" /*Some Random Events will reward the player with stat boosts or money.*/, 1, 0, 4000, 10000, func_123(), 0, 138, 0);
			func_200(51);
		}
	
		if (func_102(iParam0))
			Global_114135.f_24999.f_2 = 3;
	
		if (func_199(iParam0, iParam1) != 322)
			func_196(func_199(iParam0, iParam1), uLocal_42, uLocal_42.f_1);
	
		Global_114123 = iParam1;
	
		if (Global_114121 == 0)
			if (Global_114124 == 1 || Global_114124 == 5 || Global_114124 == 11 || Global_114124 == 25)
				func_207(2);
			else if (Global_114124 == 26 || Global_114124 == 8 || Global_114124 == 17)
				func_207(7);
			else
				func_207(1);
	}

	return;
}

void func_196(int iParam0, var uParam1, var uParam2) // Position - 0x9A35
{
	BOOL flag;

	iParam0 < 0;

	if (iParam0 == 321 || iParam0 > 321)
	{
	}
	else
	{
		_STAT_SET_PACKED_BOOL(891 + iParam0, 1, -1);
	}

	flag = true;

	if (Global_114135.f_10198[iParam0 /*12*/].f_5 == 1)
	{
		if (Global_114135.f_10198[iParam0 /*12*/].f_6 == 11 || Global_114135.f_10198[iParam0 /*12*/].f_6 == 12)
			flag = false;
	}
	else
	{
		Global_114135.f_10198[iParam0 /*12*/].f_5 = 1;
		Global_114135.f_10198[iParam0 /*12*/].f_10 = uParam1;
		Global_114135.f_10198[iParam0 /*12*/].f_11 = uParam2;
	
		if (iParam0 == 287)
			unk_0x8DD7F37773EBD5B9(joaat("NUM_HIDDEN_PACKAGES_0"), 50, 0);
	
		if (iParam0 == 286)
			unk_0x8DD7F37773EBD5B9(joaat("NUM_HIDDEN_PACKAGES_1"), 50, 0);
	
		if (iParam0 == 299)
			unk_0x8DD7F37773EBD5B9(joaat("NUM_HIDDEN_PACKAGES_3"), 50, 0);
	}

	if (flag)
		func_197();

	return;
}

void func_197() // Position - 0x9B1B
{
	int num;
	float num2;
	float num3;
	float num4;
	float num5;
	float num6;
	float num7;
	float num8;
	float value;
	int num9;

	num = 0;
	Global_113871 = 0;
	Global_113872 = 0;
	Global_113873 = 0;
	Global_113874 = 0;
	Global_113875 = 0;
	Global_113876 = 0;
	Global_113877 = 0;
	num2 = 0f;
	num3 = 0f;
	num4 = 0f;
	num5 = 0f;
	num6 = 0f;
	num7 = 0f;
	num8 = 0f;
	value = Global_114135.f_10198.f_3853;
	Global_114135.f_10198.f_3853 = 0f;

	while (num < 321)
	{
		if (Global_114135.f_10198[num /*12*/].f_5 == 1)
		{
			switch (Global_114135.f_10198[num /*12*/].f_6)
			{
				case 1:
					Global_113871 = Global_113871 + 1;
					num2 = num2 + Global_114135.f_10198[num /*12*/].f_4;
					break;
			
				case 3:
					Global_113872 = Global_113872 + 1;
					num3 = num3 + Global_114135.f_10198[num /*12*/].f_4;
					break;
			
				case 5:
					Global_113873 = Global_113873 + 1;
					num4 = num4 + Global_114135.f_10198[num /*12*/].f_4;
					break;
			
				case 7:
					Global_113874 = Global_113874 + 1;
					num5 = num5 + Global_114135.f_10198[num /*12*/].f_4;
					break;
			
				case 9:
					Global_113875 = Global_113875 + 1;
					num6 = num6 + (Global_114135.f_10198[num /*12*/].f_4 * 4f);
					break;
			
				case 11:
					Global_113876 = Global_113876 + 1;
					num7 = num7 + Global_114135.f_10198[num /*12*/].f_4;
					break;
			
				case 13:
					Global_113877 = Global_113877 + 1;
					num8 = num8 + Global_114135.f_10198[num /*12*/].f_4;
					break;
			
				default:
					break;
			}
		}
	
		num = num + 1;
	}

	if (Global_113854 > 0)
		if (Global_113871 == Global_113854)
			num2 = 55f;

	if (Global_113855 > 0)
		if (Global_113872 == Global_113855)
			num3 = 10f;

	if (Global_113856 > 0)
		if (Global_113873 == Global_113856)
			num4 = 0f;

	if (Global_113857 > 0)
		if (Global_113874 == Global_113857)
			num5 = 10f;

	if (Global_113858 > 0)
	{
		if (Global_113875 == Global_113858 || (Global_113858 * 10) / Global_113875 < 41 || Global_113875 > Global_113861 || Global_113875 == Global_113861)
		{
			if (!IS_BIT_SET(Global_114135.f_10198.f_3856, 14))
			{
				if (Global_113875 == Global_113858)
				{
					unk_0x8DD7F37773EBD5B9(joaat("NUM_RNDEVENTS_COMPLETED"), Global_113858, 0);
					unk_0x0B0C9A0F9AAEB7F0(&(Global_114135.f_10198.f_3856), 14);
				}
			}
		
			num6 = 5f;
		}
	}

	if (Global_113859 > 0)
		if (Global_113876 == Global_113859)
			num7 = 15f;

	if (Global_113860 > 0)
		if (Global_113877 == Global_113860)
			num8 = 5f;

	Global_114135.f_10198.f_3853 = num2 + num3 + num4 + num5 + num6 + num7 + num8;

	if (Global_113875 > Global_113861 || Global_113875 == Global_113861)
		num9 = Global_113861;
	else
		num9 = Global_113875;

	unk_0x1164A75E490C27B6(joaat("NUM_MISSIONS_COMPLETED"), Global_113871, 1);
	unk_0x1164A75E490C27B6(joaat("NUM_MISSIONS_AVAILABLE"), Global_113854, 1);
	unk_0x1164A75E490C27B6(joaat("NUM_MINIGAMES_COMPLETED"), Global_113872, 1);
	unk_0x1164A75E490C27B6(joaat("NUM_MINIGAMES_AVAILABLE"), Global_113855, 1);
	unk_0x1164A75E490C27B6(joaat("NUM_ODDJOBS_COMPLETED"), Global_113873, 1);
	unk_0x1164A75E490C27B6(joaat("NUM_ODDJOBS_AVAILABLE"), Global_113856, 1);
	unk_0x1164A75E490C27B6(joaat("NUM_RNDPEOPLE_COMPLETED"), Global_113874, 1);
	unk_0x1164A75E490C27B6(joaat("NUM_RNDPEOPLE_AVAILABLE"), Global_113857, 1);
	unk_0x1164A75E490C27B6(joaat("NUM_RNDEVENTS_COMPLETED"), num9, 1);
	unk_0x1164A75E490C27B6(joaat("NUM_RNDEVENTS_AVAILABLE"), Global_113861, 1);
	unk_0x1164A75E490C27B6(joaat("NUM_MISC_COMPLETED"), Global_113877 + Global_113876, 1);
	unk_0x1164A75E490C27B6(joaat("NUM_MISC_AVAILABLE"), Global_113860 + Global_113859, 1);
	Global_113878 = (Global_113871 * 100) / Global_113854;
	Global_113880 = ((Global_113873 + Global_113872) * 100) / (Global_113856 + Global_113855);
	Global_113879 = ((Global_113874 + num9) * 100) / (Global_113857 + Global_113861);
	Global_113881 = ((Global_113876 + Global_113877) * 100) / (Global_113859 + Global_113860);
	unk_0x4F8678C02360C3D2(joaat("TOTAL_PROGRESS_MADE"), Global_114135.f_10198.f_3853, 1);
	unk_0x1164A75E490C27B6(joaat("PERCENT_STORY_MISSIONS"), Global_113878, 1);
	unk_0x1164A75E490C27B6(joaat("PERCENT_AMBIENT_MISSIONS"), Global_113879, 1);
	unk_0x1164A75E490C27B6(joaat("PERCENT_ODDJOBS"), Global_113880, 1);

	if (value > 0f && SYSTEM::FLOOR(value) < SYSTEM::FLOOR(Global_114135.f_10198.f_3853))
		func_46(13, SYSTEM::FLOOR(Global_114135.f_10198.f_3853));

	if (!unk_0xB00920C3C99A34C5())
	{
		if (!Global_79539)
		{
			if (func_198() == 2 == false && !unk_0x76CD105BCAC6EB9F())
			{
				if (unk_0x5676319ACE5BEC37())
					Global_113869 = 0;
			
				if (!Global_64308)
					func_194();
			}
		}
	}

	return;
}

int func_198() // Position - 0x9FD9
{
	return Global_33087;
}

int func_199(int iParam0, int iParam1) // Position - 0x9FE4
{
	switch (iParam0)
	{
		case 0:
			return 250;
	
		case 1:
			return 226;
	
		case 2:
			return 243;
	
		case 3:
			return 256;
	
		case 4:
			return 259;
	
		case 5:
			if (iParam1 == 1)
				return 281;
		
			if (iParam1 == 2)
				return 282;
			break;
	
		case 6:
			return 265;
	
		case 7:
			return 218;
	
		case 9:
			if (iParam1 == 1)
				return 271;
		
			if (iParam1 == 2)
				return 272;
		
			if (iParam1 == 3)
				return 273;
		
			if (iParam1 == 4)
				return 274;
		
			if (iParam1 == 5)
				return 275;
		
			if (iParam1 == 6)
				return 276;
		
			if (iParam1 == 7)
				return 277;
		
			if (iParam1 == 8)
				return 278;
		
			if (iParam1 == 9)
				return 279;
		
			if (iParam1 == 10)
				return 280;
			break;
	
		case 10:
			return 219;
	
		case 11:
			if (iParam1 == 1)
				return 246;
		
			if (iParam1 == 2)
				return 247;
		
			if (iParam1 == 3)
				return 248;
		
			if (iParam1 == 4)
				return 249;
			break;
	
		case 12:
			return 254;
	
		case 13:
			if (iParam1 == 1)
				return 260;
		
			if (iParam1 == 2)
				return 261;
		
			if (iParam1 == 3)
				return 262;
		
			if (iParam1 == 4)
				return 264;
			break;
	
		case 14:
			return 283;
	
		case 15:
			if (iParam1 == 1)
				return 224;
		
			if (iParam1 == 2)
				return 225;
			break;
	
		case 16:
			return 252;
	
		case 17:
			if (iParam1 == 1)
				return 244;
		
			if (iParam1 == 2)
				return 245;
			break;
	
		case 18:
			return 253;
	
		case 19:
			return 215;
	
		case 20:
			return 216;
	
		case 21:
			return 251;
	
		case 22:
			if (iParam1 == 1)
				return 221;
		
			if (iParam1 == 2)
				return 222;
			break;
	
		case 23:
			if (iParam1 == 1)
				return 213;
		
			if (iParam1 == 2)
				return 214;
			break;
	
		case 24:
			return 242;
	
		case 25:
			if (iParam1 == 1)
				return 267;
		
			if (iParam1 == 2)
				return 268;
		
			if (iParam1 == 3)
				return 269;
			break;
	
		case 8:
			return 255;
	
		case 26:
			if (iParam1 == 1)
				return 227;
		
			if (iParam1 == 2)
				return 228;
			break;
	
		case 27:
			if (iParam1 == 1)
				return 257;
		
			if (iParam1 == 2)
				return 258;
			break;
	
		case 28:
			return 217;
	
		case 29:
			if (iParam1 == 1)
				return 229;
		
			if (iParam1 == 2)
				return 230;
		
			if (iParam1 == 3)
				return 231;
			break;
	
		case 30:
			return 285;
	
		case 31:
			return 318;
	
		case 32:
			return 319;
	
		case 33:
			return 320;
	}

	return 322;
}

void func_200(int iParam0) // Position - 0xA358
{
	int num;
	int i;

	num = iParam0;

	for (i = 0; num > 31; i = i + 1)
	{
		num = num - 32;
	}

	if (i < 3)
		unk_0x0B0C9A0F9AAEB7F0(&Global_114135.f_20414.f_150[i], num);

	return;
}

void func_201(char* sParam0, int iParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6, int iParam7, int iParam8) // Position - 0xA39A
{
	func_202(sParam0, "", iParam1, iParam2, iParam3, iParam4, iParam5, iParam6, iParam7, iParam8);
	return;
}

void func_202(char* sParam0, char* sParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6, int iParam7, int iParam8, int iParam9) // Position - 0xA3BC
{
	int i;

	if (unk_0x1B79E937E91F40C3(sParam0, ""))
		return;

	if (iParam3 < 0)
		return;

	if (iParam5 < 500 && iParam5 != -1)
		return;

	if (iParam4 < 0 && iParam4 != -1)
		return;

	if (iParam6 < 1 || iParam6 > 7)
		return;

	if (iParam7 == 235)
		return;

	if (iParam8 == 235)
		return;

	for (i = 0; i < Global_114135.f_20414.f_145; i = i + 1)
	{
		if (unk_0x1B79E937E91F40C3(&Global_114135.f_20414[i /*16*/], sParam0))
			return;
	}

	if (Global_114135.f_20414.f_145 < 9)
	{
		TEXT_LABEL_ASSIGN_STRING(&Global_114135.f_20414[Global_114135.f_20414.f_145 /*16*/], sParam0, 16);
		TEXT_LABEL_ASSIGN_STRING(&(Global_114135.f_20414[Global_114135.f_20414.f_145 /*16*/].f_4), sParam1, 16);
		Global_114135.f_20414[Global_114135.f_20414.f_145 /*16*/].f_8 = unk_0x1DD05E817C89C737() + iParam3;
		Global_114135.f_20414[Global_114135.f_20414.f_145 /*16*/].f_9 = iParam5;
		Global_114135.f_20414[Global_114135.f_20414.f_145 /*16*/].f_11 = iParam6;
		Global_114135.f_20414[Global_114135.f_20414.f_145 /*16*/].f_12 = iParam2;
		Global_114135.f_20414[Global_114135.f_20414.f_145 /*16*/].f_13 = iParam7;
		Global_114135.f_20414[Global_114135.f_20414.f_145 /*16*/].f_14 = iParam8;
		Global_114135.f_20414[Global_114135.f_20414.f_145 /*16*/].f_15 = iParam9;
	
		if (iParam4 != -1)
			Global_114135.f_20414[Global_114135.f_20414.f_145 /*16*/].f_10 = unk_0x1DD05E817C89C737() + iParam3 + iParam4;
		else
			Global_114135.f_20414[Global_114135.f_20414.f_145 /*16*/].f_10 = -1;
	
		Global_114135.f_20414.f_145 = Global_114135.f_20414.f_145 + 1;
		func_203();
	}

	return;
}

void func_203() // Position - 0xA590
{
	int i;

	for (i = 0; i < 3; i = i + 1)
	{
		Global_114135.f_20414.f_146[i] = 0;
	}

	for (i = 0; i < Global_114135.f_20414.f_145; i = i + 1)
	{
		if (IS_BIT_SET(Global_114135.f_20414[i /*16*/].f_11, 0))
			if (Global_114135.f_20414[i /*16*/].f_12 > Global_114135.f_20414.f_146[0])
				Global_114135.f_20414.f_146[0] = Global_114135.f_20414[i /*16*/].f_12;
	
		if (IS_BIT_SET(Global_114135.f_20414[i /*16*/].f_11, 1))
			if (Global_114135.f_20414[i /*16*/].f_12 > Global_114135.f_20414.f_146[1])
				Global_114135.f_20414.f_146[1] = Global_114135.f_20414[i /*16*/].f_12;
	
		if (IS_BIT_SET(Global_114135.f_20414[i /*16*/].f_11, 2))
			if (Global_114135.f_20414[i /*16*/].f_12 > Global_114135.f_20414.f_146[2])
				Global_114135.f_20414.f_146[2] = Global_114135.f_20414[i /*16*/].f_12;
	}

	return;
}

BOOL func_204(int iParam0) // Position - 0xA6A7
{
	int num;
	int i;

	num = iParam0;

	for (i = 0; num > 31; i = i + 1)
	{
		num = num - 32;
	}

	if (i < 3)
		return IS_BIT_SET(Global_114135.f_20414.f_150[i], num);

	return false;
}

void func_205(int iParam0, int iParam1) // Position - 0xA6E7
{
	unk_0x0B0C9A0F9AAEB7F0(&Global_114135.f_24999.f_8[iParam0], iParam1);
	return;
}

int func_206() // Position - 0xA702
{
	return 1;
}

void func_207(int iParam0) // Position - 0xA70B
{
	Global_114121 = iParam0;
	return;
}

void func_208() // Position - 0xA719
{
	if (!bLocal_72 && iLocal_49 == 0)
	{
		if (!unk_0x4FAFF4BCB7633475(uLocal_52) && !unk_0x4FAFF4BCB7633475(uLocal_53))
		{
			unk_0x63C8DCBEC1CF8225(uLocal_52, unk_0xD1A6A821F5AC81DB(uLocal_53, 1), 3f, &_CAN_ENTER_FREEROAM_STATE, 1193033728, 1056964608);
			unk_0x44FB298D6382876D(uLocal_52, 1);
			SYSTEM::WAIT(0);
		}
	}

	return;
}

